---
TOCTitle: 'MS12-082'
Title: 'Microsoft 安全公告 MS12-082 - 重要'
ms:assetid: 'ms12-082'
ms:contentKeyID: 61238424
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms12-082(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS12-082 - 重要
==================================

DirectPlay 中的漏洞可能允许远程执行代码 (2770660)
-------------------------------------------------

发布时间: 2012年12月11日 | 更新时间: 2013年7月9日
...
对于 Microsoft Windows 所有受支持的版本（Windows RT 除外），此安全更新的等级为“重要”。有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

此安全更新可通过更正 DirectPlay 处理特制内容的方式来解决漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
对于管理员、企业安装或者想要手动安装此安全更新的最终用户，Microsoft 建议客户使用更新管理软件尽早应用此更新或者利用 [Microsoft Update](http://go.microsoft.com/fwlink/?linkid=40747) 服务检查更新。另请参阅本公告后面部分中的“**检测和部署工具及指导**”一节。
...
**安全更新的哈希位于何处？**
安全更新的 SHA1 和 SHA2 哈希可用于验证下载的安全更新程序包的真实性。有关与此更新相关的哈希信息，请参阅 [Microsoft 知识库文章 2770660](https://support.microsoft.com/kb/2770660)。
...
<th>
DirectPlay 堆溢出漏洞 - CVE-2012-1537
</th>
<th colspan="2">
综合严重等级
</th>
...
<span></span>
DirectPlay 处理特制内容的方式中存在一个远程执行代码漏洞。如果攻击者诱使用户查看包含嵌入内容的特制 Office 文档，则该漏洞可能允许远程执行代码。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

若要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2012-1537](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1537)。

#### DirectPlay 堆溢出漏洞 (CVE-2012-1537) 的缓解因素

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。以下缓解因素在您遇到的情形中可能会有所帮助：

-   成功利用此漏洞的攻击者可以获得与当前用户相同的用户权限。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。
-   特制 Office 文档文件可作为电子邮件附件发送，但攻击者必须说服用户打开附件以利用此漏洞。
-   默认情况下，DirectPlay ActiveX 控件未包括在 Internet Explorer 中的 ActiveX 控件的默认允许列表中。仅使用 ActiveX 选择性加入功能显式批准此控件的客户会受尝试利用此漏洞的威胁。但是，如果客户在 Internet Explorer 的以前版本中使用此 ActiveX 控件，则将启用此 ActiveX 控件以便在 Internet Explorer 中工作，即使客户尚未明确批准它使用 ActiveX 选择性加入功能。

#### DirectPlay 堆溢出漏洞 (CVE-2012-1537) 的变通办法

变通办法是指一种设置或配置更改，它不能从根本上纠正漏洞，但有助于在应用更新之前封堵已知的攻击源。Microsoft 已测试了以下变通方法，并在讨论中指明了变通方法是否会降低功能性：

-   **使用 Microsoft Office 文件阻止策略禁止打开来自未知或不可信来源和位置的** **Word** **2003 以及较早版本的文档**

    Microsoft Office 文件阻止策略可用于禁止在 Microsoft Word、Microsoft Excel 和 Microsoft PowerPoint 中打开特定格式类型。

    **对于 Microsoft** **Word** **2003 和 Microsoft** **Word** **2007**
...
<p> </p>
    <table style="border:1px solid black;">
    <caption>Microsoft Office 软件</caption>
    <tbody>
    <tr class="odd">
...
<p> </p>
    ...
    **如何撤消变通方法。**
    ...
    对于 Microsoft Word 2003 和 Microsoft Word 2007，使用以下注册表脚本撤消用于设置文件阻止策略的设置：
...
#### DirectPlay 堆溢出漏洞 (CVE-2012-1537) 的常见问题

**此漏洞的影响范围有多大？**
这是一个远程执行代码漏洞。

**造成此漏洞的原因是什么？**
当 Windows DirectPlay 未正确处理特制 Office 文档时，会导致该漏洞。

**哪个组件受此漏洞的影响？**
受此漏洞影响的组件是 Microsoft DirectX（Windows XP 所有受支持版本上的 DirectX 9.0；Windows Server 2003、Windows Vista 和 Windows Server 2008 所有受支持版本上的 DirectX 10.1；Windows 7 和 Windows Server 2008 R2 所有受支持版本上的 DirectX 11.0；以及 Windows 8 和 Windows Server 2012 所有受支持版本上的 DirectX 11.1）。

**什么是 DirectPlay？**
DirectPlay 是随 Microsoft DirectX 提供的网络协议，可帮助游戏开发人员制作多玩家网络游戏而无需实现复杂的网络协议。
...
**攻击者如何利用此** **漏洞？**
在电子邮件攻击情形中，攻击者可以通过向用户发送包含嵌入内容的特制 Office 文档并诱使用户打开该文件来利用此漏洞。

**受此漏洞威胁最大的系统有哪些？**
工作站和终端服务器受到的威胁最大。当应该没有足够管理权限的用户被授予登录到服务器并运行程序的权限时，服务器会更有危险。不过，最佳做法是不要授予这样的权限。

**此更新有什么作用？**
此更新可通过更正 DirectPlay 处理特制内容的方式来解决漏洞。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”项或 %Windir%\$NTUninstallKB2770660$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="http://support.microsoft.com/kb/2770660">Microsoft 知识库文章 2770660</a></td>
</tr>  
<tr class="even">
...
<td style="border:1px solid black;" colspan="2">
使用控制面板中的“**添加或删除程序**”项或 %Windir%\\$NTUninstallKB2770660$\\Spuninst 文件夹中的 Spuninst.exe 实用程序
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<td style="border:1px solid black;" colspan="2">
请参阅 [Microsoft 知识库文章 2770660](http://support.microsoft.com/kb/2770660)
</td>
</tr>
<tr class="alternateRow">
<td style="border:1px solid black;">
...
-   [Aniway](mailto:aniway.anyway@gmail.com) 与 [VeriSign iDefense Labs](http://labs.idefense.com/) 合作报告了 DirectPlay 堆溢出漏洞 (CVE-2012-1537)
...
-   V1.1（2012 年 12 月 12 日）： 纠正了所有受影响的软件的重新启动要求的条目。这仅仅是一个信息更改。没有更改安全更新文件。  
-   V1.2（2013 年 7 月 9 日）： 公告经过修订，宣布在 2770660 更新 的 Windows Vista 程序包中的一个检测更改，以解决一个 Windows Update 重新提供问题。这只是检测更改。已经成功更新了其系统的客户不需要执行任何操作。
