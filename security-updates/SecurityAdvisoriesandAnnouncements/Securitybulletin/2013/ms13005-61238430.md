---
TOCTitle: 'MS13-005'
Title: 'Microsoft 安全公告 MS13-005 - 重要'
ms:assetid: 'ms13-005'
ms:contentKeyID: 61238430
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms13-005(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS13-005 - 重要
==================================

Windows 内核模式驱动程序中的漏洞可能允许特权提升 (2778930)
----------------------------------------------------------

发布时间: 2013年1月8日 | 更新时间: 2013年2月12日
...
此安全更新通过更正 Windows 内核模式驱动程序处理窗口广播消息的方式来解决漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
**已知问题。**[Microsoft 知识库文章 2778930](http://support.microsoft.com/kb/2778930) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。本文还介绍了这些问题的建议解决办法。
...
<td style="border:1px solid black;">
[MS12-078](http://go.microsoft.com/fwlink/?linkid=271624) 中的 KB2779030 由 KB2778930 替代
</td>
</tr>
...
**安** **全更新的**哈希位于何处？
安全更新的 SHA1 和 SHA2 哈希可用于验证下载的安全更新程序包的真实性。有关与此更新相关的哈希信息，请参阅 [Microsoft 知识库文章 2778930](https://support.microsoft.com/kb/2778930)。
...
<th>
Win32k 不正确消息处理漏洞 - CVE-2013-0008
</th>
<th>
综合严重等级
</th>
...
如果 Windows 内核不正确地处理窗口广播消息，则存在一个特权提升漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2013-0008](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0008)。

#### Win32k 不正确消息处理漏洞 (CVE-2013-0008) 的缓解因素

攻击者必须拥有有效的登录凭据并能本地登录才能利用此漏洞。

#### Win32k 不正确消息处理漏洞 (CVE-2013-0008) 的变通办法

Microsoft 并未发现此漏洞的任何变通办法

#### Win32k 不正确消息处理漏洞 (CVE-2013-0008) 的常见问题

**此漏洞的影响范围有多大？**
这是一个特权提升漏洞。

**造成此漏洞的原因是什么？**
当 Windows 内核模式驱动程序不正确地处理窗口广播消息时，会导致该漏洞。
...
**攻击者可能利用此漏洞执行什么操作？**
成功利用此漏洞的攻击者可以在更高完整性级别 (IL) 进程的上下文中运行任意代码。如果用户使用管理用户权限登录，攻击者便可完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

**什么是 Windows 完整性机制？
**Windows 完整性机制是 Windows 安全体系结构的一个核心组件，它使用完整性级别 (IL) 来限制相同用户帐户下运行的并且不值得信赖的应用程序的访问权限。完整性级别表示正在运行的应用程序进程和对象（例如，由应用程序创建的文件）的可信赖程度。完整性机制向资源管理器（如文件系统）提供使用预定义的策略阻止完整性较低或可信赖程度较低的对象读取或修改完整性较高的对象能力。完整性机制允许 Windows 安全模型强制实施新的访问控制限制，而这些限制无法通过在访问控制列表 (ACL) 中向用户或组授予权限来定义。

**攻击者如何利用此** **漏洞？**
此漏洞要求攻击者诱使用户运行特制的应用程序。

**受此漏洞威胁最大的系统有哪些？**
工作站和终端服务器受到的威胁最大。如果管理员允许用户登录服务器并运行程序，服务器可能面临更大风险。不过，最佳做法是不要授予这样的权限。

**此更新有什么作用？**
此更新通过更正 Windows 内核模式驱动程序处理窗口广播消息的方式来解决漏洞。
...
<td style="border:1px solid black;">请参阅 <a href="http://support.microsoft.com/kb/2778930">Microsoft 知识库文章 2778930</a></td>
</tr>  
<tr class="odd">
...
| **文件信息**                     | 请参阅 [Microsoft 知识库文章 2778930](http://support.microsoft.com/kb/2778930)                               |
...
-   V1.1（2013 年 1 月 9 日）： 纠正了检测和部署摘要表。这仅仅是一个信息更改。已经成功更新了其系统的客户不需要执行任何操作。  
-   V1.2（2013 年 2 月 12 日）： 已在“摘要”中的“已知问题”下为 Microsoft 知识库文章 2778930 添加一个链接。
