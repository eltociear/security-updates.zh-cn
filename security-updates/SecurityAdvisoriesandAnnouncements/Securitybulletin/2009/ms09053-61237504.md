---
TOCTitle: 'MS09-053'
Title: 'Microsoft 安全公告 MS09-053 - 重要'
ms:assetid: 'ms09-053'
ms:contentKeyID: 61237504
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms09-053(v=Security.10)'
---
...
此安全更新通过修改 FTP 服务处理列表操作的方式来解决这些漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的“常见问题 (FAQ)”小节。

此安全更新也解决了最初在 [Microsoft 安全通报 975191](http://technet.microsoft.com/security/advisory/975191) 中描述的漏洞。
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2003 SP2（用于基于 Itanium 的系统）](http://technet.microsoft.com/security/bulletin/ms09-053)
</td>
<td style="border:1px solid black;">
Microsoft Internet Information Services 6.0
</td>
...
|-----------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|
| **操作系统**                                                                                                    | **组件**                                                      |
| Windows Vista、Windows Vista Service Pack 1 和 Windows Vista Service Pack 2                                     | Microsoft Internet Information Services 7.0（FTP 服务 7.5\*） |
| Windows Vista x64 Edition、Windows Vista x64 Edition Service Pack 1 和 Windows Vista x64 Edition Service Pack 2 | Microsoft Internet Information Services 7.0（FTP 服务 7.5\*） |
| Windows Server 2008（用于 32 位系统）和 Windows Server 2008（用于 32 位系统）Service Pack 2                     | Microsoft Internet Information Services 7.0（FTP 服务 7.5\*） |
| Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2           | Microsoft Internet Information Services 7.0（FTP 服务 7.5\*） |
| Windows 7（用于 32 位系统）                                                                                     | Microsoft Internet Information Services 7.5（FTP 服务 7.5）   |
| Windows 7（用于基于 x64 的系统）                                                                                | Microsoft Internet Information Services 7.5（FTP 服务 7.5）   |
| Windows Server 2008 R2（用于基于 x64 的系统）                                                                   | Microsoft Internet Information Services 7.5（FTP 服务 7.5）   |
| Windows Server 2008 R2（用于基于 Itanium 的系统）                                                               | Microsoft Internet Information Services 7.5（FTP 服务 7.5）   |

\*作为额外下载提供。 请参阅**常见问题**中的条目。
...
**我安装 FTP 时使用 Windows Server 2008 Server Manager。 我的系统是否受影响？**
当您使用 Windows Server 2008 Server Manager 安装 FTP 时，您安装的是 FTP Service 6.0，该系统受影响。 Microsoft 建议您安装此公告中的适当安全更新或升级到 FTP Service 7.5。

**用于 Windows** **Vista** **或 Windows Server 2008 的 FTP Service 7.5 是否受此漏洞的影响？**
否。 作为 Windows Vista 和 Windows Server 2008 的 32 位和基于 x64 版本的额外下载提供的 FTP 服务 7.5 不受此漏洞影响。 请参阅下载中心的以下站点：

-   [用于 IIS 7.0 (x86) 的 Microsoft FTP Service 7.5](http://www.microsoft.com/downloads/details.aspx?displaylang=en&familyid=b7f5b652-8c5c-447a-88b8-8cfc5c13f571)
-   [用于 IIS 7.0 (x64) 的 Microsoft FTP Service 7.5](http://www.microsoft.com/downloads/details.aspx?displaylang=en&familyid=ffb7c167-279e-48d3-8169-dea85784c4d1)
...
<th>
IIS FTP 服务 DoS 漏洞 - CVE-2009-2521
</th>
<th>
IIS FTP 服务 RCE 和 DoS 漏洞 - CVE-2009-3023
</th>
<th>
综合严重等级
</th>
...
<tr>
<td style="border:1px solid black;">
Microsoft Internet Information Services 5.0，位于  
...
<tr class="alternateRow">
<td style="border:1px solid black;">
Microsoft Internet Information Services 5.1，位于  
...
<tr>
<td style="border:1px solid black;">
Microsoft Internet Information Services 6.0，位于  
...
<tr class="alternateRow">
<td style="border:1px solid black;">
Microsoft Internet Information Services 7.0，位于  
...
Microsoft Internet Information Services (IIS) 5.0、Microsoft Internet Information Services (IIS) 5.1、Microsoft Internet Information Services (IIS) 6.0 和 Microsoft Internet Information Services (IIS) 7.0 的 FTP 服务中存在一个漏洞。 该漏洞可能允许拒绝服务 (DoS)。

要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-2521](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2521)。

#### IIS FTP 服务 DoS 漏洞的缓解因素 - CVE-2009-2521

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：

-   默认情况下，Microsoft Windows 2000、Windows XP、Windows Vista、Windows Server 2003、Windows Server 2008 和 Windows 7 的所有受支持版本均未安装 FTP 服务。

#### IIS FTP 服务 DoS 漏洞的变通方法 - CVE-2009-2521
...
-   **对匿名用户拒绝 FTP 写入访问**
    ...
    此变通办法阻止来自匿名用户的拒绝服务攻击。

    此变通办法阻止已知利用并最小化此攻击媒介的任何变化的影响。 但是，此变通变法不会使漏洞完全无法利用。

    默认情况下，匿名用户不被授予 FTP 写入权限。 如果在 FTP 服务器上授予了匿名写入权限，管理员可以修改 IIS 权限以阻止匿名写入权限。 通过此变通办法，匿名用户将无法造成远程执行代码。

    要修改 IIS 权限以阻止为匿名用户授予 FTP 写入权限，请执行以下步骤：
    ...
    **变通办法的影响：**匿名用户将无法使用 FTP 服务在服务器上发布内容。
...
-   **对匿名用户拒绝 FTP 访问**
    ...
    此变通办法阻止来自没有有效用户帐户的未经身份验证的用户的拒绝服务攻击。

    此变通办法阻止已知利用并最小化此攻击媒介的任何变化的影响。 但是，此变通变法不会使漏洞完全无法利用。

    默认情况下，匿名用户不被授予 FTP 写入权限。 如果在 FTP 服务器上授予了匿名写入权限，管理员可以修改 IIS 权限以阻止匿名写入权限。 通过此变通办法，匿名用户将无法利用此漏洞。

    要修改 IIS 权限以阻止为匿名用户授予 FTP 写入权限，请执行以下步骤：

    1.  启动 IIS 管理器。
    2.  右键单击“**默认 FTP 站点**”并指向“**属性**”。
    3.  单击“**安全帐户**”选项卡。
    4.  确保“**允许匿名连接**”没有被选中。

    **变通办法的影响：**匿名用户将没有访问 FTP 服务的权限。
...
-   **修改 NTFS 文件系统权限以禁止 FTP 用户创建目录**

    此变通办法阻止来自验证用户的拒绝服务攻击。

    此变通办法阻止已知利用并最小化此攻击媒介的任何变化的影响。 但是，此变通变法不会使漏洞完全无法利用。

    管理员可以修改对服务器上托管的 FTP 站点的根目录的 NTFS 文件系统权限以禁止 FTP 用户创建目录。 此修改仍允许 FTP 用户向现有目录上载文件。 通过此变通办法，经过验证和未经验证的用户都将无法造成远程执行代码。

    作为管理员执行以下步骤从用户组中删除目录创建权限。 如果您使用一个配置的 FTP 用户或自定义组来管理您的 FTP 用户，将下面第 5 步中的用户组替换为这些自定义身份。

    1.  浏览到您的 FTP 站点的根目录。 默认情况下，此目录位于 %systemroot%\\inetpub\\ftproot。
    2.  右键单击该目录并选择“**属性**”。
    3.  单击“**安全**”选项卡，然后单击“**高级**”。
    4.  单击“**更改权限**”。
    5.  选择**用户**组并单击“**编辑**”。
    ...
    **变通办法的影响：**FTP 用户将不能通过 FTP 服务创建目录。 FTP 用户将仍能够通过 FTP 服务向现有目录上载文件。

-   **升级到 FTP Service 7.5**

    FTP Service 7.5 适用于 Windows Vista 和 Windows Server 2008。 FTP Service 的该版本不受此公告中漏洞的影响。 请参阅下载中心的以下站点：

    -   [用于 IIS 7.0 (x86) 的 Microsoft FTP Service 7.5](http://www.microsoft.com/downloads/details.aspx?displaylang=en&familyid=b7f5b652-8c5c-447a-88b8-8cfc5c13f571)
    -   [用于 IIS 7.0 (x64) 的 Microsoft FTP Service 7.5](http://www.microsoft.com/downloads/details.aspx?displaylang=en&familyid=ffb7c167-279e-48d3-8169-dea85784c4d1)

    **变通办法的影响：**FTP 站点需要从 FTP Service 6.0 迁移到 FTP Service 7.5。

#### IIS FTP 服务 DoS 漏洞的常见问题 - CVE-2009-2521

**此漏洞的影响范围有多大？**??
这是一个拒绝服务漏洞。 利用此漏洞的攻击者可导致受影响的服务停止响应和自动重新启动。 请注意，虽然攻击者无法利用拒绝服务漏洞来执行代码或提升他们的用户权限，但此漏洞可能导致受影响的系统停止接受请求。

**造成漏洞的原因是什么？**??
此漏洞由 IIS 中的 Microsoft FTP 服务处理列表命令的方式导致。

**攻击者可能利用此漏洞执行什么操作？**??
成功利用此漏洞的攻击者可使用户系统变得无响应并且重新启动。

**攻击者如何利用此漏洞**??
攻击者可通过向服务器发送特制的列表命令来利用此漏洞。

**受此漏洞威胁最大的系统有哪些？**??
运行 Microsoft 文件传输协议服务 (FTP) 的服务器系统。

**此更新有什么作用？**??
此更新将修改 Microsoft FTP 服务器处理列表命令操作的方式。

**发布此安全公告时，此漏洞是否已公开披露？**??
是。 此漏洞已公开披露。 已为此漏洞分配常见漏洞和披露号码 [CVE-2009-2521](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2521)。

**发布此安全公告时，Microsoft 是否收到任何有关此漏洞已被利用的报告？**??
是。 Microsoft 获悉尝试使用此漏洞的有限目标攻击。

**应用此安全更新有助于客户防止尝试利用此漏洞的已公开发布代码吗？**?
是。 此安全更新可消除目前被利用的漏洞。 已为消除的漏洞分配了常见漏洞和披露号码 [CVE-2009-2521](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2521)。

IIS FTP 服务 RCE 和 DoS 漏洞 - CVE-2009-3023
--------------------------------------------

Microsoft Internet Information Services (IIS) 5.0、Microsoft Internet Information Services (IIS) 5.1 和 Microsoft Internet Information Services (IIS) 6.0 的 FTP 服务中存在一个漏洞。 该漏洞可能允许对在 IIS 5.0 上运行 FTP 服务的系统远程执行代码 (RCE)，或对在 IIS 5.1 或 IIS 6.0 上运行 FTP 服务的系统拒绝服务 (DoS)。

要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-3023](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-3023)。

#### IIS FTP 服务 RCE 和 DoS 漏洞的缓解因素 - CVE-2009-3023

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：

-   默认情况下，Microsoft Windows 2000、Windows XP、Windows Vista、Windows Server 2003、Windows Server 2008 和 Windows 7 的所有受支持版本均未安装 FTP 服务。
-   Window XP 上的 Internet Information Services 5.1 和 Windows Server 2003 上的 Internet Information Services 6.0 受到的威胁较小，因为这些版本是使用 /GS 编译器选项编译的。 这不会删除漏洞但会使对漏洞的利用变得更加困难并减少拒绝服务的影响。
-   Internet Information Services 7.0 不受此漏洞影响。

#### IIS FTP 服务 RCE 和 DoS 漏洞的变通方法 - CVE-2009-3023
...
-   **对匿名用户拒绝 FTP 写入访问**
    ...
    此变通办法阻止来自匿名用户的远程执行代码攻击。 通过这个变通办法，IIS 的所有受影响版本仍然容易受到来自匿名用户的拒绝服务攻击。
...
-   **修改 NTFS 文件系统权限以禁止 FTP 用户创建目录**
    ...
    此变通办法阻止来自经过验证的用户的远程执行代码攻击。 通过这个变通办法，IIS 服务器的所有受影响版本仍然容易受到来自经过验证的用户的拒绝服务攻击。
...
#### IIS FTP 服务 RCE 和 DoS 漏洞的常见问题 - CVE-2009-3023

**此漏洞的影响范围有多大？**??
这是一个拒绝服务漏洞。 利用此漏洞的攻击者可导致受影响的服务停止响应和自动重新启动。 请注意，虽然攻击者无法利用拒绝服务漏洞来执行代码或提升他们的用户权限，但此漏洞可能导致受影响的系统停止接受请求。

在运行 Microsoft Windows 2000 Service Pack 4 的系统上，这是一个远程执行代码漏洞；在所有其他受影响的平台上，这是一个拒绝服务漏洞。 在 Microsoft Windows 2000 Service Pack 4 上的 IIS 中成功利用此漏洞的攻击者可导致在 LocalSystem 的上下文中远程执行代码。 在 IIS 5.1 和 IIS 6.0 上成功利用此漏洞的攻击者可导致 IIS FTP 服务停止，变得没有响应。

**造成漏洞的原因是什么？**??
此漏洞由 IIS 中的 Microsoft FTP 服务处理列表命令的方式导致。

**攻击者可能利用此漏洞执行什么操作？**??
访问 FTP 服务的攻击者可以利用此漏洞造成基于堆栈的溢出，从而允许在运行 IIS 5.0 的系统上的 LocalSystem 帐户上下文中执行任意代码，或在运行 IIS 5.1、IIS 6.0 的受影响系统上拒绝服务。 在允许匿名验证的 FTP 服务的配置中，攻击者不需要通过身份验证即可利用漏洞。 有关 LocalSystem 帐户的详细信息，请参阅 MSDN 文章 [LocalSystem 帐户](http://msdn.microsoft.com/en-us/library/ms684190(vs.85).aspx)。

**攻击者如何利用此漏洞**??
能够访问 FTP 服务的匿名用户或经验证用户可发送 FTP NLST 命令，从而导致在 Microsoft Windows 2000 上运行 IIS 5.0 的系统发生内存损坏和远程执行代码，以及在其他平台上运行 IIS 5.1 或 IIS 6.0 的系统拒绝服务。
...
**发布此安全公告时，此漏洞是否已公开披露？**??
是。 此漏洞已公开披露。 已为此漏洞分配常见漏洞和披露号码 [CVE-2009-3023](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-3023)。

**发布此安全公告时，Microsoft 是否收到任何有关此漏洞已被利用的报告？**??
是。 Microsoft 获悉尝试使用此漏洞的有限目标攻击。

**应用此安全更新有助于客户防止尝试利用此漏洞的已公开发布代码吗？**??
是。 此安全更新可消除目前被利用的漏洞。 已为消除的漏洞分配了常见漏洞和披露号码 [CVE-2009-3023](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-3023)。
...
| **部署**                         | ?                                                                                                                     |
| 没有用户干预的安装               | Windows2000-kb975254-x86-enu /quiet                                                                                   |
| 在不重新启动的情况下安装         | Windows2000-kb975254-x86-enu /norestart                                                                               |
| 更新日志文件                     | kb975254.log                                                                                                          |
| 更多信息                         | 请参阅“**检测和部署工具及指导**”小节                                                                                  |
| **重新启动要求**                 | ?                                                                                                                     |
| 要求重新启动？                   | 是，应用此安全更新后必须重新启动系统                                                                                  |
| HotPatching                      | 不适用                                                                                                                |
| **删除信息**                     | 使用控制面板中的“**添加或删除程序**”工具或 %Windir%\\$NTUninstallKB975254$\\Spuninst 文件夹中的 Spuninst.exe 实用程序 |
| **文件信息**                     | 请参阅 [Microsoft 知识库文章 975254](http://support.microsoft.com/kb/975254)                                          |
| **注册表项验证**                 | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB975254\\Filelist                             |
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB975254$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="http://support.microsoft.com/kb/975254">Microsoft 知识库文章 975254</a></td>
</tr>  
<tr class="even">
...
-   Kingcope 报告了 IIS FTP 服务 DoS 漏洞 (CVE-2009-2521)  
-   Kingcope 报告了 IIS FTP 服务 RCE 和 DoS 漏洞 (CVE-2009-3023)
