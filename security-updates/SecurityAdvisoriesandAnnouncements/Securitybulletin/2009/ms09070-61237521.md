---
TOCTitle: 'MS09-070'
Title: 'Microsoft 安全公告 MS09-070 - 重要'
ms:assetid: 'ms09-070'
ms:contentKeyID: 61237521
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms09-070(v=Security.10)'
---
...
**已知问题。** [Microsoft 知识库文章 971726](http://support.microsoft.com/kb/971726) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。 本文还介绍了这些问题的建议解决办法。
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=31351b9e-b5bb-4618-990b-1089ea5a3bc2)<sup>[1]</sup>                                                                 | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=b6eb9d9b-1a43-4b30-a033-19a1db786244)<sup>[2]</sup>                                                     | 远程执行代码 | 重要         | 无               |
| [Windows Server 2008（用于 32 位系统）和 Windows Server 2008（用于 32 位系统）Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=f6715abb-fd93-44ba-9854-2ecc672622da)\*           | 远程执行代码 | 重要         | 无               |
| [Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=7d1f5e9e-a7de-4f96-89c8-510fd51f16e7)\* | 远程执行代码 | 重要         | 无               |
...
[Microsoft 知识库文章 971726](http://support.microsoft.com/kb/971726) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。 本文还介绍了这些问题的建议解决办法。

**此更新为何消除了多个已报告的安全漏洞？**  
此更新之所以包含对某些漏洞的支持，是因为解决这些问题所需的修改位于相关的文件中。 客户只需安装此更新就可以了，而不用安装多个几乎相同的更新。

**Active Directory Federation Services 是什么？**  
Active Directory Federation Services (ADFS) 基于业界支持的 新 Web 服务体系结构，WS-\* 规范中对其进行了定义。 ADFS 帮助您在单一联机会话中使用单一登录 (SSO) 验证多个相关 Web 应用程序的用户。 ADFS 通过在安全和企业边界中安全共享数字身份和所有权来实现。

**我如何验证我的服务器上是否安装了 ADFS？ **
要验证服务器上安装了 ADFS，请依次单击“**开始**”、“**控制面板**”和“**添加/删除程序**”，双击“**Windows 组件**”，然后查找作为已安装组件的 **Federation Services**。

**Windows Server 2003 R2 是什么？**  
Windows Server 2003 R2 扩展了 Windows Server 2003 操作系统，提供了更高效的方式来管理和控制对本地和远程资源的访问，同时能够轻松地将其整合到您现有的 Windows Server 2003 环境中。 Windows Server 2003 R2 提供可伸缩且安全性增强的 Web 平台、与基于 UNIX 系统的无缝互操作性，并实现了包括简化分支服务器管理、改进身份与访问管理和更高效的存储管理的新方案。 Windows Server 2003 R2 Datacenter Edition 和 Windows Server 2003 R2 Enterprise Edition 也提供了动态的新许可，允许客户从服务器的虚拟化中获得更大的价值。

有关 Windows Server 2003 R2 的具体信息，请参阅 [Windows Server 2003 R2](http://www.microsoft.com/windowsserver2003/default.mspx) 站点。

**WS-\* web 规范是什么？**

基于简单对象访问协议 (SOAP)、XML、Web 服务描述语言 (WSDL) 和通用描述、发现和集成 (UDDI) 等行业标准的 Web 服务体系结构的规范。 WS-\* 为向扩展企业提供包括管理联合身份和安全性在内的完整、互操作业务解决方案建立了基础。

Web 服务模型的理念基础是以不同的语言、编程模型书写可在不同设备上运行并可通过其访问的企业系统。 Web 服务是一种建立分布式系统的方式，它能够在 Internet 中高效、轻松地与其他系统进行连接和交互，不受系统书写语言和运行平台的限制。
...
<tr class="header">
<th>受影响的软件</th>
<th>在 ADFS 漏洞 - CVE-2009-2508 中的单一登录欺骗</th>
<th>ADFS 漏洞 - CVE-2009-2509 中的远程代码执行</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
在 ADFS 漏洞 - CVE-2009-2508 中的单一登录欺骗  
---------------------------------------------
  
  
如果攻击者具有目标用户最近使用的工作站和 Web 浏览器的访问权限，则 Active Directory Federation Services 中的欺骗漏洞可能允许攻击者模拟成经身份验证的用户来访问提供单一登录的 Web 站点。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-2508](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2508)。
  
#### ADFS 漏洞中单一登录欺骗的缓解因素 - CVE-2009-2508
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   攻击者无法选择模拟的对象。 攻击者只能模拟能获取其验证令牌的对象，例如网吧。  
-   只有在服务器上的会话超时前攻击才能成功。 默认情况下，会话在服务器上保持打开的时间为 600 分钟。 如果攻击者企图在超时后再次使用受害者的身份验证令牌，攻击将失败。  
-   攻击者必须具有之前用户在该终端上使用的身份验证令牌的访问权限。 在已启用 HTTPS 的服务器中，该身份验证令牌将在传输中受到保护，攻击者必须具有受害者计算机（例如网吧）的访问权限才能攻击漏洞。 在网吧中，用户可能登录到 ADFS 单一登录 (SSO) Web 应用程序，然后注销应用程序。 然后，攻击者便可象前面用户一样使用计算机访问 Web 应用程序，不受用户是否登出的限制。
  
#### ADFS 漏洞 - CVE-2009-2508 中单一登录欺骗的解决方案
  
Microsoft 并未发现此漏洞的任何变通办法。
  
#### ADFS 漏洞 - CVE-2009-2508 中单一登录欺骗的常见问题
  
**此漏洞的影响范围有多大？**    
这是欺骗漏洞。 成功利用此漏洞的攻击者可以模拟其他用户，允许攻击者和目标用户一样访问使用 Active Directory Federation Services (ADFS) 单一登录功能的 Web 站点。
  
**造成漏洞的原因是什么？**    
此漏洞存在的原因是 ADFS 缺乏充分的会话管理验证。
  
**攻击者可能利用此漏洞执行什么操作？**    
在网吧等具有共享访问权限的系统中，成功利用此漏洞的攻击者可以访问之前经过验证的的用户的资源。 只有在 ADFS 管理员配置的会话超时期间此攻击才能执行，默认情况下配置为 600 分钟后超时。
  
**攻击者如何利用此漏洞？**    
攻击者利用此漏洞的方法：从浏览器的缓存访问特定数据，并重新将这些数据用于实施 ADFS 单一登录 (SSO) 的 Web 应用程序的身份验证。 当服务器上的会话超时后，ADFS 将禁用服务器端上经身份验证的会话，并不再允许攻击者再次使用这些凭据。
  
**受此漏洞威胁最大的系统有哪些？**    
将 ADFS 用于单一登录用途的 Web 服务器受此漏洞影响。 此漏洞不会直接影响客户端系统。
  
**此更新有什么作用？**    
更新将通过在用户注销网络时适当地放弃身份验证凭证来解决漏洞问题。
...
ADFS 漏洞 - CVE-2009-2509 中的远程代码执行  
------------------------------------------
  
  
在实施 Active Directory Federation Services (ADFS) 时存在远程代码执行漏洞。 漏洞是在经身份验证的用户连接到启用 ADFS 的 Web 服务器时不正确地验证请求标头引起的。 成功利用此漏洞的攻击者可以完全控制受影响的系统。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-2509](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-2509)。
  
#### ADFS 漏洞 - CVE-2009-2509 中远程代码执行的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   攻击者必须获得易受攻击服务器的有效登录凭证。 只有对支持 ADFS 的服务器成功进行身份验证后才能执行攻击。
  
#### ADFS 漏洞 - CVE-2009-2509 中的远程代码执行的解决方案
  
Microsoft 并未发现此漏洞的任何变通办法。
  
#### ADFS 漏洞 - CVE-2009-2509 中的远程代码执行的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者然后能够使用与工作进程标识 (WPI) 同样的权限在 IIS 服务器上执行任何操作。默认情况下，WPI 使用网络服务帐户权限进行配置。
  
**造成漏洞的原因是什么？**    
此漏洞存在的原因是 ADFS 未正确验证经过验证的客户端提交的请求标头。
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用此漏洞的攻击者然后能够使用与工作进程标识 (WPI) 同样的权限在 IIS 服务器上执行任何操作。 默认情况下，WPI 使用网络服务帐户权限进行配置。 如果带有 ASP 页的 IIS 服务器的应用程序池使用 WPI 进行配置，而 WPI 使用具有管理权限的帐户，这些 IIS 服务器与应用程序池使用默认 WPI 设置进行配置的 IIS 服务器相比，受到的影响更为严重。
  
**攻击者如何利用此漏洞？**    
攻击者可能通过向已启用 ADFS 的 Web 服务器发送特制 HTTP 请求来试图利用该漏洞。 攻击者需要成为经过身份验证的用户才能利用此漏洞。
  
**受此漏洞威胁最大的系统有哪些？**    
已启用 ADFS 的 Web 服务器受到威胁。 客户端系统不受此漏洞影响。
  
**此更新有什么作用？**    
此更新更正了已启用 ADFS 的 Web 服务器应用到 Web 客户端提交的请求标头的验证。
...
<td style="border:1px solid black;">对于带 ADFS 的 Windows Server 2003 R2 所有受支持的 32 位版本：<br />
WindowsServer2003-KB971726-x86-enu /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">对于带 ADFS 的 Windows Server 2003 R2 所有受支持的基于 x64 的版本：<br />
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\$NTUninstallKB971726$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="http://support.microsoft.com/kb/971726">Microsoft 知识库文章 971726</a></td>
</tr>  
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;">没有用户干预的安装</td>
<td style="border:1px solid black;">对于带 ADFS 的 Windows Server 2008 所有受支持的 32 位版本：<br />
Windows6.0-KB971726-x86 /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">对于带 ADFS 的 Windows Server 2008 所有受支持的基于 x64 的版本：<br />
...
-   V1.1（2009 年 12 月 9 日）： 纠正了 SMS 表中适用于 Windows Server 2003 x64 Edition Service Pack 2 的 **SMS 2.0** 和 **SMS 2003 with SUIT** 条目。 这仅仅是一个信息更改。
