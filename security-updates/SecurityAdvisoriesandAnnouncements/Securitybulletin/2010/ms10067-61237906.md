---
TOCTitle: 'MS10-067'
Title: 'Microsoft 安全公告 MS10-067 - 重要'
ms:assetid: 'ms10-067'
ms:contentKeyID: 61237906
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms10-067(v=Security.10)'
---



Microsoft 安全公告 MS10-067 - 重要
==================================

写字板文本转换器中的漏洞可能允许远程执行代码 (2259922)
------------------------------------------------------
...
此安全更新通过更改写字板文本转换器处理特制文件的的方式来消除漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows XP Service Pack 3](http://www.microsoft.com/downloads/details.aspx?familyid=fc4369ec-864a-42ae-a850-b006872241fe)                          | 远程执行代码 | 重要         | 无               |
| [Windows XP Professional x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=7567986a-261d-4def-9fa5-c667994c7844) | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=3290e7ee-1e4a-464c-abfd-17fc4108601e)                 | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=b21570cc-4f90-4ed8-b901-a34584d44a63)     | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](http://www.microsoft.com/downloads/details.aspx?familyid=c9c4427d-54c8-4f3c-9a94-818196cd5180) | 远程执行代码 | 重要         | 无               |
...
<th style="border:1px solid black;">写字板 Word 97 文本转换器内存损坏漏洞 - CVE-2010-2563</th>
...
写字板 Word 97 文本转换器内存损坏漏洞 - CVE-2010-2563  
-----------------------------------------------------
  
  
Microsoft 写字板在分析特制的 Word 97 文档时处理内存的方式中存在一个远程执行代码漏洞。 如果用户打开带有格式错误的结构的特制 Word 文件，此漏洞可能允许远程执行代码。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-2563](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2563)。
  
#### 写字板 Word 97 文本转换器内存损坏漏洞 (CVE-2010-2563) 的缓解因素
...
-   不会通过电子邮件自动利用此漏洞。 用户必须打开电子邮件附件，以电子邮件为载体的攻击才会得逞。 除非用户之前取消选中了选项“在打开这种类型的文件前始终询问”，否则系统可能会提示用户决定是否要打开文件。  
-   默认情况下，安装了 Microsoft Word 的系统通常使用 Microsoft Office（而不是写字板）打开 Word 文档。
  
#### 写字板 Word 97 文本转换器内存损坏漏洞 (CVE-2010-2563) 的变通方法
...
#### 写字板 Word 97 文本转换器内存损坏漏洞 (CVE-2010-2563) 的常见问题
  
**此漏洞的影响范围有多大？**  
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全远程控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成漏洞的原因是什么？**  
漏洞是由于写字板文本转换器未正确分析 Word 97 文档中的特定字段造成的。
...
在基于 Web 的攻击情形中，攻击者必须拥有一个网站，其中包含用来试图利用此漏洞的 Word 97 文件。 另外，接受或宿主用户提供的内容的网站以及受到破坏的网站可能包含可能利用此漏洞的特制内容。 攻击者无法强迫用户访问特制的网站。 相反，攻击者必须诱使用户访问该网站，所采用的方式通常是让用户单击指向攻击者网站的链接。 用户也将收到提示，并需要手动接受打开文档。
  
**受此漏洞威胁最大的系统有哪些？**  
写字板在其中用于打开来自不受信任来源的 Word 文档的受影响系统（如工作站或终端服务器）受到的威胁最大。 如果管理员允许用户登录服务器并运行程序，服务器可能面临更大风险。 不过，最佳做法是不要授予这样的权限。 默认情况下，安装了 Microsoft Word 的系统通常使用 Microsoft Office（而不是写字板）打开 Word 文档。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2259922$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="http://support.microsoft.com/kb/2259922">Microsoft 知识库文章 2259922</a></td>
</tr>  
<tr class="even">
...
-   [iDefense Labs](http://labs.idefense.com/)的 S0lute 报告了写字板 Word 97 文本转换器内存损坏漏洞 (CVE-2010-2563)
