---
TOCTitle: 'MS08-065'
Title: 'Microsoft 安全公告 MS08-065 - 重要'
ms:assetid: 'ms08-065'
ms:contentKeyID: 61237438
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms08-065(v=Security.10)'
---
...
对于 Microsoft Windows 2000 的所有受支持版本，此安全更新等级为“重要”。有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

此安全更新通过替换由 MSMQ 处理的容易受到攻击的字符串 APIs 来解决该漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|----------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Microsoft Windows 2000 Service Pack 4](http://www.microsoft.com/downloads/details.aspx?familyid=899e2728-2433-4ccb-a195-05b5d65e5469) | 远程执行代码 | 重要         | MS07-065         |
...
**在哪里可以找到文件信息详细资料？**  
可以在 [Microsoft 知识库文章 951071](http://support.microsoft.com/kb/951071) 中找到文件信息详细资料。
...
<tr class="header">
<th>受影响的软件</th>
<th>消息队列服务远程执行代码漏洞 - CVE-2008-3479</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
消息队列服务远程执行代码漏洞 - CVE-2008-3479  
--------------------------------------------
  
  
由于在分析消息队列服务的 RPC 请求时存在特定的缺陷，使得消息队列服务中出现了远程执行代码漏洞。
  
攻击者可能通过发送特制的 RPC 请求来利用此漏洞。 他们会控制堆请求，然后在未经检查的字符串复制操作过程中将其溢出。 成功利用此问题便可完全访问系统上下文中受影响的系统。 成功利用此漏洞的攻击者可以完全控制受影响的系统。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2008-3479](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-3479)。
  
#### 消息队列服务远程执行代码漏洞 (CVE-2008-3479) 的缓解因素
...
#### 消息队列服务远程执行代码漏洞 (CVE-2008-3479) 的变通方法
...
#### 消息队列服务远程执行代码漏洞 (CVE-2008-3479) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是启用了 MSMQ 服务的 Microsoft Windows 2000 系统的一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全远程控制受影响的系统。 然后，攻击者可以安装程序，或者，查看、更改或删除数据。
  
**造成漏洞的原因是什么？**    
MSMQ 服务分析特制 RPC 请求的方式不正确。
...
**什么是 RPC？**    
Microsoft RPC 是分布式计算环境中的一种编程模型。 RPC 的目标是提供透明的通信以便客户端好象直接在与服务器进行通讯。 Microsoft 实施的 RPC 可与开放软件基金会（OSF）分布式计算环境 (DCE) RPC 兼容。 有关 RPC 的详细信息，请访问 [RPC MSDN 网站](http://msdn.microsoft.com/en-us/library/ms691207(vs.85).aspx)。
  
**攻击者可能利用此漏洞执行什么操作？**    
未经身份验证的攻击者可能通过向 MSMQ 服务发送特制的 RPC 请求，从而使得任意代码可以在系统上下文中远程运行。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**攻击者如何利用此漏洞？**    
攻击者可能通过向 MSMQ 服务发送特制的 RPC 请求来利用此漏洞。 他们会控制堆请求，然后在未经检查的字符串复制操作过程中将其溢出。 成功利用此漏洞便可完全访问系统上下文中受影响的系统。
  
**受此漏洞威胁最大的系统有哪些？**    
启用了 MSMQ 服务的 Microsoft Windows 2000 系统受到的威胁最大。
  
**此更新有什么作用？**    
此更新通过修改 MSMQ 服务验证 MSMQ 调用的字符串 APIs 的参数的方式来消除此漏洞。
...
<td style="border:1px solid black;">Microsoft Windows 2000 Service Pack 4：<br />
使用控制面板中的“添加或删除程序”工具或 %Windir%\$NTUninstallKB951071$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="http://support.microsoft.com/kb/951071">Microsoft 知识库文章 951071</a></td>
</tr>  
<tr class="even">
...
-   [TippingPoint](http://www.tippingpoint.com/) 和 [Zero Day Initiative](http://www.zerodayinitiative.com/) 报告了 MSMQ 远程执行代码漏洞 (CVE-2008-3479)
