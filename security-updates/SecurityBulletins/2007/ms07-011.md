---
TOCTitle: 'MS07-011'
Title: 'Microsoft 安全公告 MS07-011 - 重要'
ms:assetid: 'ms07-011'
ms:contentKeyID: 61236863
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms07-011(v=Security.10)'
---



Microsoft 安全公告 MS07-011 - 重要
==================================

Microsoft OLE Dialog 中的漏洞可能允许远程执行代码 (926436)
----------------------------------------------------------
...
-   Microsoft Windows 2000 Service Pack 4 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=7b1a81d5-1072-49d9-a24a-0e2630f62d8c)
-   Microsoft Windows XP Service Pack 2 - [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=e9b84661-25e3-4d38-95b1-8d3e7af565aa)
-   Microsoft Windows XP Professional x64 Edition - [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=57c1b19f-3242-457c-bedf-d35a8efe525c)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 - [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=eaed6f59-801e-45d7-9518-469d0de13cad)
-   Microsoft Windows Server 2003（用于基于 Itanium 的系统）和 Microsoft Windows Server 2003 SP1（用于基于 Itanium 的系统）- [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=cd1b18ae-bc8d-4d73-847f-4fa7ca672c88)
-   Microsoft Windows Server 2003 x64 Edition - [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=11f4f8f6-b8ce-4a5f-b7ed-8389ccc56473)
...
| 漏洞标识符                                                                                             | 漏洞的影响   | Windows 2000 Service Pack 4 | Windows XP Service Pack 2 | Windows Server 2003 和 Windows Server 2003 Service Pack 1 |
|--------------------------------------------------------------------------------------------------------|--------------|-----------------------------|---------------------------|-----------------------------------------------------------|
| OLE Dialog 内存损坏漏洞 - [CVE-2007-0026](https://cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-0026) | 远程执行代码 | 重要                        | 重要                      | 重要                                                      |
...
**此安全更新解决的漏洞与 MS07-012 和 MS07-013 中解决的漏洞是否相同？**  
...
**我是否需要在部署 MS07-012 和 MS07-013 之前部署此安全更新？**  
...
| Microsoft Windows Server 2003（用于基于 Itanium 的系统）和 Microsoft Windows Server 2003 SP1（用于基于 Itanium 的系统） | 否         | 是       |
| Microsoft Windows Server 2003 x64                                                                                       | 否         | 是       |
...
#### OLE Dialog 内存损坏漏洞 - CVE-2007-0026：

Microsoft Windows 提供的 OLE Dialog 组件 中存在一个远程执行代码漏洞。 当用户与 RTF 文件或 RTF 电子邮件中格式错误的嵌入式 OLE 对象进行交互时，攻击者可能企图利用此漏洞。

#### OLE Dialog 内存损坏漏洞 (CVE-2007-0026) 的缓解因素：
...
-   此漏洞无法通过电子邮件自动加以利用。 用户必须打开在电子邮件中发送的附件，并与在 RTF 文件中嵌入的 OLE 对象进行交互，或与 RTF 电子邮件中嵌入的 OLE 对象进行互动，攻击才会成功。

#### OLE Dialog 内存损坏漏洞 (CVE-2007-0026) 的变通办法：
...
#### OLE Dialog 内存损坏漏洞 (CVE-2007-0026) 的常见问题解答：

**此漏洞的影响范围有多大？**  
Microsoft Windows 提供的 OLE Dialog 组件 中存在一个远程执行代码漏洞。 当用户与 RTF 文件或 RTF 电子邮件中格式错误的嵌入式 OLE 对象进行交互时，攻击者可能会利用此漏洞。 如果用户使用管理用户权限登录，成功利用此漏洞的攻击者便可完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。 那些帐户被配置为拥有较少系统用户权限的用户比具有管理权限的用户受到的影响要小。

**什么是 RTF 格式？**  
RTF 格式规范为文本和图形交换提供一种格式，可与不同的输出设备、操作环境和操作系统一起使用。

**什么是 OLE？**  
通过使用 OLE 技术，应用程序可以提供嵌入和链接支持。 OLE 是应用程序用于创建和编辑复合文档的技术。 这些可以是一种格式（例如 Microsoft Word 文档）的文档，包含另一种格式（例如 Microsoft Excel）文档的嵌入或链接。 OLE 2.0 比 OLE 前进了一大步，允许就地编辑。 在激活 OLE 对象时，用户会查看现有应用程序内的一组新菜单项，而非启动新应用程序。 有关 OLE 的详细信息，请访问以下 MSDN 网站。

**造成漏洞的原因是什么？**  
Windows OLE Dialog 组件在分析 RTF 文件中嵌入的 OLE 对象时没有执行足够的验证。 当用户与 RTF 文件中嵌入的格式错误的 OLE 对象进行交互时，它可能以一种攻击者可以执行任意代码的方式来破坏系统内存。
...
**此更新有什么作用？**  
此更新通过修改 Windows OLE Dialog 组件分析 RTF 文件中嵌入的 OLE 流的方式来消除此漏洞。
...
**注意** 使用 **/quiet** 开关将抑制所有消息。 这包括抑制失败消息。 管理员应该使用其中一种支持的方法，验证使用 **/quiet** 开关时安装是否成功。 管理员还应该检查 KB926436.log 文件，查看使用此开关时的任何失败消息。
...
系统管理员也可以使用 Spuninst.exe 实用工具删除此安全更新。 Spuninst.exe 实用工具位于 %Windir%\\$NTUninstallKB926436$\\Spuninst 文件夹中。
...
-   **注册表项验证**
    ...
    **注意** 此注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 926436 安全更新集成或组合到 Windows 安装源文件中，则可能无法正确创建此注册表项。
...
-   **注册表项验证**
    ...
    **注意** 这些注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 926436 安全更新集成或组合到 Windows 安装源文件中，则可能无法正确创建这些注册表项。
...
-   [Immunity, Inc.](https://www.immunityinc.com/) 的 Kostya Kortchinsky 和 [EADS Common Research Center](https://www.eads.net/) 的 Fabrice Desclaux 报告了 OLE 内存损坏漏洞 (CVE-2007-0026)。
