---
TOCTitle: 'MS07-007'
Title: 'Microsoft 安全公告 MS07-007 - 重要'
ms:assetid: 'ms07-007'
ms:contentKeyID: 61236857
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms07-007(v=Security.10)'
---



Microsoft 安全公告 MS07-007 - 重要
==================================

Windows 图像捕获服务中的漏洞可能允许特权提升 (927802)
-----------------------------------------------------
...
-   Microsoft Windows XP Service Pack 2 - [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=ce695e0e-938c-4fc6-a9a2-0eb9fc3e5512)
...
| 漏洞标识符                                                                                              | 漏洞的影响 | Windows XP Service Pack 2 |
|---------------------------------------------------------------------------------------------------------|------------|---------------------------|
| Windows 图像捕获漏洞 - [CVE-2007-0210](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-0210) | 特权提升   | 重要                      |
...
#### Windows 图像捕获漏洞 - CVE-2007-0210：

在 Windows XP Service Pack 2 内，Window 图像捕获启动应用程序的方式中存在一个[特权提升](https://go.microsoft.com/fwlink/?linkid=21142)漏洞。 已登录的用户可通过利用此漏洞完全控制系统。

#### Windows 图像捕获漏洞 (CVE-2007-0210) 的缓解因素：

-   攻击者必须拥有有效的登录凭据并能本地登录才能利用此漏洞。 匿名用户无法利用此漏洞，也无法以远程方式利用此漏洞。

#### Windows 图像捕获漏洞 (CVE-2007-0210) 的变通办法：

Microsoft 已测试以下变通办法。 尽管这些变通办法不能从根本上消除此漏洞，但它们有助于阻止已知的攻击媒介。 如果某种变通办法导致功能下降，下一节中将进行说明。

-   **禁用 Windows 图像捕获服务**

    禁用 Windows 图像捕获服务将帮助防止受影响的系统遭受尝试利用此漏洞进行的攻击。 要禁用 Windows 图像捕获服务，请执行以下步骤：

    1.  单击“**开始**”，然后单击“**控制面板** ”。或者，指向“**设置**”，然后单击“**控制面板**”。
    2.  双击“**管理工具**”。
    3.  双击“**服务**”。
    4.  双击 **Windows 图像捕获 (WIA**)。
    5.  在“**启动类型**”列表中，单击“**禁用**”。
    6.  单击“**停止**”，然后单击“**确定**”。

    您还可以通过在命令提示符处使用以下命令来停止和禁用 Windows 图像捕获 (WIA) 服务：

    `sc stop stisvc & sc config stisvc start= disabled`

**变通办法的影响：** 如果禁用 Windows 图像捕获 (WIA) 服务，则可能无法与包括数码相机和扫描仪在内的各种成像设备进行连接或通信。 因此，我们仅在不要求与数字成像设备通信的系统上推荐此变通办法。

#### Windows 图像捕获漏洞 (CVE-2007-0210) 的常见问题解答：

**此漏洞的影响范围有多大？**  
这是一个[权限提升](https://go.microsoft.com/fwlink/?linkid=21142)漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。 要尝试利用此漏洞，攻击者必须能够本地登录到系统并运行程序。

**造成漏洞的原因是什么？**  
Windows 图像获取服务中有一个未经检查的缓冲区。

**什么是 Windows 图像捕获服务 (WIA) ？**  
Windows 图像捕获 (WIA) 使 Microsoft Picture It! 2000、Kodak Imaging 或 Adobe Photoshop 等成像程序能够与数码相机和扫描仪等成像设备通信。 WIA 支持数码相机以及低端和高端扫描仪，您还可以使用它从基于 [IEEE 1394](https://www.1394ta.org/technology/) 的数字视频 (DV) 摄像机和通用串行总线 (USB) 网络摄像机中获得静态图像。
...
**受此漏洞威胁最大的系统有哪些？**  
Windows XP Service Pack 2 系统受此漏洞威胁。

**是否可以通过 Internet 利用此漏洞？**  
否。 攻击者必须能够本地登录到作为攻击目标的特定系统。

**此更新有什么作用？**  
此更新通过修改 Windows 图像获取 (WIA) 在将消息传递到所分配缓冲区之前验证消息长度的方式来消除此漏洞。
...
**注意** 使用 **/quiet** 开关将抑制所有消息。 这包括抑制失败消息。 管理员应该使用其中一种支持的方法，验证使用 **/quiet** 开关时安装是否成功。 管理员还应该检查 KB927802.log 文件，查看使用此开关时的任何失败消息。
...
系统管理员也可以使用 Spuninst.exe 实用工具删除此安全更新。 Spuninst.exe 实用工具位于 %Windir%\\$NTUninstallKB927802$\\Spuninst 文件夹中。
