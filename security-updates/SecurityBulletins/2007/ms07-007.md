...
|---------------------------------------------------------------------------------------------------------|------------|---------------------------|
| Windows 图像捕获漏洞 - [CVE-2007-0210](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-0210) | 特权提升   | 重要                      |
...
在 Windows XP Service Pack 2 内，Window 图像捕获启动应用程序的方式中存在一个[特权提升](http://go.microsoft.com/fwlink/?linkid=21142)漏洞。 已登录的用户可通过利用此漏洞完全控制系统。
...
**此漏洞的影响范围有多大？**  
这是一个[权限提升](http://go.microsoft.com/fwlink/?linkid=21142)漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。 要尝试利用此漏洞，攻击者必须能够本地登录到系统并运行程序。

**造成漏洞的原因是什么？**  
Windows 图像获取服务中有一个未经检查的缓冲区。

**什么是 Windows 图像捕获服务 (WIA) ？**  
Windows 图像捕获 (WIA) 使 Microsoft Picture It! 2000、Kodak Imaging 或 Adobe Photoshop 等成像程序能够与数码相机和扫描仪等成像设备通信。 WIA 支持数码相机以及低端和高端扫描仪，您还可以使用它从基于 [IEEE 1394](http://www.1394ta.org/technology/) 的数字视频 (DV) 摄像机和通用串行总线 (USB) 网络摄像机中获得静态图像。
