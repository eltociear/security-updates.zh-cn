...
<td style="border:1px solid black;">
[MS12-005](https://go.microsoft.com/fwlink/?linkid=230777) 中的 2584146
...
如果用户需要获得较旧软件的定制支持，则必须与其 Microsoft 客服小组代表、其技术客户经理或适当的 Microsoft 合作伙伴代表联系以了解定制支持选项。没有联合合同、优先支持合同或授权合同的客户可与其当地的 Microsoft 销售分支机构联系。有关联系信息，请参阅 [Microsoft Worldwide Information](https://go.microsoft.com/fwlink/?linkid=33329) 网站，然后在列表中选择国家/地区以查看电话号码列表。在拨打电话时，请找当地的“企业技术咨询支持服务”销售经理进行洽谈。有关详细信息，请参阅 [Microsoft 技术支持生命周期策略常见问题](https://go.microsoft.com/fwlink/?linkid=169557)。
...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2014-4114](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2014-4114)。 
...
-   **阻止 TCP 端口 139 和 445**
    ...
    这些 TCP 端口可用于启动与受影响组件的连接。将其阻止在企业防火墙处（入站和出站），有助于保护位于防火墙后面的系统免受利用此漏洞进行的攻击。我们建议阻止所有来自 Internet 的未经请求的入站通信，以帮助防止可能使用其他端口进行的攻击。有关端口的详细信息，请访问以下网站 [TCP 和 UDP 端口分配](https://go.microsoft.com/fwlink/?linkid=21312)。有关 Windows 防火墙的详细信息，请参阅以下网站[如何在单台计算机上配置 Windows 防火墙](https://www.microsoft.com/technet/security/smallbusiness/prodtech/windowsxp/cfgfwall.mspx)。

    **变通办法的影响。**

    TCP 端口 139 和 445 用于其他服务，包括[通用 Internet 文件系统](https://technet.microsoft.com/library/cc939973.aspx) (CIFS)、DNS 管理、NetBt 服务会话、打印机共享会话等。禁用这些端口可能会影响这些服务的功能。

-   **阻止通过安装信息文件启动可执行文件**

    [安装信息文件](https://go.microsoft.com/fwlink/?linkid=157952)（.inf 文件扩展名）是指支持使用“install”谓词启动可执行文件的脚本。系统注册表存储的指令可帮助运行 .INF 文件中指定的 install 谓词；通过删除存储的指令，它将阻止使用 .INF 文件传递恶意代码的潜在攻击方法。
...
**什么是 OLE？**  
OLE（对象链接与嵌入）是一种允许应用程序共享数据和功能的技术，例如创建和编辑复合数据的能力。复合数据是指包含多种格式的信息的数据。例如，复合 Microsoft Word 文档可能包含嵌入的 Microsoft Excel 电子表格（或 OLE 对象）。此技术也支持就地编辑；在激活 OLE 对象时，用户会查看现有应用程序内的一组新菜单项，而非启动新应用程序。有关 OLE 的详细信息，请参阅[复合文档](https://msdn.microsoft.com/library/ms693383)。
...
是。我们获悉涉及 Microsoft PowerPoint 2007 文件的有限目标攻击。Microsoft PowerPoint 演示文稿（.PPTX 或 .PPT 文件扩展名）和 PowerPoint 放映（.PPSX 或 .PPS 文件扩展名）数据文件都可能会在攻击情形中使用；如果在攻击情形中使用 PowerPoint 放映文件，则可能会在没有[用户帐户控制](https://technet.microsoft.com/library/cc709691(v=ws.10).aspx) (UAC) 安全提示的情况下安装任意代码。
