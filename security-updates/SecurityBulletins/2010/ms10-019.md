...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-0486](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0486)。
...
Windows Authenticode 签名验证功能或 WinVerifyTrust 对指定对象执行信任验证操作。 此功能将查询转到支持操作标识符（如果存在）的信任提供程序。 WinVerifyTrust 功能执行以下两种操作： 指定对象上的签名检查和信任验证操作。 有关详细信息，请参阅 MSDN 文章 [WinVerifyTrust 功能](https://msdn.microsoft.com/en-us/library/aa388208(vs.85).aspx)。

**什么是 Windows Authenticode 可移植可执行签名格式？**  
Authenticode 是一种用于确定软件二进制文件来源和完整性的数字签名格式。 Authenticode 使用公钥加密标准 (PKCS) \#7 已签名数据和 X.509 证书将 Authenticode 签名的二进制文件与软件发行商身份绑定。 "Authenticode" 签名指一种使用 Authenticode 签名验证功能生成和验证的数字签名格式。 有关此格式的详细信息，请参阅 Windows 文章，[Windows Authenticode 可移植可执行签名格式](https://www.microsoft.com/whdc/winlogo/drvsign/authenticode_pe.mspx)。

**什么是 cabinet 文件？**  
cabinet 是一个在文件库中存储压缩文件的单独文件，其后缀通常为 .CAB。 一个压缩文件可在多个 cabinet 文件中传播。 安装期间，安装应用程序解压缩存储在 cabinet 文件中的文件并将他们复制到用户系统中。 有关详细信息，请参阅 MSDN 文章[cabinet 文件](https://msdn.microsoft.com/en-us/library/aa376906(v=vs.85).aspx)。
...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-0487](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0487)。
...
<td style="border:1px solid black;">对于 Authenticode Signature Verification 5.1：<br />
请参阅 <a href="https://support.microsoft.com/kb/978601">Microsoft 知识库文章 978601</a><br />  
<br />  
对于 Cabinet File Viewer Shell Extension 5.1：<br />
请参阅 <a href="https://support.microsoft.com/kb/979309">Microsoft 知识库文章 979309</a></td>
</tr>
<tr class="even">
