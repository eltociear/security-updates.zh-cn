---
TOCTitle: 'MS10-097'
Title: 'Microsoft 安全公告 MS10-097 - 重要'
ms:assetid: 'ms10-097'
ms:contentKeyID: 61237936
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms10-097(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS10-097 - 重要
==================================

Internet 连接注册向导中的库加载不安全可能允许远程执行代码 (2443105)
-------------------------------------------------------------------
...
此安全更新可解决 Microsoft Windows 的 Internet 连接注册向导中一个公开披露的漏洞。 对于 Windows XP 和 Windows Server 2003 的所有受支持版本，此安全更新等级为“重要”。Windows Vista、Windows Server 2008、Windows 7 和 Windows Server 2008 R2 的所有受支持版本不受此漏洞影响。 有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

如果用户打开与特制库文件位于同一网络文件夹下的 .ins 或 .isp 文件，此漏洞可能允许远程执行代码。 要成功进行攻击，用户必须访问不受信任的远程文件系统位置或 WebDAV 共享，并从该位置打开文档，然后由容易受攻击的应用程序加载此文档。

此安全更新通过更正 Internet 连接注册向导加载外部库的方式来解决此漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows XP Service Pack 3](https://www.microsoft.com/download/details.aspx?familyid=fa9a1aac-b9c5-4d4e-9083-a080ad4ccc6f)                          | 远程执行代码 | 重要         | 无               |
| [Windows XP Professional x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=6dabc306-c858-46b1-815c-cd8d011ff62e) | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=8fa2cfa4-a01d-4910-b69f-736aeb585bab)                 | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=17b0b340-73b2-42a7-9d86-1297c63dcc2b)     | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=04a178cc-1afd-4e47-8cab-05e402e5a568) | 远程执行代码 | 重要         | 无               |
...
<tr class="header">
<th>受影响的软件</th>
<th>Internet 连接注册向导库加载不安全漏洞 - CVE-2010-3144</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
Internet 连接注册向导库加载不安全漏洞 - CVE-2010-3144  
-----------------------------------------------------
  
<span></span>  
Microsoft Windows 组件 Internet 连接注册向导处理 DLL 文件加载的方式中存在一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。 那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-3144](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3144)。
  
#### Internet 连接注册向导库加载不安全漏洞 (CVE-2010-3144) 的缓解因素
...
#### Internet 连接注册向导库加载不安全漏洞 (CVE-2010-3144) 的变通办法
...
#### Internet 连接注册向导库加载不安全漏洞 (CVE-2010-3144) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成此漏洞的原因是什么？**    
此漏洞是由于 Internet 连接注册向导不正确限制加载外部库使用的路径所导致的。
  
**什么是 Internet 连接注册向导？**  
Internet 连接注册向导使用户可在新计算机上配置连接，或使用其在以前计算机上拥有的现有 Internet 服务帐户。 此向导还可帮助已损坏 Internet 设置的用户。 自动配置功能可还原设置 — 用户只需提供用户 ID、密码和存在点 (POP) 服务器的名称。
...
**攻击者如何利用此漏洞？**    
攻击者可能诱使用户打开与特制动态链接库 (DLL) 文件位于同一网络目录下的合法 .ins 或 .isp 文件。 然后，在打开文件时，Internet 连接注册向导可能尝试加载 DLL 文件并执行其包含的任何代码。
  
在电子邮件攻击情形中，攻击者可能通过向用户发送合法的 .ins 或 .isp 附件并诱使用户将附件放在包含特制 DLL 文件的目录下并打开 .ins 或 .isp 文件来利用此漏洞。 然后，在打开 .ins 或 .isp 文件时，Internet 连接注册向导可能尝试加载 DLL 文件并执行其包含的任何代码。
  
在网络情形中，攻击者可能将合法的 .ins 或 .isp 文件与特制 DLL 一起放在 UNC 或 WebDAV 位置上，并诱使用户打开该合法的文件。 攻击者还可能托管一个网站，并引用此网站上的合法文件，从而导致通过 Web 浏览器触发漏洞。
  
**受此漏洞威胁最大的系统有哪些？**    
此漏洞主要影响可能期望用户从不受信任的网络位置打开附件的工作站。 如果管理员允许用户登录服务器并运行程序，服务器同样可能面临风险。 不过，最佳做法是不要授予这样的权限。
  
**此更新有什么作用？**    
此更新通过更正 Internet 连接注册向导加载外部库的方式来解决此漏洞。
  
**发布此安全公告时，此漏洞是否已公开披露？**    
是。 此漏洞已公开披露。 已为此漏洞分配了常见漏洞和披露号码 [CVE-2010-3144](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3144)。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2443105$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2443105">Microsoft 知识库文章 2443105</a></td>
</tr>  
<tr class="even">
...
-   [NGS Software](https://www.ngssoftware.com/) 的 Muhaimin Dzulfakar 报告了 Internet 连接注册向导库加载不安全漏洞 (CVE-2010-3144)
