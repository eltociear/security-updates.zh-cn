---
TOCTitle: 'MS10-100'
Title: 'Microsoft 安全公告 MS10-100 - 重要'
ms:assetid: 'ms10-100'
ms:contentKeyID: 61237939
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms10-100(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS10-100 - 重要
==================================

Consent 用户界面中的漏洞可能允许特权提升 (2442962)
--------------------------------------------------
...
此安全更新通过更正 Consent UI 处理从注册表读取的值的方式来解决此漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows Vista Service Pack 1 和 Windows Vista Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=85265a23-5094-4007-8d33-f402cabd1664)                                                  | 特权提升     | 重要         | 无               |
| [Windows Vista x64 Edition Service Pack 1 和 Windows Vista x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=63c7257a-16bf-4108-80b9-9dfe53528348)                          | 特权提升     | 重要         | 无               |
| [Windows Server 2008（用于 32 位系统）和 Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=14e079a8-01a4-47c9-bd47-f5c9a6ca070a)\*\*               | 特权提升     | 重要         | 无               |
| [Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=6baf92b7-a336-45f2-a1ba-c00c34dfb76f)\*\*     | 特权提升     | 重要         | 无               |
| [Windows Server 2008（用于基于 Itanium 的系统）和 Windows Server 2008（用于基于 Itanium 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=7a4b23d4-f68e-4d5b-8814-d9247145f164) | 特权提升     | 重要         | 无               |
| [Windows 7（用于 32 位系统）](https://www.microsoft.com/download/details.aspx?familyid=f7c7d57a-d031-46a3-9613-eae2b9cb6401)                                                                                   | 特权提升     | 重要         | 无               |
| [Windows 7（用于基于 x64 的系统）](https://www.microsoft.com/download/details.aspx?familyid=e52c36f5-637b-4928-83d0-27514c6cc384)                                                                              | 特权提升     | 重要         | 无               |
| [Windows Server 2008 R2（用于基于 x64 的系统）](https://www.microsoft.com/download/details.aspx?familyid=b823a7aa-0eb9-42dd-bf56-8907d94b314a)\*\*                                                             | 特权提升     | 重要         | 无               |
| [Windows Server 2008 R2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=66b2506d-80e0-4e32-86e6-0908ef56ae90)                                                             | 特权提升     | 重要         | 无               |
...
<tr class="header">
<th>受影响的软件</th>
<th>Consent UI 模拟漏洞 - CVE-2010-3961</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
Consent UI 模拟漏洞 - CVE-2010-3961  
-----------------------------------
  
<span></span>  
Consent 用户界面 (UI) 未正确处理从注册表读取的特殊值的方式中存在特权提升漏洞。 该漏洞能够允许攻击者使用提升的特权运行代码。 成功利用此漏洞的攻击者可执行任意代码，并可完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-3961](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3961)。
  
#### Consent UI 模拟漏洞 (CVE-2010-3961) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   攻击者必须拥有有效的登录凭据并能本地登录才能利用此漏洞。 匿名用户无法利用此漏洞，也无法以远程方式利用此漏洞。  
-   要成功利用此漏洞，攻击者必须具有 SeImpersonatePrivilege。 以下组件也具有此用户权限：  
    - 服务控制管理器启动的服务  
    - 由 COM 基础结构启动的并且配置为在特定帐户下运行的组件对象模型 (COM) 服务器  
    - 配置为以常规用户身份运行的 IIS 服务
  
#### Consent UI 模拟漏洞 (CVE-2010-3961) 的变通办法
  
Microsoft 并未发现此漏洞的任何变通办法。
  
#### Consent UI 模拟漏洞 (CVE-2010-3961) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个特权提升漏洞。 成功利用此漏洞的攻击者可以使用 LocalSystem 特权执行任意代码。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成此漏洞的原因是什么？**    
此漏洞是由于 Consent UI 未正确处理已设置为特定值的注册表项所导致的。
  
**什么是 Consent 用户界面 (UI)？**    
Consent UI 是用户帐户控制 (UAC) 的部件。 完全启用 UAC 后，交互式管理员通常以最小用户权限运行，但他们可以通过 Consent UI 给予明确同意来自我提升以执行管理任务。 此类管理任务包括安装软件和驱动程序、更改系统范围设置、查看或更改其他用户帐户，以及运行管理工具。 有关详细信息，请参阅 MSDN 文章[用户帐户控制](https://msdn.microsoft.com/en-us/library/aa511445.aspx)。
  
**什么是 SeImpersonatePrivilege？**    
SeImpersonatePrivilege 或“身份验证之后模拟客户端”用户权限是 Microsoft Windows 2000 Service Pack 4 中首次引用的安全设置。默认情况下，设备的本地管理员组的成员和设备的 LocalService 帐户被分配此用户权限。 [Microsoft 知识库文章 821546](https://support.microsoft.com/kb/821546) 中介绍了 SeImpersonatePrivilege 设置。
...
**攻击者可能利用此漏洞执行什么操作？**    
只有已授予 SeImpersonatePrivilege 的用户帐户才可进行此攻击。 成功利用此漏洞的攻击者可能使用 LocalSystem 权限运行特制的代码。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
...
**此更新有什么作用？**    
此更新通过更正 Consent UI 处理从注册表读取的值的方式来解决此漏洞。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2442962">Microsoft 知识库文章 2442962</a></td>
</tr>  
<tr class="odd">
...
-   [Argeniss](https://www.argeniss.com/) 的 Cesar Cerrudo 报告了 Consent UI 模拟漏洞 (CVE-2010-3961)
