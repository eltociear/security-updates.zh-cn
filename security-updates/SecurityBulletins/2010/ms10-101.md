---
TOCTitle: 'MS10-101'
Title: 'Microsoft 安全公告 MS10-101 - 重要'
ms:assetid: 'ms10-101'
ms:contentKeyID: 61237940
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms10-101(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS10-101 - 重要
==================================

Windows Netlogon 服务中的漏洞可能允许拒绝服务 (2207559)
-------------------------------------------------------
...
对于 Microsoft Windows 2003、Windows Server 2008（基于 Itanium 的版本除外）和 Windows Server 2008 R2（基于 Itanium 的版本除外）的所有受支持版本，此安全更新的等级为“重要”。 有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

此安全更新通过更正验证 Netlogon RPC 服务接口中的用户提供的数据来解决此漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=ad843b97-2f6e-4406-a17a-627b7db8a926)                                                                      | 拒绝服务     | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=0b0a06e7-0ae5-41f4-9ff5-d524fc0afbfa)                                                          | 拒绝服务     | 重要         | 无               |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=7c0c2850-e81d-4347-aeb3-47036caa7c1b)                                                      | 拒绝服务     | 重要         | 无               |
| [Windows Server 2008（用于 32 位系统）和 Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=6793f75b-cdf4-42ef-a53e-a1acb5b662d1)\*           | 拒绝服务     | 重要         | 无               |
| [Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=85add876-ca5a-4a92-984e-188a72e349fc)\* | 拒绝服务     | 重要         | 无               |
| [Windows Server 2008 R2（用于基于 x64 的系统）](https://www.microsoft.com/download/details.aspx?familyid=d7307afd-84a0-434e-9658-bf9f8ae4b938)\*                                                         | 拒绝服务     | 重要         | 无               |
...
<tr class="header">
<th>受影响的软件</th>
<th>Netlogon RPC 空解除引用 DOS 漏洞 - CVE-2010-2742</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
Netlogon RPC 空解除引用 DOS 漏洞 - CVE-2010-2742  
------------------------------------------------
  
<span></span>  
受影响版本的 Windows Server 上的 Netlogon RPC 服务实施中存在一个远程验证的拒绝服务漏洞。 成功利用此漏洞的攻击者可能导致重新启动受影响版本的 Windows Server。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-2742](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2742)。
  
#### Netlogon RPC 空解除引用 DOS 漏洞 - CVE-2010-2742
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   此漏洞仅影响配置为域控制器的 Windows 服务器。  
-   攻击者需要加入到与受影响域控制器相同的域中的计算机的管理员权限才能利用此漏洞。
  
#### Netlogon RPC 空解除引用 DOS 漏洞 (CVE-2010-2742) 的变通办法
  
Microsoft 并未发现此漏洞的任何变通办法。
  
#### Netlogon RPC 空解除引用 DOS 漏洞 (CVE-2010-2742) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个拒绝服务漏洞。 利用此漏洞的攻击者可能导致重新启动受影响的系统。 请注意，虽然攻击者无法利用拒绝服务漏洞来执行代码或提升他们的用户权限，但此漏洞可能导致受影响的系统重新启动。
  
**造成此漏洞的原因是什么？**    
Netlogon RPC 服务未正确验证用户提供的数据时存在此漏洞。
  
**什么是 Netlogon RPC？**    
Netlogon 远程协议是用于基于域的网络上的用户和计算机验证的远程过程调用 (RPC) 接口。 有关 Netlogon RPC 的详细信息，请参阅 MSDN 文章 [Netlogon 远程协议规范](https://msdn.microsoft.com/en-us/library/cc237009(v=prot.10).aspx)。
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用该漏洞的攻击者可能致使域控制器重新启动。
  
**攻击者如何利用此漏洞？**    
攻击者可能通过向 Netlogon RPC 服务接口发送特制 RPC 数据包来利用此漏洞。 在所有受影响的平台上，攻击者必须具有管理员权限的有效验证凭据才能利用此漏洞。
  
**受此漏洞威胁最大的系统有哪些？**    
配置为域控制器和托管 Netlogon 服务的 Windows Servers 受此漏洞的影响。 Windows 客户端操作系统不会受到此漏洞的影响。
  
**我如何知道 Windows Server 是否配置为域控制器？**    
如果注册表项 HKLM\\SYSTEM\\CurrentControlSet\\Control\\ProductOptions\\ProductType 设置为 LanmanNT，则您的系统已配置为域控制器。
  
**此更新有什么作用？**    
此更新通过正确验证 Netlogon RPC 服务接口内用户提供的数据来解决此漏洞。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2207559$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2207559">Microsoft 知识库文章 2207559</a></td>
</tr>  
<tr class="even">
...
-   The Samba Team 的 Matthias Dieter Wallnöfer 和 Andrew Bartlett 报告了 Netlogon RPC 空解除引用 DOS 漏洞 (CVE-2010-2742)
