...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-2554](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2554)。
...
**什么是访问控制列表 (ACL)？**    
访问控制列表 (ACL) 是应用于某个对象的安全保护列表。 对象可以是文件、进程、事件或具有安全描述符的任何实体。 ACL 中的条目称为访问控制条目 (ACE)。 访问控制列表有两种类型： 任意型和系统型。 ACL 中的每个 ACE 识别针对受信任者允许、拒绝或审核的访问权限。 有关其他信息，请参阅 MSDN 文章[访问控制列表](https://msdn.microsoft.com/en-us/library/aa374872(vs.85).aspx)。
  
**什么是命名管道？**    
命名管道是一个命名的、单向或双向管道，用于管道服务器和一个或多个管道客户端之间的通信。 命名管道的所有实例共享同一个管道名称，但每个实例都有其各自的缓冲区和句柄，并为客户端/服务器通信提供独立管道。 使用实例可让多个管道客户端同时使用相同的命名管道。 任何进程都可访问命名管道，接受安全检查，使命名管道成为相关或不相关的进程之间方便的通信方式。 有关详细信息，请参阅 MSDN 文章[“命名管道”](https://msdn.microsoft.com/en-us/library/aa365590(vs.85).aspx)。
...
**发布此安全公告时，此漏洞是否已公开披露？**    
是。 此漏洞已公开披露。 已为此漏洞分配了常见漏洞和披露号码 [CVE-2010-2554](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2554)。
...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-2555](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2555)。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/982799">Microsoft 知识库文章 982799</a></td>
</tr>  
<tr class="odd">
...
-   [Argeniss](https://www.argeniss.com/) 的 Cesar Cerrudo 报告了 Tracing 注册表项 ACL 漏洞 (CVE-2010-2554)。  
-   [Argeniss](https://www.argeniss.com/) 的 Cesar Cerrudo 报告了 Tracing 内存损坏漏洞 (CVE-2010-2555)
