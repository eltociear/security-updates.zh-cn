---
TOCTitle: 'MS12-033'
Title: 'Microsoft 安全公告 MS12-033 - 重要'
ms:assetid: 'ms12-033'
ms:contentKeyID: 61238375
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms12-033(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS12-033 - 重要
==================================
...
该安全更新通过更正 Windows 分区管理器分配内存中对象的方式来解决漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
<th>
即插即用 (PnP) 配置管理器漏洞 - CVE-2012-0178
</th>
<th>
综合严重等级
</th>
...
Windows 分区管理器处理设备关系请求的方式中存在一个特权提升漏洞。成功利用此漏洞的攻击者可以运行内核模式中的任意代码。攻击者随后可安装程序；查看、更改或删除数据；或者创建拥有完全管理权限的新帐户。

若要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2012-0178](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0178)。

#### 即插即用 (PnP) 配置管理器漏洞 （CVE-2012-0178） 的缓解因素

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。以下缓解因素在您遇到的情形中可能会有所帮助：

-   攻击者必须拥有有效的登录凭据并能本地登录才能利用此漏洞。

#### 即插即用 (PnP) 配置管理器漏洞 （CVE-2012-0178） 的变通办法

Microsoft 并未发现此漏洞的任何变通办法。

#### 即插即用 (PnP) 配置管理器漏洞 （CVE-2012-0178） 的常见问题

**此漏洞的影响范围有多大？**  
这是一个特权提升漏洞。

**造成此漏洞的原因是什么？**  
当一个或多个进程或线程同时调用即插即用 (PnP) 配置管理器功能时，会导致该漏洞。

**哪个 组件受此漏洞的影响？**  
受此漏洞影响的组件为 Windows 分区管理器 (partmgr.sys)。

**什么是 Windows 分区管理器 (partmgr.sys)？ 
**Windows 分区管理器负责发现、创建、删除和管理分区。分区管理器充当由设备类驱动程序创建的磁盘设备对象的筛选器驱动程序。它负责确保所有联机磁盘具有唯一的签名或 GUID。
...
**此更新有什么作用？**  
该更新通过更正 Windows 分区管理器分配内存中的对象的方式来解决漏洞。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2690533">Microsoft 知识库文章 2690533</a></td>
</tr>  
<tr class="odd">
