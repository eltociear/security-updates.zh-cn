---
TOCTitle: 'MS12-049'
Title: 'Microsoft 安全公告 MS12-049 - 重要'
ms:assetid: 'ms12-049'
ms:contentKeyID: 61238391
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms12-049(v=Security.10)'
---
Microsoft 安全公告 MS12-049 - 重要
==================================

TLS 中的漏洞可能允许信息泄露 (2655992)
--------------------------------------

发布时间: 2012年7月10日 | 更新时间: 2013年7月9日
...
此安全更新解决了 TLS 中一个公开披露的漏洞。如果攻击者截取受影响的系统中经过加密的 Web 通信，则该漏洞可能会允许信息泄露。不使用 CBC 模式的所有密码套件不受影响。

对于 Microsoft Windows 所有受支持的版本，此安全更新的等级为“重要”。有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

该安全更新通过修改 Windows 安全通道 (SChannel) 和加密 API： 下一代 (CNG) 组件处理加密的网络数据包的方式来解决漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
<td style="border:1px solid black;">
[MS10-049](https://go.microsoft.com/fwlink/?linkid=197104) 中的 KB980436 由 KB2655992 替代
</td>
</tr>
...
<td style="border:1px solid black;">
[MS12-006](https://go.microsoft.com/fwlink/?linkid=232510) 中的 KB2585542 由 KB2655992 替代
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<td style="border:1px solid black;">
[MS10-085](https://go.microsoft.com/fwlink/?linkid=201705) 中的 KB2207566 由 KB2655992 替代
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=97030267-6b19-46ae-84ce-0bb1f91ab951)（服务器核心安装）  
(KB2655992)
</td>
<td style="border:1px solid black;">
信息泄露
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=8a5f6e84-5e5b-42c3-a5b7-65defdb0665f)（服务器核心安装）  
(KB2655992)
</td>
<td style="border:1px solid black;">
信息泄露
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008 R2（用于基于 x64 的系统）](https://www.microsoft.com/download/details.aspx?familyid=c9ef728f-abef-4076-bb13-7488af471aa4)（服务器核心安装）  
(KB2655992)
</td>
<td style="border:1px solid black;">
信息泄露
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008 R2（用于基于 x64 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=c9ef728f-abef-4076-bb13-7488af471aa4)（服务器核心安装）  
(KB2655992)
</td>
<td style="border:1px solid black;">
信息泄露
...
**安全更新的** **哈希位于**何处？  
安全更新的 SHA1 和 SHA2 哈希可用于验证下载的安全更新程序包的真实性。有关与此更新相关的哈希信息，请参阅 [Microsoft 知识库文章 2655992](https://support.microsoft.com/kb/2655992)。
...
TLS 加密协议中存在一个信息泄露漏洞。此漏洞影响协议本身，并不特定于 Windows 操作系统。这是一个信息泄露漏洞，允许对经过加密的 TLS 通信进行解密。此漏洞主要影响 HTTPS 通信，浏览器是主要攻击媒介，所有通过 HTTPS 或 HTTP/HTTPS 混合内容提供服务的 Web 通信均受影响。

若要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2012-1870](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1870)。

#### TLS 协议漏洞 (CVE-2012-1870) 的缓解因素

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。以下缓解因素在您遇到的情形中可能会有所帮助：

-   不使用 CBC 模式的所有密码套件不受影响。

#### TLS 协议漏洞 (CVE-2012-1870) 的变通办法
...
#### TLS 协议漏洞 (CVE-2012-1870) 的常见问题

**此漏洞的影响范围有多大？**  
这是一个信息泄露漏洞。

**造成此漏洞的原因是什么？**  
当使用操作的加密块链接 (CBC) 模式时，TLS 协议中的设计缺陷会导致该漏洞。
...
**攻击者如何利用此** **漏洞？**  
攻击者可以在 HTTP 响应中注入恶意代码或者托管包含恶意代码的特制网站，强迫浏览器执行此恶意代码。此代码在相同 TLS 会话中向第三方 HTTPS 网站发送许多请求，其中，如果存在以前经过身份验证的会话，则将自动发送 Cookie。这是要利用此漏洞所必需的条件。攻击者需要拦截此 HTTPS 通信以便利用 SSL 中的此漏洞，从而导致可能解密加密通信部分（例如，身份验证 Cookie）。

**受此漏洞威胁最大的系统有哪些？**  
由于浏览器是主要的攻击媒介，发送和接收 HTTPS 通信的任何系统（例如，工作站或终端服务器）受此漏洞的威胁最大。

**此更新有什么作用？**  
此更新修改了 Windows 安全通道 (SChannel) 和加密 API： 下一代 (CNG) 组件处理加密的网络数据包的方式来解决漏洞。

**发布此安全公告时，此漏洞是否已公开披露？**  
是。此漏洞已公开披露。已为此漏洞分配了常见漏洞和披露号码 [CVE-2012-1870](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1870)。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”项或 %Windir%\\$NTUninstallKB2655992$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2655992">Microsoft 知识库文章 2655992</a></td>
</tr>  
<tr class="even">
...
<td style="border:1px solid black;" colspan="2">
使用控制面板中的“**添加或删除程序**”项或 %Windir%\\$NTUninstallKB2655992$\\Spuninst 文件夹中的 Spuninst.exe 实用程序
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<td style="border:1px solid black;" colspan="2">
请参阅 [Microsoft 知识库文章 2655992](https://support.microsoft.com/kb/2655992)
</td>
</tr>
<tr class="alternateRow">
<td style="border:1px solid black;">
...
-   V1.1（2013 年 7 月 9 日）： 公告经过修订，宣布在 2655992 更新的 Windows Vista 程序包中的一个检测更改，以解决一个 Windows Update 重新提供问题。这只是检测更改。已经成功更新了其系统的客户不需要执行任何操作。
