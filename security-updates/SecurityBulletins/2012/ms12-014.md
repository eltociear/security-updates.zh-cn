---
TOCTitle: 'MS12-014'
Title: 'Microsoft 安全公告 MS12-014 - 重要'
ms:assetid: 'ms12-014'
ms:contentKeyID: 61238356
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms12-014(v=Security.10)'
---

Microsoft 安全公告 MS12-014 - 重要
==================================

Indeo 编解码器中的漏洞可能允许远程执行代码 (2661637)
----------------------------------------------------

发布时间: 2012年2月14日 | 更新时间: 2012年2月22日
...
对于 Windows XP Service Pack 3，此安全更新的等级为“重要”。有关详细信息，请参阅本节中的“**受影响和不受影响的软件**”小节。

此安全更新通过更正 Indeo 编解码器加载外部库的方式来解决此漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
**已知问题。**[Microsoft 知识库文章 2661637](https://support.microsoft.com/kb/2661637) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。本文还介绍了这些问题的建议解决办法。
...
|----------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows XP Service Pack 3](https://www.microsoft.com/download/details.aspx?familyid=32e5c9ad-b610-4afb-b6f0-bb0b5fbdd1f6) | 远程执行代码 | 重要         | 无               |
...
是的，此安全更新解决的 Indeo 编解码器不安全库加载漏洞 (CVE-2010-3138) 与 [Microsoft 安全通报 2269637](https://technet.microsoft.com/security/advisory/2269637) 中介绍的漏洞类别有关，其影响应用程序加载外部库的方式。此安全更新针对这种漏洞的特定实例。
...
<th style="border:1px solid black;">
Indeo 编解码器不安全库加载漏洞 - CVE-2010-3138
</th>
...
Indeo 编解码器处理 DLL 文件加载的方式中存在一个远程执行代码漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-3138](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3138)。

#### Indeo 编解码器不安全库加载漏洞 (CVE-2010-3138) 的缓解因素

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。以下缓解因素在您遇到的情形中可能会有所帮助：

-   外围防火墙通常禁用文件共享协议服务器消息块 (SMB)。这样可限制此漏洞的可能攻击媒介。
-   要成功进行攻击，用户必须访问不受信任的远程文件系统位置或 WebDAV 共享并打开媒体文件（例如 .avi 文件）。

#### Indeo 编解码器不安全库加载漏洞 (CVE-2010-3138) 的变通办法
...
#### Indeo 编解码器不安全库加载漏洞 (CVE-2010-3138) 的常见问题

**此漏洞的影响范围有多大？**  
这是一个远程执行代码漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。

**造成此漏洞的原因是什么？**  
此漏洞是由于 Indeo 编解码器不正确限制加载外部库使用的路径所导致的。

**什么是 Indeo 编解码器？**  
Indeo 编解码器是为在 Windows Media Player 等应用程序中使用而将数字媒体文件解压缩的编解码器。有关编解码器的详细信息，请参阅 [编解码器： 常见问题](https://windows.microsoft.com/en-us/windows7/codecs-frequently-asked-questions)。

**什么是 AVI 媒体文件？**  
音频视频交错 (AVI) 是由 Microsoft 定义的一种使用资源交换文件格式 (RIFF) 的多媒体容器格式。AVI 由 Microsoft 定义。AVI 文件格式是计算机上常见的音频和视频数据格式。
...
**攻击者如何利用此** **漏洞？**  
攻击者可能诱使用户打开与特制动态链接库 (DLL) 文件位于同一网络目录下的合法媒体文件（例如 .avi 文件）。然后，在打开合法文件时，Indeo 编解码器可能尝试加载 DLL 文件并执行其包含的任何代码。

在电子邮件攻击情形中，攻击者可能通过向用户发送合法文件（例如 .avi 文件），诱使用户将此附件放在包含特制 DLL 文件的目录下和打开此合法文件来利用此漏洞。然后，在打开合法文件时，Indeo 编解码器可能尝试加载 DLL 文件并执行其包含的任何代码。

在网络攻击情形中，攻击者可能将合法文件（例如 .avi 文件）和特制 DLL 文件放在网络共享、UNC 或 WebDAV 位置中，然后诱使用户打开该文件。

**受此漏洞威胁最大的系统有哪些？**  
可能期望用户从不受信任的网络位置打开文件的工作站受到的威胁最大。

**此更新有什么作用？**  
此安全更新通过更正 Indeo 编解码器加载外部库的方式来解决此漏洞。
...
**发布此安全公告时，此漏洞是否已公开披露？**  
是。此漏洞已公开披露。已为此漏洞分配了常见漏洞和披露号码 [CVE-2010-3138](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3138)。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”项或 %Windir%\\$NTUninstallKB2661637$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2661637">Microsoft 知识库文章 2661637</a></td>
</tr>  
<tr class="even">
...
-   V1.1（2012 年 2 月 22 日）： 已在“摘要”中的“已知问题”下为 Microsoft 知识库文章 2661637 添加一个链接。
