---
TOCTitle: 'MS12-012'
Title: 'Microsoft 安全公告 MS12-012 - 重要'
ms:assetid: 'ms12-012'
ms:contentKeyID: 61238354
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms12-012(v=Security.10)'
---

Microsoft 安全公告 MS12-012 - 重要
==================================

颜色控制面板中的漏洞可能允许远程执行代码 (2643719)
--------------------------------------------------
...
对于 Windows Server 2008 和 Windows Server 2008 R2 的所有受支持版本，此安全更新等级为“重要”。有关详细信息，请参阅本节中的“**受影响和不受影响的软件**”小节。

此安全更新通过更正颜色控制面板加载外部库的方式来解决此漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=3f6f89b3-3bc8-4325-b8d8-9a5a398b99c6)&#42;&#42;                                                             | 远程执行代码 | 重要         | 无               |
| [Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=35e6bd04-594f-42ef-97f8-abcf578b4f10)&#42;&#42;                                                        | 远程执行代码 | 重要         | 无               |
| [Windows Server 2008（用于基于 Itanium 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=d1d51b26-2d01-42a9-9bb1-9fb82c1fb914)                                                        | 远程执行代码 | 重要         | 无               |
| [Windows Server 2008 R2（用于基于 x64 的系统）和 Windows Server 2008 R2（用于基于 x64 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=8c51e09b-51c3-4860-836e-6bcffde75f04)&#42;&#42;     | 远程执行代码 | 重要         | 无               |
| [Windows Server 2008 R2（用于基于 Itanium 的系统）和 Windows Server 2008 R2（用于基于 Itanium 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=fba58a1b-9d9f-4a10-b1df-08a0ebfa2358) | 远程执行代码 | 重要         | 无               |
...
是的，此更新解决的颜色控制面板不安全库加载漏洞 (CVE-2010-5082) 与 [Microsoft 安全通报 2269637](https://technet.microsoft.com/security/advisory/2269637) 中介绍的漏洞类别有关，其影响应用程序加载外部库的方式。此安全更新针对这种漏洞的特定实例。
...
<th style="border:1px solid black;">颜色控制面板不安全库加载漏洞 - CVE-2010-5082</th>
...
颜色控制面板不安全库加载漏洞 - CVE-2010-5082  
--------------------------------------------
  
<span></span>  
颜色控制面板处理 DLL 文件加载的方式中存在一个远程执行代码漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-5082](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-5082)。
  
#### 颜色控制面板不安全库加载漏洞 (CVE-2010-5082) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   要成功进行攻击，用户必须访问不受信任的远程文件系统位置或 WebDAV 共享并打开合法文件（例如 .icm 或 .icc 文件）。  
-   外围防火墙通常禁用文件共享协议服务器消息块 (SMB)。这样可限制此漏洞的可能攻击媒介。  
-   成功利用此漏洞的攻击者可以获得与登录用户相同的用户权限。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。
  
#### 颜色控制面板不安全库加载漏洞 (CVE-2010-5082) 的变通办法
  
变通办法是指一种设置或配置更改，它不能从根本上纠正漏洞，但有助于在应用更新之前封堵已知的攻击源。Microsoft 已测试了以下变通方法，并在讨论中指明了变通方法是否会降低功能性：
  
-   **在 Windows Server 2008 和 Windows Server 2008 R2 上安装桌面体验**  
  
    安装了桌面体验功能的系统不会受此漏洞影响。
  
    有关其他信息（包括如何安装和删除桌面体验的说明），请参阅 TechNet 文章 [桌面体验概述](https://technet.microsoft.com/en-us/library/cc772567.aspx)。
...
#### 颜色控制面板不安全库加载漏洞 (CVE-2010-5082) 的常见问题
  
**此漏洞的影响范围有多大？**  
这是一个远程执行代码漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成此漏洞的原因是什么？**  
此漏洞是由于颜色控制面板不正确限制加载外部库使用的路径所导致的。
  
**Windows Server 2008** **中的** **桌面体验功能是什么？**  
默认情况下，Windows Server 2008 上有许多桌面功能（如颜色控制面板）不会安装。但是，您可以通过在 Windows Server 2008 上安装桌面体验功能将它们一起安装。有关详细信息，请参阅 TechNet 文章 [桌面体验概述](https://technet.microsoft.com/en-us/library/cc772567.aspx)。
...
**攻击者如何利用此** **漏洞？**  
攻击者可能诱使用户打开与特制动态链接库 (DLL) 文件位于同一网络目录下的合法文件（例如 .icm 或 .icc 文件）。然后，在打开合法文件时，颜色控制面板可能尝试加载 DLL 文件并执行其包含的任何代码。
  
在电子邮件攻击情形中，攻击者可能通过向用户发送合法文件（例如 .icm 或 .icc 文件），诱使用户将此附件放在包含特制 DLL 文件的目录下和打开此合法文件来利用此漏洞。然后，在打开合法文件时，颜色控制面板可能尝试加载 DLL 文件并执行其包含的任何代码。
  
在网络攻击情形中，攻击者可能将合法文件（例如 .icm 或 .icc 文件）和特制 DLL 文件放在网络共享、UNC 或 WebDAV 位置中，然后诱使用户打开该文件。
  
**受此漏洞威胁最大的系统有哪些？**  
可能期望用户从不受信任的网络位置打开文件的服务器受到的威胁最大。
  
**此更新有什么作用？**  
此更新通过更正颜色控制面板加载外部库的方式来解决此漏洞。
...
**发布此安全公告时，此漏洞是否已公开披露？**  
是。此漏洞已公开披露。已为此漏洞分配了常见漏洞和披露号码 [CVE-2010-5082](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-5082)。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2643719">Microsoft 知识库文章 2643719</a></td>
</tr>  
<tr class="odd">
