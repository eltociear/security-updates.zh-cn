---
TOCTitle: 'MS12-048'
Title: 'Microsoft 安全公告 MS12-048 - 重要'
ms:assetid: 'ms12-048'
ms:contentKeyID: 61238390
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms12-048(v=Security.10)'
---
Microsoft 安全公告 MS12-048 - 重要
==================================

Windows Shell 中的漏洞可能允许远程执行代码 (2691442)
----------------------------------------------------
...
该安全更新通过修改 Windows 处理包含特制名称的文件和目录的方式来解决漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
<td style="border:1px solid black;">
[MS10-046](https://go.microsoft.com/fwlink/?linkid=197393) 中的 KB2286198 由 KB2691442 替代
</td>
</tr>
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=6d873c5a-57dc-4792-942e-124ce1a4ec2b)（服务器核心安装）  
(KB2691442)
</td>
<td style="border:1px solid black;">
远程执行代码
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=d49e3e7e-910a-4069-b0b0-0987bb9fdde2)（服务器核心安装）  
(KB2691442)
</td>
<td style="border:1px solid black;">
远程执行代码
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008 R2（用于基于 x64 的系统）](https://www.microsoft.com/download/details.aspx?familyid=d16fa6b9-43aa-4d0f-a230-52664e972ab9)（服务器核心安装）  
(KB2691442)
</td>
<td style="border:1px solid black;">
远程执行代码
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008 R2（用于基于 x64 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=d16fa6b9-43aa-4d0f-a230-52664e972ab9)（服务器核心安装）  
(KB2691442)
</td>
<td style="border:1px solid black;">
远程执行代码
...
**安全更新的** **哈希位于**何处？  
安全更新的 SHA1 和 SHA2 哈希可用于验证下载的安全更新程序包的真实性。有关与此更新相关的哈希信息，请参阅 [Microsoft 知识库文章 2691442](https://support.microsoft.com/kb/2691442)。
...
Windows 处理文件和目录名称的方式中存在一个远程执行代码漏洞。如果用户打开包含特制名称的文件或目录，则此漏洞可能允许远程执行代码。如果用户使用管理用户权限登录，成功利用此漏洞的攻击者便可完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

若要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2012-0175](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0175)。

#### 命令注入漏洞 (CVE-2012-0175) 的缓解因素
...
#### 命令注入漏洞 (CVE-2012-0175) 的变通办法

Microsoft 并未发现此漏洞的任何变通办法。

#### 命令注入漏洞 (CVE-2012-0175) 的常见问题

**此漏洞的影响范围有多大？**  
这是当前用户上下文中的远程执行代码漏洞。

**造成此漏洞的原因是什么？**  
当 Windows 不正确地处理特制文件或目录名称时，会导致该漏洞。
...
**攻击者如何利用此** **漏洞？**  
在电子邮件攻击情形中，攻击者可以通过向用户发送包含特制名称的电子邮件，并诱使用户打开该文件来利用此漏洞。

在基于 Web 的攻击情形中，攻击者必须拥有一个包含使用特制名称的文件的网站。另外，接受或宿主用户提供的内容的网站以及受到破坏的网站可能包含可用于利用此漏洞的特制内容。攻击者无法强迫用户访问宿主该文件的网站。相反，攻击者必须诱使用户访问该网站，方法通常是让用户单击电子邮件或 Instant Messenger 消息中的链接以使用户链接到攻击者的网站。

**受此漏洞威胁最大的系统有哪些？**  
用户可能从不受信任的网络位置打开附件的工作站。如果管理员允许用户登录服务器并运行程序，服务器同样可能面临风险。不过，最佳做法是不要授予这样的权限。

**此更新有什么作用？**  
该更新通过修改 Windows 处理包含特制名称的文件和目录的方式来解决漏洞。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”项或 %Windir%\\$NTUninstallKB2691442$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2691442">Microsoft 知识库文章 2691442</a></td>
</tr>  
<tr class="even">
...
<td style="border:1px solid black;" colspan="2">
使用控制面板中的“**添加或删除程序**”项或 %Windir%\\$NTUninstallKB2691442$\\Spuninst 文件夹中的 Spuninst.exe 实用程序
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<td style="border:1px solid black;" colspan="2">
请参阅 [Microsoft 知识库文章 2691442](https://support.microsoft.com/kb/2691442)
</td>
</tr>
<tr class="alternateRow">
<td style="border:1px solid black;">
...
-   [IBM Security Systems - Application Security](https://blog.watchfire.com/) 的 Adi Cohen 报告了命令注入漏洞 (CVE-2012-0175)
...
-   V1.1（2013 年 7 月 9 日）： 公告经过修订，宣布在 2691442 更新的 Windows Vista 程序包中的一个检测更改，以解决一个 Windows Update 重新提供问题。这只是检测更改。已经成功更新了其系统的客户不需要执行任何操作。
