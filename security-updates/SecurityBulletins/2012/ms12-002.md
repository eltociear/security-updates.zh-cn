---
TOCTitle: 'MS12-002'
Title: 'Microsoft 安全公告 MS12-002 - 重要'
ms:assetid: 'ms12-002'
ms:contentKeyID: 61238240
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms12-002(v=Security.10)'
---


Microsoft 安全公告 MS12-002 - 重要
==================================

Windows 对象包装程序中的漏洞可能允许远程执行代码 (2603381)
----------------------------------------------------------
...
此安全更新通过更正与 Windows 对象包装程序相关的注册表项来解决漏洞。有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows XP Service Pack 3](https://www.microsoft.com/download/details.aspx?familyid=feaeda8c-84ee-47be-8c68-736f0e5b1fc7)                          | 远程执行代码 | 重要         | 无               |
| [Windows XP Professional x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=211827f2-fe6a-4e16-a90c-0cc3b60a722d) | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=b8e46267-7988-4fbe-ae94-68b6fdb2e7d9)                 | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=5975b01e-139b-4b9d-a0d5-a87a279bfea1)     | 远程执行代码 | 重要         | 无               |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=26079c35-4b96-42fc-ad47-138be25a6bf0) | 远程执行代码 | 重要         | 无               |
...
<tr class="header">
<th>受影响的软件</th>
<th>对象包装程序不安全可执行启动漏洞 - CVE-2012-0009</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
对象包装程序不安全可执行启动漏洞 - CVE-2012-0009  
------------------------------------------------
  
<span></span>  
Windows 注册和使用 Windows 对象包装程序的方式中存在一个远程执行代码漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2012-0009](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0009)。
  
#### 对象包装程序不安全可执行启动漏洞 (CVE-2012-0009) 的缓解因素
...
#### 对象包装程序不安全可执行启动漏洞 (CVE-2012-0009) 的变通办法
...
-   **注册具有完整路径的** **packager.exe**
  
    **警告** 如果不正确地使用注册表编辑器，可能导致严重的问题，或许需要您重新安装操作系统。Microsoft 不保证您可以解决因错误运用注册表编辑器而产生的问题。使用注册表编辑器的风险由您自己承担。
  
    建议您在编辑注册表之前先对其进行备份。
  
    要注册具有完整路径的 packager.exe，请执行以下步骤：
  
    1.  单击“**开始**”，单击“**运行**”，键入 **regedit**，然后单击“**确定**”。  
    2.  依次展开 **HKEY\_CLASSES\_ROOT**、**Package**、**Protocol**、**StdFileEditing** 和 **Server**。  
    3.  单击（默认）项并将值更改为： `C:\Windows\System32\Packager.exe`**注意**
  
#### 对象包装程序不安全可执行启动漏洞 (CVE-2012-0009) 的常见问题
  
**此漏洞的影响范围有多大？**   
这是一个远程执行代码漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成此漏洞的原因是什么？**   
此漏洞是由于在 Microsoft Windows 的受影响版本中注册和实施 Windows 对象包装程序不当造成的。
  
**什么是** **Windows** **对象包装程序？**   
Windows 对象包装程序是一个用于创建可插入文件的程序包的工具。有关对象包装程序的详细信息，请参阅以下 [Microsoft 产品文档。](https://www.microsoft.com/resources/documentation/windows/xp/all/proddocs/en-us/packager_what_is_obj_pkg.mspx?mfr=true)
...
**攻击者如何利用此漏洞？**   
在网络攻击情形中，攻击者可能将含有嵌入的包对象的合法文件与特制可执行文件一起放在网络共享上，例如 UNC 或 WebDAV 位置，然后诱使用户打开该合法文件。
  
**受此漏洞威胁最大的系统有哪些？**   
可能期望用户从不受信任的网络位置打开文件的工作站受到的威胁最大。如果管理员允许用户登录服务器并运行程序，服务器同样可能面临风险。不过，最佳做法是不要授予这样的权限。
  
**此更新有什么作用？**   
此更新通过更正与 Windows 对象包装程序相关的注册表项来解决此漏洞。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”项或 %Windir%\\$NTUninstallKB2603381$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
...
-   Parvez Anwar 与[Secunia Research](https://secunia.com/) 合作报告了对象包装程序不安全可执行启动漏洞 (CVE-2012-0009)
