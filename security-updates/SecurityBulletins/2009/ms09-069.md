---
TOCTitle: 'MS09-069'
Title: 'Microsoft 安全公告 MS09-069 - 重要'
ms:assetid: 'ms09-069'
ms:contentKeyID: 61237520
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms09-069(v=Security.10)'
---
...
|---------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|----------------------------------------------------------|
| [Microsoft Windows 2000 Service Pack 4](https://www.microsoft.com/download/details.aspx?familyid=560e01db-5f59-4ef1-9406-f5d7e0fd4128)                  | 拒绝服务     | 重要         | [MS06-025](https://go.microsoft.com/fwlink/?linkid=62072) |
| [Windows XP Service Pack 2 和 Windows XP Service Pack 3](https://www.microsoft.com/download/details.aspx?familyid=5448b168-6bf7-4bae-9627-b88d76c4d5c5) | 拒绝服务     | 重要         | 无                                                       |
| [Windows XP Professional x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=c2bbf515-f81a-436b-947b-cbf2db85fdd9)     | 拒绝服务     | 重要         | 无                                                       |
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=a779aae1-7724-4458-94fb-a2343356ecae)                     | 拒绝服务     | 重要         | 无                                                       |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=a8a9bf12-4ad6-49fd-b2b7-f379dc3309d2)         | 拒绝服务     | 重要         | 无                                                       |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=f5b003ad-af25-488a-91fb-98835a0bfeac)     | 拒绝服务     | 重要         | 无                                                       |
...
<th>
本地安全机构子系统服务资源枯竭漏洞 - CVE-2009-3675
</th>
<th>
综合严重等级
</th>
...
由于通过 IPSec 进行通信时，本地安全授权子系统服务 (LSASS) 处理特制 ISAKMP 信息的方式不正确，Microsoft Windows 中存在一个服务拒绝漏洞。

要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-3675](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-3675)。

#### 本地安全机构子系统服务资源枯竭漏洞 (CVE-2009-3675) 的缓解因素

Microsoft 并未发现此漏洞的任何缓解因素。

#### 本地安全机构子系统服务资源枯竭漏洞 (CVE-2009-3675) 的变通办法

变通办法是指一种设置或配置更改，它不能从根本上纠正漏洞，但有助于在应用更新之前封堵已知的攻击源。 Microsoft 已测试了以下变通方法，并在讨论中指明了变通方法是否会降低功能性：

-   禁用 IPSec 服务

    你可以在不使用 IPSec 服务时将它禁用。
    ...
    **注意** 如果注册表编辑器使用不当，可能会导致严重的问题，可能必须重新安装操作系统。 Microsoft 不保证能够解决因注册表编辑器使用不当而导致的问题。 使用注册表编辑器的风险由您自己承担。 有关如何编辑注册表的信息，请查看注册表编辑器 (Regedit.exe) 中的“更改项和值”帮助主题，或查看 Regedt32.exe 中的“在注册表中添加和删除信息”和“编辑注册表数据”帮助主题。
    ...
    -   **使用交互方法**
        ...
        5.  通过执行以下操作为该注册表项创建备份：
            在“文件”菜单上，单击“**导出**”。
            在“导出注册表文件”对话框中，键入 **PolicyAgent\_configuration\_backup.reg**，然后单击“**保存**”。
    ...
    -   **使用被管理的部署脚本**
        ...
        2.  将下列内容保存到扩展名为 .REG 的文件（例如：Disable\_PolicyAgent.reg）：
        ...
        5.  通过执行以下操作为该注册表项创建备份：
            在“文件”菜单上，单击“**导出**”。
            在“导出注册表文件”对话框中，键入 **PolicyAgent\_configuration\_backup.reg**，然后单击“**保存**”。
        6.  在目标系统上，使用下列命令运行在步骤 2 中创建的上述注册表脚本：
            Regedit /s Disable\_PolicyAgent.reg
        7.  然后将**值数据**字段改为 **4**。
        8.  单击“**确定**”。
        9.  在目标系统上以管理员身份通过命令提示符运行下列命令：
            net stop PolicyAgent

    **变通办法的影响。**特定计算机将不能使用 IPSec。
...
-   **使用交互方法**
    1.  单击“**开始**”，单击“**运行**”，在“**打开**”框中键入 **Regedit**，然后单击“**确定**”。
    2.  在“**文件**”菜单上，单击“导入”。
    3.  在“导入注册表文件”对话框中，选择 **PolicyAgent\_configuration\_backup.reg**，然后单击“**打开**”。
...
#### 本地安全机构子系统服务资源枯竭漏洞 (CVE-2009-3675) 的常见问题

**此漏洞的影响范围有多大？**  
这是一个拒绝服务漏洞。 通过 IPSec 进行通信时，经过身份验证的远程攻击者可以发送特制 ISAKMP 信息到服务器，从而导致 LSASS.exe 消耗系统资源，并最终造成服务拒绝一直持续到服务器重启为止。

**造成漏洞的原因是什么？**  
存在此漏洞是由于 LSASS 没有正确地处理带有格式错误的数据包的特制 ISAKMP 信息。

**什么是 IPSec** 
Internet 协议安全性 (IPSec) 是一个开放标准框架，它通过使用加密安全服务来保护 Internet 协议 (IP) 网络上的通信。 IPSec 支持网络级对等身份验证、数据来源身份验证、数据完整性、数据保密性（加密）和重放保护。 IPSec 的 Microsoft 实施基于 Internet 工程任务组 (IETF) IPSec 工作组开发的标准之上。

**什么是 LSASS？**  
本地安全机构子系统服务 (LSASS) 提供一个用于管理本地安全、域身份验证和 Active Directory 服务进程的接口。 LSASS 处理客户端和服务器的身份验证。 LSASS 还包含支持 Active Directory 实用工具的功能。

**什么是 IKE？**  
IKE 是一项 Internet 标准，由 RFC 2409 定义，描述了创建 IPSec 安全关联 (SAs) 的机制。 SA 是相互协定的策略与密钥的组合，定义了可帮助保护 IPSec 对等机间通信的安全服务和机制。 IKE 特别结合了 RFC 2408 内的 Internet 安全关联和密钥管理协议 (ISAKMP) 与 RFC 2412 内的 Oakley 密钥确定协议
IKE 使用 Oakley 协议为受保护的通信生成密钥材料。 IKE 使用 ISAKMP 来协调 SA。

**什么是 ISAKMP 信息？**  
ISAKMP 是一个协调受保护通信的结构，与特定的密钥交换协议、加密和完整性算法以及身份验证方法无关。 ISAKMP 包括识别和验证对等机、管理 SA、和交换密钥材料的设施。

**攻击者可能利用此漏洞执行什么操作？**  
成功利用此漏洞的攻击者可能使受影响的系统消耗 CPU 资源，变得没有响应，直至重新启动。

**攻击者如何利用此漏洞？**  
攻击者可通过连接到 IPSEC 上的 LSASS 服务器并通过身份验证后发送特制 ISAKMP 信息来利用此漏洞。

**受此漏洞威胁最大的系统有哪些？**  
要求或需要与客户机进行 IPSec 通信的服务器受此漏洞的威胁最大。

**此更新有什么作用？**  
次更新更改 LSASS 处理特制 ISAKMP 请求的方式。
...
| **部署**                         |                                                                                                                       |
| 没有用户干预的安装               | Windows2000-KB974392-x86-ENU /quiet                                                                                   |
| 在不重新启动的情况下安装         | Windows2000-KB974392-x86-ENU /norestart                                                                               |
| 更新日志文件                     | KB974392.log                                                                                                          |
| 更多信息                         | 请参阅“**检测和部署工具及指导**”小节                                                                                  |
| **重新启动要求**                 |                                                                                                                       |
| 要求重新启动？                   | 是，应用此安全更新后必须重新启动系统。                                                                                |
| HotPatching                      | 不适用                                                                                                                |
| **删除信息**                     | 使用控制面板中的“**添加或删除程序**”工具或 %Windir%\\$NTUninstallKB974392$\\Spuninst 文件夹中的 Spuninst.exe 实用程序 |
| **文件信息**                     | 请参阅 [Microsoft 知识库文章 974392](https://support.microsoft.com/kb/974392)                                          |
| **注册表项验证**                 | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB974392\\Filelist                             |
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\$NTUninstallKB974392$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/974392">Microsoft 知识库文章 974392</a></td>
</tr>  
<tr class="even">
