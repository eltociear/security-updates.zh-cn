...
**已知问题。** [Microsoft 知识库文章 967723](https://support.microsoft.com/kb/967723) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。 本文还介绍了这些问题的建议解决办法。
...
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=48d82036-2fde-4bb0-a60e-92eed83ddc3f)                                                                              | 拒绝服务     | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=e0298ddf-026e-4137-8197-ed9d9b889825)                                                                  | 拒绝服务     | 重要         | 无               |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=c948c4d8-5788-4c1a-9fb6-a969b06a888d)                                                              | 拒绝服务     | 重要         | 无               |
| [Windows Vista、Windows Vista Service Pack 1 和 Windows Vista Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=7d72f845-9feb-4685-a669-f9d6ab54f9ed)                                     | 远程执行代码 | 严重         | 无               |
| [Windows Vista x64 Edition、Windows Vista x64 Edition Service Pack 1 和 Windows Vista x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=b2930ff1-5f0a-4a5d-bf2a-9fb76dd8da63) | 远程执行代码 | 严重         | 无               |
| [Windows Server 2008（用于 32 位系统）和 Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=35c1d5a9-a953-4fc6-90c0-d2358c7b89e6)\*\*                 | 远程执行代码 | 严重         | 无               |
| [Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=6e46822e-f79d-492d-ad01-ee680ad324f5)\*\*       | 远程执行代码 | 严重         | 无               |
| [Windows Server 2008（用于基于 Itanium 的系统）和 Windows Server 2008（用于基于 Itanium 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=2ac76ee2-b1b6-4300-9cba-af33d9dd54eb)   | 远程执行代码 | 严重         | 无               |

**\*** 无更新。 有关详细信息，请参阅**与此安全更新相关的常见问题 (FAQ)** 条目。

**\*\*Windows Server 2008 服务器核心安装受影响。** 此更新适用于 Windows Server 2008 的受支持版本，严重等级相同，无论安装 Windows Server 2008 时是否使用“服务器核心”安装选项。 有关该安装选项的详细信息，请参阅[服务器核心](https://msdn.microsoft.com/en-us/library/ms723891(vs.85).aspx)。 注意，“服务器核心”安装选项不适用于某些 Windows Server 2008 版本；请参阅[比较“服务器核心”安装选项](https://www.microsoft.com/windowsserver2008/en/us/compare-core-installation.aspx)。
...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2008-4609](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4609)。
...
**什么是 TCP/IP？**    
TCP/IP 是一套在 Internet 上广泛应用的网络协议。 TCP/IP 在相互连接的计算机网络间提供通信，这些计算机有着不同的硬件结构并运行了不同的操作系统。 TCP/IP 包含了计算机通信的标准及网络连接和路由通信的惯例。 有关 TCP/IP 的详细信息，请参阅 TechNet 文章[网络和 TCP/IP 概述](https://go.microsoft.com/fwlink/?linkid=40954)。
  
**什么是 TCP 接收窗口尺寸？**    
传输控制协议 (TCP) 接收窗口尺寸是在连接的接收方一次可缓冲的最大接收数据量，以字节为单位。 发送主机只能发送该数量的数据，然后必须等待接收主机的确认和窗口更新。 将 TCP 接收窗口大小设置为零将使连接保持活动，但无法传递任何数量的额外数据，直到增加窗口大小。 有关 TCP 接收窗口大小选项的详细信息，请参阅 MSDN 文章 [TCP 接收窗口大小和窗口缩放](https://msdn.microsoft.com/en-us/library/ms819736.aspx)。
...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-1925](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1925)。
...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-1926](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1926)。
...
TCP 连接在客户端与服务器之间的事务期间会经过一系列不同的状态。 FIN-WAIT-1 状态代表系统等待来自远程 TCP 的连接终止请求，或之前发送的连接终止请求的确认。 FIN-WAIT-2 状态代表系统等待来自远程 TCP 的连接终止请求。 有关详细信息，请参阅 [RFC 793](https://www.ietf.org/rfc/rfc793.txt)。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/967723">Microsoft 知识库文章 967723</a></td>
</tr>  
<tr class="even">
...
-   [Outpost24](https://www.outpost24.com/) 的 Jack C. Louis 报告 TCP/IP 窗口大小为零漏洞 (CVE-2008-4609)  
-   [Recurity Labs GmbH](https://www.recurity-labs.com/) 的 Felix Lindner 报告 TCP/IP 孤立连接漏洞 (CVE-2009-1926)
