---
TOCTitle: 'MS09-024'
Title: 'Microsoft 安全公告 MS09-024 - 严重'
ms:assetid: 'ms09-024'
ms:contentKeyID: 61237475
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms09-024(v=Security.10)'
---
...
对于 Microsoft Office Word 2000 的支持版本，此安全更新的等级为“严重”。对于 Microsoft Office Word 2002、带 Microsoft Works 6–9 文件转换器的 Microsoft Office Word 2003 和 Microsoft Office Word 2007 Service Pack 1 的支持版本；以及 Microsoft Works 8.5 和 Microsoft Works 9.0，此安全更新的等级也是“严重”。有关详细信息，请参阅本节中的“**受影响和不受影响的软件**”小节。

此安全更新通过修改 Microsoft Office 打开 Works 文件的方式来解决该漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
**MS09-017 也描述了 Microsoft Works 8.5 和 Microsoft Works 9.0 中的漏洞。MS09-017 与此公告 (MS09-024) 有何关联？**  
本公告 MS09-024 中的 Microsoft Works 8.5 (KB967043) 和 Microsoft Works 9 (KB967044) 安全更新包也解决了 [MS09-017](https://go.microsoft.com/fwlink/?linkid=141704) 中的漏洞。安装了 Microsoft Works 8.5 或 Microsoft Works 9 的用户只需安装该安全更新包一次。

**Microsoft Works 6-9 文件转换器是什么？**  
Microsoft Works 6-9 转换器允许 Microsoft Office Word 2003 以 Works 版本 6、7、8 和 9 支持的 Works Word Processor 格式打开、编辑和保存文件。有关详细信息，请参阅[Microsoft Works 6-9 文件转换器](https://www.microsoft.com/download/details.aspx?familyid=bf41401e-70fa-465d-ae2e-cf44dbf05297&displaylang=en)。
...
Windows 文档转换器的 Works 处理特制 Works 文件的方式中存在一个远程执行代码漏洞。 如果用户打开特制的 .wps 文件，该漏洞可能允许远程执行代码。 那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-1533](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1533)。

#### 文件转换器缓冲区溢出漏洞 (CVE-2009-1533) 的缓解因素

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：

-   2007 Microsoft Office System Service Pack 2 不受此漏洞影响。
-   Microsoft Office Word 2003 只有在安装了 Works 转换器的情况下才受影响。 有关详细信息，请参阅 [Microsoft Works 6-9 文件转换器](https://www.microsoft.com/download/details.aspx?familyid=bf41401e-70fa-465d-ae2e-cf44dbf05297&displaylang=en)。
...
#### 文件转换器缓冲区溢出漏洞 (CVE-2009-1533) 的变通办法

变通办法是指一种设置或配置更改，它不能从根本上纠正漏洞，但有助于在应用更新之前封堵已知的攻击源。 Microsoft 已测试了以下变通方法，并在讨论中指明了变通方法是否会降低功能性：

-   **对于 Word 2000 和 Word 2002，通过限制访问来禁用 Works 4.x 转换器**

    管理员可以向受影响的转换器应用访问控制列表，从而确保 Works 和 Office 不再加载转换器。 这样做可有效地防止借助此攻击媒介利用该漏洞。
    ...
    **对于 Vista/Server 2008**
    ...
    icacls "%CommonProgramFiles%\Microsoft Shared\TextConv\works432.cnv" /deny everyone:(F)takeown /f "%ProgramFiles(x86)%\Common Files\Microsoft Shared\TextConv\works432.cnv"
    icacls "%ProgramFiles(x86)%\Common Files\Microsoft Shared\TextConv\works432.cnv" /save works432_ACL.TXTicacls "%ProgramFiles(x86)%\Common Files\Microsoft Shared\TextConv\works432.cnv" /deny everyone:(F)
    ```

    **变通办法的影响。** 实行变通方法时，用户不能打开或保存 Works 4.x 文档。 尝试打开 Works 4.x 文档将返回一个错误，表明“文件似乎损坏”。

-   **对于带**[Microsoft Works 6–9 文件转换器的 Word 2003](https://www.microsoft.com/download/details.aspx?familyid=bf41401e-70fa-465d-ae2e-cf44dbf05297&displaylang=en)**和 Word 2007，通过限制访问来禁用 Works 6-9 转换器**
    ...
    **对于 Windows Vista 和 Windows Server 2008**
    ...
    icacls "%CommonProgramFiles%\Microsoft Shared\TextConv\works632.cnv" /deny everyone:(F)takeown /f "%ProgramFiles(x86)%\Common Files\Microsoft Shared\TextConv\works632.cnv"
    icacls "%ProgramFiles(x86)%\Common Files\Microsoft Shared\TextConv\works632.cnv" /save works632_ACL.TXTicacls "%ProgramFiles(x86)%\Common Files\Microsoft Shared\TextConv\works632.cnv" /deny everyone:(F)
    ```

    **变通办法的影响。** 实行变通方法时，用户不能打开或保存 Works 6-9 文档。 尝试打开 Works 6-9 文档将返回一个错误，表明“文件似乎损坏”。

#### 文件转换器缓冲区溢出漏洞 (CVE-2009-1533) 的常见问题

**此漏洞的影响范围有多大？**  
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。

**造成漏洞的原因是什么？**  
当用户打开特制 Works 文件时，它可能以一种攻击者可以执行任意代码的方式来破坏系统内存。
...
**攻击者如何利用此漏洞？**  
此漏洞需要用户使用该软件的受影响版本打开特制的 Works 文件。

在电子邮件攻击情形中，攻击者可以通过向用户发送特制的 Works 文件并诱使用户打开该文件来利用此漏洞。

在基于 Web 的攻击情形中，网站可能包含用于利用此漏洞的 Works 文件。 攻击者必须说服用户访问该网站然后打开特制的 Works 文件，方法通常是让用户单击电子邮件或 Instant Messenger 消息中的链接以使用户链接到攻击者的网站。

**受此漏洞威胁最大的系统有哪些？**  
使用受影响产品的系统（包括工作站和终端服务器）受到的威胁最大。 如果管理员允许用户登录服务器并运行程序，服务器可能面临更大风险。 不过，最佳做法是不要授予这样的权限。

**此更新有什么作用？**  
此更新通过修改 Microsoft Office 打开 Works 文件的方式来解决该漏洞。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/957838">Microsoft 知识库文章 957838</a></td>
</tr>  
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/957646">Microsoft 知识库文章 957646</a></td>
</tr>  
<tr class="even">
<td style="border:1px solid black;"><strong>注册表项验证</strong></td>
<td style="border:1px solid black;">不适用</td>
...
#### 带 Microsoft Works 6-9 文件转换器 的 Word 2003（所有版本）
...
<tr class="even">
<td style="border:1px solid black;">没有用户干预的安装</td>
<td style="border:1px solid black;">对于带 Microsoft Works 6-9 文件转换器的 Microsoft Office Word 2003：<br />
...
<tr class="even">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/968326">Microsoft 知识库文章 968326</a></td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>注册表项验证</strong></td>
<td style="border:1px solid black;">不适用</td>
...
<tr class="even">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/969559">Microsoft 知识库文章 969559</a></td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>注册表项验证</strong></td>
<td style="border:1px solid black;">不适用</td>
...
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">对于 Microsoft Works 8.5：<br />
请参阅 <a href="https://support.microsoft.com/kb/967043">Microsoft 知识库文章 967043</a><br />  
<br />  
对于 Microsoft Works 9：<br />
请参阅 <a href="https://support.microsoft.com/kb/967044">Microsoft 知识库文章 967044</a></td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>注册表项验证</strong></td>
<td style="border:1px solid black;">不适用</td>
...
-   [NGS Software](https://www.ngssoftware.com/) 的 Shaun Colley 报告了文件转换器缓冲区溢出漏洞 (CVE-2009-1533)。
