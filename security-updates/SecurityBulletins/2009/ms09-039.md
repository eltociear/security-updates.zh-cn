---
TOCTitle: 'MS09-039'
Title: 'Microsoft 安全公告 MS09-039 - 严重'
ms:assetid: 'ms09-039'
ms:contentKeyID: 61237490
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms09-039(v=Security.10)'
---
...
对于 Microsoft Windows 2000 Server Service Pack 4 和 Windows Server 2003 的所有受支持版本，此安全更新的等级为“严重”。有关详细信息，请参阅本节中的“**受影响和不受影响的软件**”小节。

此安全更新通过更正 WINS 服务计算缓冲区长度的方式并在 WINS 服务器上对收到的数据包采用正确的数据验证来解决漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的“常见问题 (FAQ)”小节。
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|-----------------------------------------------------------|
| [Microsoft Windows 2000 Server Service Pack 4](https://www.microsoft.com/download/details.aspx?familyid=b5b9228a-66c0-49e6-afde-cc2825a6851f)       | 远程执行代码 | 严重         | [MS09-008](https://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=3a8d8ef9-ad41-4237-9cbb-daecfd8f216c)                 | 远程执行代码 | 严重         | [MS09-008](https://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=e132d051-4444-4ef1-9b6f-2d7da9d2e88e)     | 远程执行代码 | 严重         | [MS09-008](https://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=96c3f496-7b2f-4dbc-b484-216c9943c2b1) | 远程执行代码 | 严重         | [MS09-008](https://go.microsoft.com/fwlink/?linkid=139821) |
...
只有当“受影响的软件”表中列出的操作系统上启用了 WINS 时，才可以安装该更新。 默认情况下，这些操作系统上未启用 WINS，因此除非已经启用了 WINS，否则不提供更新。
...
<tr class="header">
<th>受影响的软件</th>
<th>WINS 堆溢出漏洞 - CVE-2009-1923</th>
<th>WINS 整数溢出漏洞 - CVE-2009-1924</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
WINS 堆溢出漏洞 - CVE-2009-1923  
-------------------------------
  
  
由于在处理特制 WINS 网络数据包时错误计算缓冲区长度导致缓冲区溢出，在 Windows Internet 名称服务 (WINS) 中存在一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建新帐户。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-1923](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1923)。
  
#### WINS 堆溢出漏洞 (CVE-2009-1923) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   默认情况下，WINS 不会安装在任何受影响的操作系统版本上。 仅手动安装此组件的客户可能受到此问题的影响。  
-   采用防火墙最佳做法和标准的默认防火墙配置，有助于保护网络免受从企业外部发起的攻击。 按照最佳做法，应使连接到 Internet 的系统所暴露的端口数尽可能少。
  
#### WINS 堆溢出漏洞 (CVE-2009-1923) 的变通办法
...
#### WINS 堆溢出漏洞 (CVE-2009-1923) 的常见问题解答
  
**此漏洞的影响范围有多大？**    
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建新帐户。
  
**造成漏洞的原因是什么？**    
此漏洞是由于在处理特制 WINS 网络数据包时 WINS 计算缓冲区长度的方式造成的。
  
**什么是 WINS？**    
WINS 专门为支持 TCP/IP (NetBT) 上的 NetBIOS 而设计的协议。 用户在其中访问具有 NetBIOS 名称的资源的任何环境都需要 WINS。 如果不在此类网络中使用 WINS，除非您使用 Lmhosts 文件，否则无法使用其 NetBIOS 名称连接到远程网络资源，且可能无法建立文件和打印共享连接。 有关详细信息，请参阅 [WINS](https://technet.microsoft.com/en-us/library/cc784180.aspx) TechNet 文章。
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建新帐户。
  
**攻击者如何利用此漏洞？**    
攻击者可能通过向运行 WINS 服务的受影响系统发送特制 WINS 复制数据包来利用该漏洞。 此攻击无需启用 WINS 系统的复制功能即可成功。
  
**受此漏洞威胁最大的系统有哪些？**    
Windows WINS 服务器受此漏洞的威胁。
  
**此更新有什么作用？**    
该更新通过更正 WINS 服务计算缓冲区长度的方式来解决该漏洞。
...
WINS 整数溢出漏洞 - CVE-2009-1924  
---------------------------------
  
  
由于从受信任的 WINS 复制伙伴收到的特制 WINS 网络数据包中的数据结构验证不足，Windows Internet 名称服务 (WINS) 的默认配置中存在一个远程执行代码漏洞。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2009-1924](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1924)。
  
#### WINS 整数溢出漏洞 (CVE-2009-1924) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   在默认配置中，该攻击只能从受信任的 WINS 复制伙伴成功进行。 但是，如果注册表项“HKLM\\System\\CCS\\Services\\WINS\\Parameteres\\RplOnlyWCnfPnrs”被设为 0，那么网络上的任何人都可以利用该漏洞。 有关此注册表项及其用法的完整说明，请参阅 [KB251067](https://support.microsoft.com/kb/251067)。  
-   默认情况下，WINS 不会安装在任何受影响的操作系统版本上。 仅手动安装此组件的客户可能受到此问题的影响。  
-   采用防火墙最佳做法和标准的默认防火墙配置，有助于保护网络免受从企业外部发起的攻击。 按照最佳做法，应使连接到 Internet 的系统所暴露的端口数尽可能少。  
-   此问题仅影响运行在 Windows 2000 Server Service Pack 4 上的 WINS。
  
#### WINS 整数溢出漏洞 (CVE-2009-1924) 的变通办法
...
**攻击者如何利用此漏洞？**    
受到 WINS 复制信任的攻击者可能通过创建特制的数据包并将其发送到受影响的 WINS 系统来试图利用此漏洞。
  
**受此漏洞威胁最大的系统有哪些？**    
运行在 Windows 2000 Server 上的 Windows WINS 可能受到此漏洞的威胁。
  
**此更新有什么作用？**    
此更新通过在 WINS 服务器上对收到的数据包采用正确的数据验证来解决漏洞。
...
| **部署**                         |                                                                                                                       |  
| 没有用户干预的安装               | Windows2000-kb969883-x86-enu /quiet                                                                                   |  
| 在不重新启动的情况下安装         | Windows2000-kb969883-x86-enu /norestart                                                                               |  
| 更新日志文件                     | kb969883.log                                                                                                          |  
| 更多信息                         | 请参阅“**检测和部署工具及指导**”小节                                                                                  |  
| **重新启动要求**                 |                                                                                                                       |  
| 要求重新启动？                   | 是，应用此安全更新后必须重新启动系统。                                                                                |  
| HotPatching                      | 不适用                                                                                                                |  
| **删除信息**                     | 使用控制面板中的“**添加或删除程序**”工具或 %Windir%\\$NTUninstallKB969883$\\Spuninst 文件夹中的 Spuninst.exe 实用程序 |  
| **文件信息**                     | 请参阅 [Microsoft 知识库文章 969883](https://support.microsoft.com/kb/969883)                                          |  
| **注册表项验证**                 | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB969883\\Filelist                             |
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\$NTUninstallKB969883$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/969883">Microsoft 知识库文章 969883</a></td>
</tr>  
<tr class="even">
...
-   TippingPoint 和 [Zero Day Initiative](https://www.zerodayinitiative.com/) 报告了 WINS 堆溢出漏洞 (CVE-2009-1923)
-   [National University of Defense Technology](https://english.nudt.edu.cn/) 的 LiGen 报告了 WINS 整数溢出漏洞 (CVE-2009-1924)
