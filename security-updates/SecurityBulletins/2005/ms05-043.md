---
TOCTitle: 'MS05-043'
Title: 'Microsoft 安全公告 MS05-043 - 严重'
ms:assetid: 'ms05-043'
ms:contentKeyID: 61236651
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms05-043(v=Security.10)'
---



Microsoft 安全公告 MS05-043 - 严重
==================================

Print Spooler 服务中的漏洞可能允许远程执行代码 (896423)
-------------------------------------------------------

发布时间: 2005年8月9日 | 更新时间: 2007年4月18日
...
**注意事项：** [Microsoft 知识库文章 908506](https://support.microsoft.com/kb/908506) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。 本文还介绍了这些问题的建议解决办法。 有关详细信息，请参阅 [Microsoft 知识库文章 908506](https://support.microsoft.com/kb/908506)。

**测试过的软件和安全更新下载位置：**

**受影响的软件：**

-   Microsoft Windows 2000 Service Pack 4 – [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=3dd3b530-7f43-4c18-8298-6e8797431a5d)
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 – [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=ef402946-1c3b-47e9-9d51-77d890df8725)
-   Microsoft Windows Server 2003 – [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=25469675-df28-4889-8d13-25efcd498388)
-   Microsoft Windows Server 2003（用于基于 Itanium 的系统） – [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=f0aec064-34a3-4ee4-9f15-be1e3dd02bc7)
...
|---------------------------------------------------------------------------------------------------------|--------------|--------------|---------------------------|---------------------------|---------------------|
| 打印后台处理程序漏洞 - [CAN-2005-1984](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-1984) | 远程执行代码 | 严重         | 严重                      | 中等                      | 中等                |
...
[Microsoft 知识库文章 908506](https://support.microsoft.com/kb/908506): 在运行 Windows 2000 Service Pack 4 的计算机上安装安全更新 896423 之后，您可能无法打印到网络打印机。
...
#### 打印后台处理程序漏洞 - CAN-2005-1984：

打印后台处理程序服务中存在远程执行代码漏洞，成功利用此漏洞的攻击者可以完全控制受影响的系统。

#### 打印后台处理程序漏洞 (CAN-2005-1984) 的缓解因素：
...
-   在 Windows XP Service Pack 2 和 Windows Server 2003 上，此问题将导致拒绝服务情形。 在 Windows XP Service Pack 2 和 Windows Server 2003 上，不能利用此问题来远程执行代码或进行特权提升。
    在其他操作系统版本上，尝试利用此漏洞的攻击很可能导致拒绝服务情形。 但是，很有可能是远程执行代码。
-   采用防火墙最佳做法和标准的默认防火墙配置，有助于保护网络免受从企业外部发起的攻击。 按照最佳做法，应使连接到 Internet 的系统所暴露的端口数尽可能少。

#### 打印后台处理程序漏洞 (CAN-2005-1984) 的变通方法：

Microsoft 已测试以下变通办法。 尽管这些变通办法不能从根本上消除此漏洞，但它们有助于阻止已知的攻击媒介。 如果某种变通办法导致功能下降，下一节中将进行说明。

-   **禁用 Print Spooler 服务**

    禁用 Print Spooler 服务将帮助防止受影响的系统尝试利用此漏洞。 若要禁用打印 Print Spooler 服务，请按照以下步骤操作：
...
-   **在 Windows 2000 Server Service Pack 4 上，从 NullSessionPipes 注册表项中删除 Print Spooler 服务：**
    Windows 2000 Server Service Pack 4 和 Windows XP Service Pack 1 允许匿名连接到受影响的服务。 为帮助阻止匿名攻击者尝试利用此漏洞，从 NullSessionPipes 注册表子项中删除 Print Spooler 服务。 此变通办法不将阻止来自经过身份验证的用户的攻击。 如果无法禁用 Printer Spooler 服务，则使用此变通方法。
...
#### 打印后台处理程序漏洞 (CAN-2005-1984) 的常见问题解答：
...
Print Spooler 服务 Spoolsv.exe 是作为服务安装的可执行文件。 当操作系统启动时加载后台处理程序，它将持续运行，直到操作系统关闭。 Print Spooler 服务管理打印进程，包括检索正确的打印机驱动程序位置、加载该驱动程序、后台处理高级级函数调用至打印作业以及计划打印作业等任务。 当某个特定打印作业的任务完成时，Print Spooler 服务将把此作业传给打印路由器。 有关 Print Spooler 服务的详细信息，请访问以下[网站](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/techref/d58ce7b9-49cf-4f5e-95e9-1ade005c13e0.mspx)。

**攻击者可能利用此漏洞执行什么操作？**  
成功利用此漏洞以远程执行代码的攻击者可以完全控制受影响的系统。 在 Windows XP Service Pack 2 和 Windows Server 2003 上，此问题将导致拒绝服务的情况。 在其他操作系统版本上，尝试利用此漏洞很可能导致拒绝服务的情况。 但是，很有可能是远程执行代码。
...
**攻击者如何利用此漏洞？**  
攻击者可能试图通过创建特制的消息并将其发送到受影响的系统来利用此漏洞。 此消息可能造成受影响的系统在容易受到远程攻击媒介攻击的操作系统版本和配置上执行代码。 默认情况下，Windows 2000 和 Windows XP Service Pack 1 容易受到远程攻击。 无法在 Windows XP SP2 或 Windows Server 2003 上创建远程攻击媒介，除非拥有合适权限的用户共享打印机或试图连接到共享打印机。
要在所有操作系统版本上以本地方式利用此漏洞，攻击者必须先登录到系统。 然后，攻击者可以运行一个经过特制的、可利用此漏洞的应用程序。
...
**注意** 使用 **/quiet** 开关将抑制所有消息。 这包括抑制失败消息。 管理员应该使用其中一种支持的方法，验证使用 **/quiet** 开关时安装是否成功。 管理员还应该检查 KB896423.log 文件，查看使用此开关时的任何失败消息。
...
此更新不需要重新启动。 为了降低被迫重新启动的机率，请首先停止后台处理程序服务并且关闭所有可能使用受影响的文件的应用程序，然后安装安全更新。 有关可能提示您重新启动计算机的原因的详细信息，请参阅 [Microsoft 知识库文章 887012](https://support.microsoft.com/kb/887012)。

**删除信息**

要删除此更新，请使用“控制面板”中的“添加/删除程序”工具。

系统管理员也可以使用 Spuninst.exe 实用工具删除此安全更新。 Spuninst.exe 实用工具位于 %Windir%\\$NTUninstallKB896423$\\Spuninst 文件夹中。
...
-   **注册表项验证**
    ...
    **注意** 此注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 896423 安全更新集成或组合到 Windows 安装源文件中，可能无法正确创建此注册表项。
...
-   **注册表项验证**
    ...
    **注意** 这些注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 896423 安全更新集成或组合到 Windows 安装源文件中，可能无法正确创建这些注册表项。
...
-   **注册表项验证**
    ...
    **注意** 此注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 896423 安全更新集成或组合到 Windows 安装源文件中，则可能无法正确创建此注册表项。
...
-   CERT RENATER 的 [Kostya Kortchinsky](https://www.microsoft.com/technet/security/bulletin/邮件地址：kostya.kortchinsky@renater.fr) 报告了打印后台处理程序漏洞 (CAN-2005-1984)。
...
-   V1.1（2005 年 8 月 17 日）： 已更新公告，阐述“变通办法”部分提供的文本。
-   V1.2（2007 年 4 月 18 日）： 已更新公告，作为“摘要”部分的注意事项以及“与此安全更新相关的常见问题”部分中的已知问题介绍 Microsoft 知识库文章 908506。
