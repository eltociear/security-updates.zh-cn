...
|--------------------------------------------------------------------------------------------------|----------------|---------------------------|---------------------|------------|---------------------|
| Kerberos 漏洞 - [CAN-2005-1981](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-1981) | 拒绝服务       | 无                        | 中等                | 无         | 中等                |
| PKINIT 漏洞 - [CAN-2005-1982](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-1982)   | 信息泄露和欺骗 | 低                        | 低                  | 低         | 低                  |
...
是。 除了本公告“漏洞详细资料”部分列出的更改外，该更新还包含对功能的以下更改： 提供的安全更新解决外部报告的安全漏洞。 但是，已经添加新的可选的、但建议的注册表项 RequireAsChecksum，以提供额外的保护，帮助防止其他潜在的与 PKINIT 相关的漏洞。 此注册表项在 [Microsoft 知识文库文章 904766](https://support.microsoft.com/kb/904766) 中进行了介绍。REG\_DWORD 注册表值位于以下注册表项：
...
存在的[拒绝服务](https://go.microsoft.com/fwlink/?linkid=21142)漏洞可能允许攻击者将特制消息发送至 Windows 域控制器，从而导致负责对 Active Directory 域中的用户进行身份验证的服务停止响应。
...
**此漏洞的影响范围有多大？**  
这是一个[拒绝服务](https://go.microsoft.com/fwlink/?linkid=21142)漏洞。 利用此漏洞的攻击者可能会导致服务器自动重新启动，而且在此期间，还会使服务器停止响应身份验证请求。 此漏洞存在于执行域控制器角色的系统（例如 Windows 2000 Server 或 Window Server 2003）中。对客户端的影响仅仅是：如果客户端的域控制器停止响应，则客户端可能无法登录到域。
...
Windows 2000 和更高操作系统版本使用 Kerberos 作为默认的身份验证协议。 Kerberos 提供安全用户身份验证。 同时，因为它是行业标准，Kerberos 允许互操作性。 Active Directory 域控制器维护用户账户和登录信息以支持 Kerberos 服务。 例如，Kerberos 是用于访问 [Active Directory](https://go.microsoft.com/fwlink/?linkid=21200) 中数据的协议。 有关 Kerberos 的详细信息，请访问下列 [Kerberos 身份验证解释的网站](https://www.microsoft.com/technet/prodtechnol/windows2000serv/maintain/featusability/winmsi.mspx)或 [TechNet 网站](https://www.microsoft.com/technet/archive/security/news/kerb2000.mspx)。
...
-   攻击者使用的帐户和此攻击目标所使用的帐户必须为其帐户启用智能卡身份验证。 有关在企业内部启用智能卡使用的所需步骤的详细信息，请访问以下[网站](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/serverhelp/b989f4fd-febd-42e1-a130-9e0f33800734.mspx)。
...
**什么是 PKINT？**  
PKINIT 是“Kerberos 初始身份验证之公钥加密技术”的 Internet 工程任务小组 (IETF) Internet 草案。 Windows 2000 和更高版本使用 IETF“Kerberos 初始身份验证之公钥加密技术”Internet 草案 的草案 9。 当您使用智能卡交互式登录时，Windows 使用此协议。 IETF Internet 草案在下列 [IETF 网站](https://www.ietf.org)提供。
...
-   [Shell, Inc.](https://www.shell.com) 的 [Tony Chin](https://www.microsoft.com/technet/security/bulletin/tony.chin@shell.com) 报告了 Kerberos 漏洞 (CAN-2005-1981)。
