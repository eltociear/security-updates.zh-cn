---
TOCTitle: 'MS11-001'
Title: 'Microsoft 安全公告 MS11-001 - 重要'
ms:assetid: 'ms11-001'
ms:contentKeyID: 61237946
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-001(v=Security.10)'
---
...
Windows 备份管理器中的漏洞可能允许远程执行代码 (2478935)
--------------------------------------------------------
...
对于 Windows Vista 所有受支持的版本，此安全更新等级为“重要”。 有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

此安全更新通过更正 Windows 备份管理器加载外部库的方式来解决此漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows Vista Service Pack 1 和 Windows Vista Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=4eeaad20-729c-4594-8853-e4ae55e9d491)                         | 远程执行代码 | 重要         | 无               |
| [Windows Vista x64 Edition Service Pack 1 和 Windows Vista x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=b2839cd0-1958-4a22-9aac-0e0aa8f2b52c) | 远程执行代码 | 重要         | 无               |
...
<tr class="header">
<th>受影响的软件</th>
<th>备份管理器库加载不安全漏洞 - CVE-2010-3145</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
备份管理器库加载不安全漏洞 - CVE-2010-3145  
------------------------------------------
  
<span></span>  
Microsoft Windows 备份管理器处理 DLL 文件加载的方式中存在一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。 那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2010-3145](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3145)。
  
#### 备份管理器库加载不安全漏洞 (CVE-2010-3145) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   要成功进行攻击，用户必须访问不受信任的远程文件系统位置或 WebDAV 共享并打开 Windows 备份目录 (.wbcat) 文件。  
-   外围防火墙通常禁用文件共享协议服务器消息块 (SMB)。 这样可限制此漏洞的可能攻击媒介。
  
#### 备份管理器库加载不安全漏洞 (CVE-2010-3145) 的变通方法
...
#### 备份管理器库加载不安全漏洞 (CVE-2010-3145) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成此漏洞的原因是什么？**    
此漏洞是由于 Windows 备份管理器不正确限制加载外部库使用的路径所导致的。
  
**什么是 Windows 备份管理器？**    
Windows 备份管理器 (sdclt.exe) 使用户可以将计算机的系统文件还原到以前的时间点。 “系统还原”使用一种叫做系统保护的功能定期在计算机上创建还原点并进行保存。 这些还原点包含注册表设置的相关信息以及 Windows 使用的其他系统信息。 这些还原点也可以手动创建。
...
**攻击者如何利用此漏洞？**    
此漏洞需要用户打开与特制动态链接库 (DLL) 文件位于同一目录下的合法 .wbcat 文件。 当用户打开 .wbcat 文件后，Windows 备份管理器应用程序可能尝试加载特制的 DLL 文件并执行其中所包含的任何代码。
  
在网络攻击情形中，攻击者可能将合法的 .wbcat 文件与特制 DLL 一起放在网络共享上，例如 UNC 或 WebDAV 位置，然后诱使用户打开 .wbcat 文件。 当用户打开 .wbcat 文件后，可能会加载与该 .wbcat 文件相同的目录中包含的特制 DLL 文件。 攻击者无法强迫用户从网络共享打开 .wbcat 文件。 相反，攻击者必须诱使他们打开该文件，方法通常是让其单击电子邮件或 Instant Messenger 消息中的链接以使用户链接到网络共享，然后诱使他们打开 .wbcat 文件。
  
在电子邮件攻击情形中，攻击者可能通过向用户发送一个合法的 .wbcat 附件，并诱使用户将该附件放到包含特制 DLL 文件的目录中，然后诱使用户打开该 .wbcat 文件来利用此漏洞。 当用户打开 .wbcat 文件后，可能会加载与该 .wbcat 文件相同的目录中包含的特制 DLL 文件。 在网络攻击情形中，如果用户单击电子邮件中的链接，他们可能很容易受到利用此漏洞的影响。
  
**受此漏洞威胁最大的系统有哪些？**    
此漏洞主要影响可能期望用户从不受信任的网络位置打开附件的工作站。 如果管理员允许用户登录服务器并运行程序，服务器同样可能面临风险。 不过，最佳做法是不要授予这样的权限。
  
**此更新有什么作用？**    
此更新通过更正 Windows 备份管理器加载外部库的方式来解决此漏洞。
  
**发布此安全公告时，此漏洞是否已公开披露？**    
是。 此漏洞已公开披露。 已为此漏洞分配了常见漏洞和披露号码 [CVE-2010-3145](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3145)。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2478935">Microsoft 知识库文章 2478935</a></td>
</tr>  
<tr class="odd">
