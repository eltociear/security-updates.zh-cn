---
TOCTitle: 'MS11-062'
Title: 'Microsoft 安全公告 MS11-062 - 重要'
ms:assetid: 'ms11-062'
ms:contentKeyID: 61238200
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-062(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS11-062 - 重要
==================================

远程访问服务 NDISTAPI 驱动程序中的漏洞可能允许特权提升 (2566454)
----------------------------------------------------------------
...
此安全更新解决了 Windows XP 和 Windows Server 2003 所有受支持的版本中一个秘密报告的漏洞。对于 Windows XP 和 Windows Server 2003 的所有受支持版本，此安全更新的等级为“重要”。Windows Vista、Windows Server 2008、Windows 7 和 Windows Server 2008 R2 不受此漏洞影响。 有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

如果攻击者登录受影响的系统，并运行设计为利用该漏洞并完全控制受影响的系统的特制应用程序，该漏洞可能允许特权提升。 攻击者必须拥有有效的登录凭据并能本地登录才能利用此漏洞。

该安全更新通过更正 NDISTAPI 驱动程序在将用户模式输入发送到 Windows 内核之前验证该输入的方式来解决漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows XP Service Pack 3](https://www.microsoft.com/download/details.aspx?familyid=6bc1f0ac-223d-4b0b-86cd-2ba3863955c4)                          | 特权提升     | 重要         | 无               |
| [Windows XP Professional x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=d5ee6787-408d-4870-af70-9338158b161b) | 特权提升     | 重要         | 无               |
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=8de0f2db-aa09-48cd-a13b-e26a973e9cdc)                 | 特权提升     | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=8f208407-4bb3-41fe-b0d6-fc8a5e30e667)     | 特权提升     | 重要         | 无               |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=ef140089-d022-4ee8-9cc4-7fb25295a39d) | 特权提升     | 重要         | 无               |
...
<tr class="header">
<th>受影响的软件</th>
<th>NDISTAPI 特权提升漏洞 - CVE-2011-1974</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
NDISTAPI 特权提升漏洞 - CVE-2011-1974  
-------------------------------------
  
<span></span>  
远程访问服务 NDISTAPI 驱动程序中存在一个特权提升漏洞。 如果 NDISTAPI 驱动程序在将数据从用户模式传递到 Windows 内核时未正确验证用户提供的输入，则会导致该漏洞。 成功利用此漏洞的本地攻击者可执行任意代码，并可完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1974](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1974)。
  
#### NDISTAPI 特权提升漏洞 (CVE-2011-1974) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   攻击者必须拥有有效的登录凭据并能本地登录才能利用此漏洞。
  
#### NDISTAPI 特权提升漏洞 (CVE-2011-1974) 的变通办法
  
Microsoft 并未发现此漏洞的任何变通办法。
  
#### NDISTAPI 特权提升漏洞 (CVE-2011-1974) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个特权提升漏洞。
  
**造成此漏洞的原因是什么？**    
如果 NDISTAPI 驱动程序在将数据从用户模式传递到 Windows 内核时未正确验证用户提供的输入，则会导致该漏洞。
  
**哪个组件受此漏洞的影响？**    
NDISTAPI.sys 组件会受此漏洞的影响。
  
**什么是远程访问服务 (RAS)？**    
远程访问服务 (RAS) 让用户能够通过电话线、Internet 或其他网络连接连接到远程计算机，这样，他们可以就像系统物理连接到远程网络一样进行工作。 RAS 使用户能够远程执行发送和接收电子邮件、以传真方式发送文档、检索文件以及在办公室打印机上打印文档等活动。 有关详细信息，请参阅 MSDN 文章“[RAS](https://msdn.microsoft.com/en-us/library/ff563285(v=vs.85).aspx) 体系结构概述 (NDIS 5.1)”。
  
RAS 是 Windows XP 和 Windows Server 2003 中的一项本地服务。在更高版本的操作系统中，RAS 将替换为路由和远程访问服务 (RRAS)。 RRAS 使计算机可能充当网络路由器。 RRAS 也为 Windows 提供了下一代的 RAS 服务器功能。 RRAS 服务器功能建立在以前的操作系统中包括的 RAS 功能的基础之上。
  
**什么是 NDISTAPI 驱动程序？**    
NDISTAPI 驱动程序是 RAS 体系结构的一部分，将 NDISWAN 连接到 TAPI 服务。 电话应用程序实施 TAPI 服务，可以发出 TAPI 请求以设置、监控、终止线路和呼叫。 NDISTAPI 驱动程序将这些 TAPI 请求传递到 NDISWAN。 有关详细信息，请参阅 MSDN 文章“[NDISTAPI 概述 (NDIS 5.1)](https://msdn.microsoft.com/en-us/library/ff554813(v=vs.85).aspx)”。
  
**什么是 NDISWAN？**    
网络驱动程序接口标准广域网 (NDISWAN) 是一种系统提供的 NDIS 中间驱动程序，提供了数据压缩、加密、环回和 WAN 微型端口驱动程序使用的简单 PPP 帧等功能。 因此，WAN 微型端口驱动程序是仅实施特定于媒体的那些功能所必需的（例如，对于 ISDN 需要 Q931 信号）。 有关详细信息，请参阅 MSDN 文章 [NDISWAN 概述](https://msdn.microsoft.com/en-us/library/ff564653(v=vs.85).aspx)。
  
**什么是 TAPI 服务？**    
电话应用程序编程接口 (TAPI) 服务是一种向支持电话通信的应用程序提供电话服务的编程接口。
  
**什么是 NDIS？**    
网络驱动程序接口标准 (NDIS) 是 Windows Driver Kit 的一部分，Windows Driver Kit 是一种完全集成的驱动程序开发系统，包含 Windows Driver Device Kit 以及 Windows 驱动程序的稳定性和可靠性测试。 NDIS 库将网络驱动程序中的网络硬件抽象化。 NDIS 也指定分层网络驱动程序之间的标准接口，从而将管理较高级别驱动程序（如网络传输）的较低级别驱动程序抽象化。 NDIS 也维护网络驱动程序的状态信息和参数，包括函数指针、句柄和参数块以及其他系统值。
...
**此更新有什么作用？**    
该更新通过更正 NDISTAPI 驱动程序在将用户模式输入发送到 Windows 内核之前验证该输入的方式来解决漏洞。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2566454$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2566454">Microsoft 知识库文章 2566454</a></td>
</tr>  
<tr class="even">
...
-   的 Lufeng Li 报告了 NDISTAPI 特权提升漏洞 (CVE-2011-1974)
