---
TOCTitle: 'MS11-035'
Title: 'Microsoft 安全公告 MS11-035 - 严重'
ms:assetid: 'ms11-035'
ms:contentKeyID: 61238173
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-035(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS11-035 - 严重
==================================
...
对于安装了 WINS 的 Windows Server 2003、Windows Server 2008（Itanium 除外）和 Windows Server 2008 R2（Itanium 除外）的所有受支持版本，此安全更新的等级为“严重”。 有关详细信息，请参阅本节中“**受影响和不受影响的软件**”小节。

该安全更新通过更正作为参数传递缓冲区时发生的逻辑错误来解决漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
**已知问题。** [Microsoft 知识库文章 2524426](https://support.microsoft.com/kb/2524426) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。 本文还介绍了这些问题的建议解决办法。
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|-----------------------------------------------------------|
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=0d289146-8470-45dd-a886-d1b35a2c6d4b)                                                                            | 远程执行代码 | 严重         | [MS09-039](https://go.microsoft.com/fwlink/?linkid=155974) |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=0d3e85a9-42ff-447b-ab8c-095a1c0b224c)                                                                | 远程执行代码 | 严重         | [MS09-039](https://go.microsoft.com/fwlink/?linkid=155974) |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=4c388fe8-883a-4118-a7ff-50f1e9d0a071)                                                            | 远程执行代码 | 严重         | [MS09-039](https://go.microsoft.com/fwlink/?linkid=155974) |
| [Windows Server 2008（用于 32 位系统）和 Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=9019a3ff-d953-4b0b-8c7a-05aa9cf65c92)\*                 | 远程执行代码 | 严重         | 无                                                        |
| [Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=841130fa-4522-4079-b7eb-f8466ac3ef12)\*       | 远程执行代码 | 严重         | 无                                                        |
| [Windows Server 2008 R2（用于基于 x64 的系统）和 Windows Server 2008 R2（用于基于 x64 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=8732b41e-2f25-4ee1-a8b7-a8b8b9e22d08)\* | 远程执行代码 | 严重         | 无                                                        |
...
只有当“受影响的软件”表中列出的操作系统上安装了 WINS 时，才可以安装该更新。 默认情况下，这些操作系统上未安装 WINS，因此除非已经安装了 WINS，否则不提供更新。
...
<tr class="header">
<th>受影响的软件</th>
<th>WINS 服务失败响应漏洞 - CVE-2011-1248</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
WINS 服务失败响应漏洞 - CVE-2011-1248  
-------------------------------------
  
<span></span>  
由于发送到 WINS 服务的特制 WINS 网络数据包中的数据结构验证不足，Windows Internet 名称服务 (WINS) 中存在一个远程执行代码漏洞。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1248](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1248)。
  
#### WINS 服务失败响应漏洞 (CVE-2011-1248) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   默认情况下，WINS 不会安装在任何受影响的操作系统上。 仅手动安装此组件的客户可能受到此问题的影响。
  
#### WINS 服务失败响应漏洞 (CVE-2011-1248) 的变通办法
...
#### WINS 服务失败响应漏洞 (CVE-2011-1248) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以使用提升的特权在容易受攻击的系统上运行任意代码。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成此漏洞的原因是什么？**    
由于 WINS 处理套接字和异常时的逻辑错误，用户提供的特定值将保留在堆栈帧中，并且将在另一上下文中重复使用。 成功利用此漏洞的远程攻击者可能以一种可能导致远程执行代码的方式来破坏系统内存。 然后，攻击者可能会在 Windows Server 2003 的系统上下文或者 Windows Server 2008 和 Windows Server 2008 R2 的本地服务上下文中控制系统。
  
**什么是 WINS？**    
Windows Internet 名称服务 (WINS) 专门为支持 TCP/IP 上的 NetBIOS (NetBT) 而设计的。 用户在其中访问具有 NetBIOS 名称的资源的任何环境都需要 WINS。 如果不在此类网络中使用 WINS，除非您使用 LMHOSTS 文件，否则无法使用其 NetBIOS 名称连接到远程网络资源，且可能无法建立文件和打印共享连接。 有关详细信息，请参阅 TechNet 文章“[什么是 WINS？](https://technet.microsoft.com/en-us/library/cc784180.aspx)”
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用此漏洞的攻击者可以运行任意代码。 然后，攻击者可以安装程序；查看、更改或删除数据；或者使用 WINS 服务的权限创建新帐户。
  
**攻击者如何利用此漏洞？**    
攻击者可能通过创建特制的数据包并将其发送到受影响的 WINS 系统来利用此漏洞。
  
**受此漏洞威胁最大的系统有哪些？**    
Windows Server 2003、Windows Server 2008 或 Windows Server 2008 R2 上运行的 WINS 服务器受此漏洞威胁最大。
  
**此更新有什么作用？**    
该更新可更正在作为参数传递缓冲区时发生的逻辑错误，该错误违反函数之间的约定。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2524426$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2524426">Microsoft 知识库文章 2524426</a></td>
</tr>  
<tr class="even">
...
-   Luigi Auriemma 与 [TippingPoint's](https://www.tippingpoint.com/) [Zero Day Initiative](https://www.zerodayinitiative.com/) 合作报告了 WINS 服务失败响应漏洞 (CVE-2011-1248)
