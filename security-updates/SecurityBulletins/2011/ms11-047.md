---
TOCTitle: 'MS11-047'
Title: 'Microsoft 安全公告 MS11-047 - 重要'
ms:assetid: 'ms11-047'
ms:contentKeyID: 61238185
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-047(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS11-047 - 重要
==================================
...
该安全更新通过更正 Hyper-V 服务器处理虚拟机中格式错误的请求的方式来解决此漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|-----------------------------------------------------------|
| [Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=24789423-72b7-48d1-bdc1-f0e5174d99bb)\* | 拒绝服务     | 重要         | [MS10-102](https://go.microsoft.com/fwlink/?linkid=205309) |
| [Windows Server 2008 R2（用于基于 x64 的系统）](https://www.microsoft.com/download/details.aspx?familyid=c9c6c36d-a455-42f7-b7d4-9fb9824c07cb)\*                                                         | 拒绝服务     | 重要         | [MS10-102](https://go.microsoft.com/fwlink/?linkid=205309) |
| [Windows Server 2008 R2（用于基于 x64 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=c9c6c36d-a455-42f7-b7d4-9fb9824c07cb)\*                                           | 拒绝服务     | 重要         | 无                                                        |
...
<tr class="header">
<th>受影响的软件</th>
<th>VMBus 持久 DoS 漏洞 - CVE-2011-1872</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
VMBus 持久 DoS 漏洞 - CVE-2011-1872  
-----------------------------------
  
<span></span>  
Windows Server 2008 和 Windows Server 2008 R2 上的 Hyper-V 中存在一个拒绝服务。该漏洞是由于 Hyper-V 服务器未充分验证计算机指令的特定顺序造成的。 成功利用此漏洞的攻击者可能会导致 Hyper-V 系统停止响应。 这将影响受该系统托管的所有虚拟机。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1872](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1872)。
  
#### VMBus 持久 DoS 漏洞 (CVE-2011-1872) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   攻击者必须拥有有效的登录凭据并能本地登录才能利用此漏洞。 匿名用户无法利用此漏洞，也无法以远程方式利用此漏洞。
  
#### VMBus 持久 DoS 漏洞 (CVE-2011-1872) 的变通办法
  
Microsoft 并未发现此漏洞的任何变通办法。
  
#### VMBus 持久 DoS 漏洞 (CVE-2011-1872) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个拒绝服务漏洞。 成功利用此漏洞的攻击者可能会导致 Hyper-V 服务器停止响应，需要重新启动。 请注意，虽然攻击者无法利用拒绝服务漏洞在 Hyper-V 服务器上执行代码或提升其用户权限，但此漏洞可能导致受影响的系统停止接受请求。
  
**造成此漏洞的原因是什么？**    
该漏洞是由于从来宾发送到主机的特定数据包的不充分验证导致的。 由于缺少验证，处理这些指令会导致 Hyper-V 服务器应用程序过度消耗资源。
  
**什么是 Hyper-V？**    
Hyper-V 是基于虚拟机监控程序的技术，该技术是 Windows Server 2008 和 Windows Server 2008 R2 的主要功能。它提供可伸缩、可靠和高度可用的虚拟化平台。 此类环境非常有用，因为您可以创建和管理虚拟机，从而允许您在一台物理计算机上运行多个操作系统，并将操作系统相互隔离。
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用此漏洞的攻击者可能导致用户的系统停止响应并需要重新启动。 请注意，此漏洞可能导致 Hyper-V 服务器的资源耗尽，包括由该服务器托管的所有来宾虚拟机。
  
**攻击者如何利用此漏洞？**    
攻击者必须是由 Hyper-V 服务器托管的其中一个来宾虚拟机中的经身份验证的管理员。 攻击者必须能够在该系统上执行任意代码，例如通过运行不受信任的可执行文件。 然后，此可执行文件可能会调用错误的计算机指令顺序，从而导致资源消耗，并且 Hyper-V 服务器无限循环，无法处理其他虚拟机的请求。
  
**攻击者需要哪些权限才能执行此拒绝服务攻击？**    
为了对 Hyper-V 服务器和 Hyper-V 服务器上所有正在运行的虚拟机执行此拒绝服务攻击，攻击者必须对 Hyper-V 服务器上托管的其中一个虚拟机具有管理员特权。 攻击者无需对 Hyper-V 服务器本身具有管理员访问权限。
  
**受此漏洞威胁最大的系统有哪些？**    
允许访问不受信任的用户的托管客户虚拟机的 Hyper-V 服务器受此漏洞的威胁最大。
  
**此更新有什么作用？**    
此更新更正了 Hyper-V 服务器处理来自虚拟机的格式错误的请求的方式。 修补程序允许 Hyper-V 服务器应用程序正确验证从来宾发送到主机的 VMBus 数据包。
...
<td style="border:1px solid black;">对于 Windows Server 2008 所有受支持的基于 x64 的版本上安装的 Hyper-V 服务器：<br />
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2525835">Microsoft 知识库文章 2525835</a></td>
</tr>  
<tr class="odd">
...
<tr class="odd">
<td style="border:1px solid black;">没有用户干预的安装</td>
<td style="border:1px solid black;">对于 Windows Server 2008 R2 所有受支持的基于 x64 的版本上安装的 Hyper-V 服务器：<br />
...
-   [Core Security Technologies](https://www.coresecurity.com/) 的 Nicolas Economou 报告了 VMBus 持久 DoS 漏洞 (CVE-2011-1872)
