---
TOCTitle: 'MS11-073'
Title: 'Microsoft 安全公告 MS11-073 - 重要'
ms:assetid: 'ms11-073'
ms:contentKeyID: 61238211
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-073(v=Security.10)'
---


Microsoft 安全公告 MS11-073 - 重要
==================================
...
此安全更新解决 Microsoft Office****中两个秘密报告的漏洞。如果用户打开特制的 Office 文件，或者如果用户打开特制库文件所在的相同网络目录中的某个合法 Office 文件，则该漏洞可能允许远程执行代码。成功利用其中任何一个漏洞的攻击者可以获得与登录用户相同的用户权限。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

对于 Microsoft Office 2003、Microsoft Office 2007 和 Microsoft Office 2010 的所有受支持版本，此安全更新的等级为“重要”。有关详细信息，请参阅本节中的“**受影响和不受影响的软件**”小节。

此安全更新通过更正 Microsoft Office 加载外部库和初始化对象的方式来解决漏洞。有关这些漏洞的详细信息，请参阅下一节“漏洞信息”下面特定漏洞条目的“常见问题 (FAQ)”小节。
...
<tr>
<td style="border:1px solid black;">
[Microsoft Office 2010（32 位版本）](https://www.microsoft.com/download/details.aspx?familyid=3c8fd04a-9df6-4726-a9bc-811f49665981)  
(KB2584066)
</td>
<td style="border:1px solid black;">
远程执行代码
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Microsoft Office 2010 Service Pack 1（32 位版本）](https://www.microsoft.com/download/details.aspx?familyid=3c8fd04a-9df6-4726-a9bc-811f49665981)  
(KB2584066)
</td>
<td style="border:1px solid black;">
远程执行代码
...
<tr>
<td style="border:1px solid black;">
[Microsoft Office 2010（64 位版本）](https://www.microsoft.com/download/details.aspx?familyid=85360dc1-99e7-4e3e-be6f-3795e8a8122f)  
(KB2584066)
</td>
<td style="border:1px solid black;">
远程执行代码
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Microsoft Office 2010 Service Pack 1（64 位版本）](https://www.microsoft.com/download/details.aspx?familyid=85360dc1-99e7-4e3e-be6f-3795e8a8122f)  
(KB2584066)
</td>
<td style="border:1px solid black;">
远程执行代码
...
<th>
Office 组件不安全库加载漏洞 - CVE-2011-1980
</th>
<th>
Office 未初始化的对象指针漏洞 - CVE-2011-1982
</th>
<th>
综合严重等级
</th>
...
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1980](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1980)。

#### Office 组件不安全库加载漏洞 (CVE-2011-1980) 的缓解因素

缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。以下缓解因素在您遇到的情形中可能会有所帮助：

-   攻击要想成功，用户必须访问不受信任的远程文件系统位置或 WebDAV 共享，并打开与 Office 相关的文件（例如 .doc 文件）。
-   外围防火墙通常禁用文件共享协议服务器消息块 (SMB)。这样可限制此漏洞的可能攻击媒介。
-   成功利用此漏洞的攻击者可以获得与登录用户相同的用户权限。那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。

#### Office 组件不安全库加载漏洞 (CVE-2011-1980) 的变通办法
...
-   **禁用从 WebDAV 和远程网络共享中加载库**
    ...
    **意** 参见 [Microsoft 知识库文章 2264107](https://support.microsoft.com/kb/2264107) 以部署一个变通办法工具，使客户能够禁用从远程网络或 WebDAV 共享加载库。可以配置该工具以禁止通过每个应用程序或全局系统进行非安全加载。
...
#### Office 组件不安全库加载漏洞 (CVE-2011-1980) 的常见问题
...
尽管已知攻击媒介只能利用特定的 Microsoft Office 二进制格式，但 Microsoft Office 的共享组件中存在易受攻击的代码。
...
**攻击者如何利用此漏洞？**  
当未安装 Office 文件验证加载项时，此漏洞仅影响 .doc、.xls 和 .ppt Office 二进制格式。

攻击者可能诱使用户打开与特制动态链接库 (DLL) 文件位于同一网络目录下的合法 Office 相关文件（例如 .doc 文件）。然后，当打开该合法文件时，Microsoft Office 可能尝试加载 DLL 文件并执行其包含的任何代码。

在电子邮件攻击情形中，攻击者可能通过向用户发送合法的 Office 相关文件附件（例如 .doc 文件），并诱使用户将此附件放在包含特制 DLL 文件的目录下并打开该合法文件来利用此漏洞。然后，当打开该合法文件时，Microsoft Office 可能尝试加载 DLL 文件并执行其包含的任何代码。

在网络攻击情形中，攻击者可能将合法的 Office 相关文件与特制 DLL 文件一起放在网络共享上，例如 UNC 或 WebDAV 位置，然后诱使用户打开该文件。

**什么是 Microsoft Office 文件验证加载项？**  
Office 文件验证是 Office 2003 和 2007 的安全加载项。Office 文件验证用于验证二进制文件格式文件是否符合 Microsoft Office 文件格式。如果用户不符合格式，则将向用户通知可能的安全风险。可以在[知识库文章 2501584](https://support.microsoft.com/kb/2501584) 中找到有关此加载项的更多信息。
...
Microsoft Office 处理特制 Word 文件的方式中存在一个远程执行代码漏洞。成功利用此漏洞的攻击者可以完全控制受影响的系统。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。

要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1982](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1982)。

#### Office 未初始化的对象指针漏洞 (CVE-2011-1982) 的缓解因素
...
#### Office 未初始化的对象指针漏洞 (CVE-2011-1982) 的变通办法
...
-   **使用 Microsoft Office 文件阻止策略禁止打开来自未知或不可信来源和位置的 Office 2003 以及较早版本的文档。**
    ...
    **对于 Microsoft Office 2010**
    ...
    打开 Microsoft Word 2010，并执行以下步骤：
...
**攻击者如何利用此漏洞？**  
利用此漏洞需要用户使用 Microsoft Office 的受影响版本打开特制 Word 文件。
...
**受此漏洞威胁最大的系统有哪些？**  
使用 Microsoft Office 的系统（如工作站）受到的威胁最大。如果管理员允许用户登录服务器并运行程序，服务器可能面临更大风险。不过，最佳做法是不要授予这样的权限。

**此更新有什么作用？**  
该更新通过更正 Microsoft Office 初始化对象的方式来解决漏洞。
...
**注意** 对于使用不受最新版本的 MBSA、Microsoft Update 和 Windows Server Update Services 支持的旧版软件的客户，请访问 [Microsoft Baseline Security Analyzer](https://technet.microsoft.com/security/cc184924) 并参考旧版产品支持部分，了解如何使用旧版工具创建全面的安全更新检测。
...
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2584052">Microsoft 知识库文章 2584052</a></td>
</tr>  
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2584063">Microsoft 知识库文章 2584063</a></td>
</tr>  
<tr class="even">
<td style="border:1px solid black;"><strong>注册表项验证</strong></td>
<td style="border:1px solid black;">不适用</td>
...
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2584066">Microsoft 知识库文章 2584066</a></td>
</tr>  
<tr class="even">
<td style="border:1px solid black;"><strong>注册表项验证</strong></td>
<td style="border:1px solid black;">不适用</td>
...
-   Parvez Anwar 与 [Secunia Research](https://secunia.com/) 合作报告了 Office 组件不安全库加载漏洞 (CVE-2011-1980) 中描述的问题  
-   [CERT/CC](https://www.cert.org/) 的 David Warren 报告了 Office 未初始化的对象指针漏洞 (CVE-2011-1982)
