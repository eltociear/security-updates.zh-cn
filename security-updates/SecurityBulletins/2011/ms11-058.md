---
TOCTitle: 'MS11-058'
Title: 'Microsoft 安全公告 MS11-058 - 严重'
ms:assetid: 'ms11-058'
ms:contentKeyID: 61238196
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-058(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS11-058 - 严重
==================================

DNS 服务器中的漏洞可能允许远程执行代码 (2562485)
------------------------------------------------

发布时间: 2011年8月9日 | 更新时间: 2011年10月25日
...
对于 Windows Server 2008 的 32 位和基于 x64 的版本以及 Windows Server 2008 R2 的基于 x64 的版本，此安全更新的等级为“严重”；对于 Windows Server 2003 的所有受支持版本，此安全更新的等级为“重要”。有关详细信息，请参阅本节中的“**受影响和不受影响的软件**”小节。

该安全更新通过修改 DNS 服务器处理内存中的 NAPTR 查询的方式以及在使用之前初始化内存中的对象的方式来解决漏洞。有关这些漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的“常见问题 (FAQ)”小节。
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|-----------------------------------------------------------|
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=c2d4aa38-9241-465d-8d65-aba73e936d0f)                                                                            | 拒绝服务     | 重要         | [MS09-008](https://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=2db4098a-f4f9-4211-b55d-5d0df1409c43)                                                                | 拒绝服务     | 重要         | [MS09-008](https://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=1934acfa-5637-4ae9-9e9a-fc7e58930b7a)                                                            | 拒绝服务     | 重要         | [MS09-008](https://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=ac2d5122-6f9b-46f5-9840-77aa7ff84308)\*                                                         | 远程执行代码 | 严重         | 无                                                        |
| [Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=5f605f6f-3854-403d-878b-637626aef78f)\*                                                    | 远程执行代码 | 严重         | 无                                                        |
| [Windows Server 2008 R2（用于基于 x64 的系统）和 Windows Server 2008 R2（用于基于 x64 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=6a86e2cf-4e7d-4012-88c3-6957a3842dd3)\* | 远程执行代码 | 严重         | 无                                                        |
...
**为什么在** **2011 年 10 月25** **日修订此公告？**  
Microsoft 修订此公告是为了宣布检测逻辑更改，以及更正 Windows Server 2003 的所有受支持版本的公告替代信息。没有更改安全更新文件。已经成功更新了其系统的客户不需要执行任何操作。
...
<tr class="header">
<th>受影响的软件</th>
<th>DNS NAPTR 查询漏洞 - CVE-2011-1966</th>
<th>DNS 未初始化的内存损坏漏洞 - CVE-2011-1970</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
DNS NAPTR 查询漏洞 - CVE-2011-1966  
----------------------------------
  
<span></span>  
Windows DNS 服务器不正确地处理内存中的特制 NAPTR 查询字符串的方式中存在一个远程代码执行漏洞。成功利用此漏洞的攻击者可以在系统的上下文中运行任意代码。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1966](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1966)。
  
#### DNS NAPTR 查询漏洞 (CVE-2011-1966) 的缓解因素
  
Microsoft 并未发现此漏洞的任何缓解因素。
  
#### DNS NAPTR 查询漏洞 (CVE-2011-1966) 的变通办法
...
-   **禁用不使用的 DNS 服务**
    ...
    **交互方法：**
    ...
    2.  依次展开 **HKEY\_LOCAL\_MACHINE**、**System**、**CurrentControlSet** 和 **Services**。  
    3.  单击“**DNS**”。  
    4.  单击“**文件**”菜单，然后选择“**导出**”。  
    5.  在“导出注册表文件”对话框中，键入 **DNS\_configuration\_backup.reg**，然后单击“**保存**”。  
    ...
    **使用被管理的部署脚本：**
    ...
    1.  使用下列命令备份被管理的部署脚本中的注册表项：  
        ...
        regedit /e DNS_configuration_backup.reg HKEY_LOCAL_MACHINE\System\  
        
        CurrentControlSet\Services\DNS

    2.  将下列文本保存到扩展名为 .REG 的文件（例如 Disable\_service\_DNSame.reg）：  
    ...
    3.  在目标计算机上，使用下列命令运行在上面的步骤 2 中创建的注册表脚本：  
    ...
    **交互方法：**
    ...
    1.  单击“**开始**”，单击“**运行**”，键入 **regedit**，然后单击“**确定**”。  
    2.  单击“**文件**”菜单，然后选择“**导入**”。  
    3.  在“导入注册表文件”窗口中，选择 **DNS\_configuration\_backup.reg**，然后单击“**打开**”。
  
    **使用被管理的部署脚本：**
  
    以管理员身份通过命令提示符运行下列命令，以将注册表还原为其原始状态：  
    
    Regedit /s DNS_configuration_backup.reg
  
#### DNS NAPTR 查询漏洞 (CVE-2011-1966) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个远程执行代码漏洞。
  
**造成此漏洞的原因是什么？**    
当非权威的 DNS 服务器不正确地处理内存中的特制 NAPTR 记录，同时以递归方式从权威 DNS 服务器中请求记录时，会导致该漏洞。
  
**哪个组件受此漏洞的影响？**    
Dns.exe 组件受此漏洞的影响。
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用此漏洞的攻击者可以在系统的上下文中运行任意代码。攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**攻击者如何利用此漏洞？**    
未经身份验证的远程攻击者可以通过注册域、创建 NAPTR DNS 资源记录，然后将特制 NAPTR 查询发送到目标 DNS 服务器来利用此漏洞。
  
**受此漏洞威胁最大的系统有哪些？**    
运行 DNS 角色的服务器受到的威胁最大。没有启用此角色的服务器不受威胁。
  
**什么是 NAPTR DNS 资源记录？**    
命名权威指针 (NAPTR) DNS 资源记录 (RR) 允许使用 DNS 为各种资源名称（包括未采用域名语法的 URL）查找服务。有关 NAPTR 的详细信息，请参阅 [RFC-2915](https://www.ietf.org/rfc/rfc2915.txt)。
  
**什么是 DNS 资源记录？**    
DNS 资源记录 (RR) 标识域中的特殊资源。有关详细信息，请参阅 TechNet 文章 [DNS 的工作原理](https://technet.microsoft.com/en-us/library/dd197446(ws.10).aspx)。
  
**什么是 URI？**    
统一资源标识符 (URI) 是指用于通过网络处理或识别 Internet 中的资源的字符串。URL 是一个典型的 URI 示例，它引用网站等资源。有关 URI 的详细信息，请参阅 [RFC-2396](https://www.ietf.org/rfc/rfc2915.txt)。
  
**此更新有什么作用？**    
此更新通过修改 DNS 服务器处理内存中的 NAPTR 查询的方式来解决漏洞。
...
DNS 未初始化的内存损坏漏洞 - CVE-2011-1970  
------------------------------------------
  
<span></span>  
DNS 服务器不正确地处理内存中未初始化的对象的方式中存在一个拒绝服务漏洞。成功利用此漏洞的攻击者可能导致目标系统上的 DNS 服务器服务停止响应。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1970](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1970)。
  
#### DNS 未初始化的内存损坏漏洞 (CVE-2011-1970) 的缓解因素
  
Microsoft 并未发现此漏洞的任何缓解因素。
  
#### DNS 未初始化的内存损坏漏洞 (CVE-2011-1970) 的变通办法
  
Microsoft 并未发现此漏洞的任何变通办法。
  
#### DNS 未初始化的内存损坏漏洞 (CVE-2011-1970) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个拒绝服务漏洞。
  
**造成此漏洞的原因是什么？**    
当 DNS 服务器不正确地处理内存中未初始化的对象，同时查找不存在的域的资源记录时，会导致该漏洞。
  
**哪个组件受此漏洞的影响？**    
Dns.exe 组件受此漏洞的影响。
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用此漏洞的攻击者可能导致目标系统上的 DNS 服务器服务停止响应。
  
**攻击者如何利用此漏洞？**    
未经身份验证的远程攻击者可以通过向目标 DNS 服务器发送特制 DNS 查询以查找不存在的域的资源记录来利用此漏洞。
...
**此更新有什么作用？**    
此更新通过修改 DNS 服务器在使用之前初始化内存中的对象的方式来解决漏洞。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2562485$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2562485">Microsoft 知识库文章 2562485</a></td>
</tr>  
<tr class="even">
...
-   Grischa Zengel (Zengel Medizintechnik GmbH) 报告了 DNS NAPTR 查询漏洞 (CVE-2011-1966)
...
-   V1.1（2011 年 9 月 21 日）： 更正了“受影响的软件”表，删除了 Windows Server 2003 的所有受影响版本上此更新取代的公告 MS11-046。这只是检测更改。没有更改安全更新文件。已经成功更新了其系统的客户不需要执行任何操作。  
-   V1.2（2011 年 10 月 25 日）： 宣布检测逻辑更改，并更正了一些受影响的配置的公告替代信息。没有更改安全更新文件。有关详细信息，请参阅更新常见问题。
