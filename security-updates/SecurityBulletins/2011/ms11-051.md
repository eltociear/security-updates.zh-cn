---
TOCTitle: 'MS11-051'
Title: 'Microsoft 安全公告 MS11-051 - 重要'
ms:assetid: 'ms11-051'
ms:contentKeyID: 61238189
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-051(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS11-051 - 重要
==================================

Active Directory 证书服务 Web 注册中的漏洞可能允许特权提升 (2518295)
--------------------------------------------------------------------

发布时间: 2011年6月14日 | 更新时间: 2011年6月15日
...
对于 Windows Server 2003、Windows Server 2008 和 Windows Server 2008 R2 的所有受支持版本（Itanium 除外），此安全更新的等级为“重要”。有关详细信息，请参阅本节的“**受影响和不受影响的软件**”小节。

此安全更新通过更正 Active Directory 证书服务 Web 注册站点验证输入参数的方式来解决漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
**已知问题。** [Microsoft 知识库文章 2518295](https://support.microsoft.com/kb/2518295) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。 本文还介绍了这些问题的建议解决办法。
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|------------------|
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=ef90d6c1-ea7f-4c32-9c90-0303e04c7436)                                                                              | 特权提升     | 重要         | 无               |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=62944095-33d6-4131-be32-a79d9ec4d4a9)                                                                  | 特权提升     | 重要         | 无               |
| [Windows Server 2008（用于 32 位系统）和 Windows Server 2008（用于 32 位系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=46ade106-e0cb-4c71-8230-793a15062823)\*\*                 | 特权提升     | 重要         | 无               |
| [Windows Server 2008（用于基于 x64 的系统）和 Windows Server 2008（用于基于 x64 的系统）Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=6141a1c5-ecaf-4553-9d27-dd6e5c4a13fd)\*\*       | 特权提升     | 重要         | 无               |
| [Windows Server 2008 R2（用于基于 x64 的系统）和 Windows Server 2008 R2（用于基于 x64 的系统）Service Pack 1](https://www.microsoft.com/download/details.aspx?familyid=1da04414-6210-43ea-8e0a-cf21cf144076)\*\* | 特权提升     | 重要         | 无               |
...
此更新仅提供给安装并启用受影响组件的系统。 未在 Active Directory 证书服务 Web 注册角色中配置的 Microsoft Windows Server 系统不受影响，不会收到此更新。

**我已经自定义了我的 Web 注册 ASP 页面。 我是否需要关注 Microsoft 知识库文章中列出的已知问题？**  
是的，Microsoft 建议编辑了其 Web 注册 ASP 页面的客户应该访问 [Microsoft 知识库文章 2518295](https://support.microsoft.com/kb/2518295) 以获取有关可能适用于其系统的已知问题的重要信息。
...
<tr class="header">
<th>受影响的软件</th>
<th>Active Directory 证书服务漏洞 - CVE-2011-1264</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
Active Directory 证书服务漏洞 - CVE-2011-1264  
---------------------------------------------
  
<span></span>  
Active Directory 证书服务 Web 注册中存在一个已反映的 XSS 漏洞，该漏洞可能允许攻击者在 Internet Explorer 的用户实例中注入客户端脚本。 该脚本可能欺骗内容、泄露信息或执行用户可以在网站上代表目标用户执行的任何操作。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-1264](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1264)。
  
#### Active Directory 证书服务漏洞 (CVE-2011-1264) 的缓解因素
...
#### Active Directory 证书服务漏洞 (CVE-2011-1264) 的变通办法
  
变通办法是指一种设置或配置更改，它不能从根本上纠正漏洞，但有助于在应用更新之前封堵已知的攻击源。 Microsoft 已测试了以下变通方法，并在讨论中指明了变通方法是否会降低功能性：
  
-   **为 Intrane 区域启用 Internet Explorer 8 和 Internet Explorer 9 XSS 筛选器**
  
    1.  在 Internet Explorer 的“**工具**”菜单上，单击“**Internet 选项**”。  
    2.  在“**Internet 选项**”对话框中，单击“**安全**”选项卡，然后单击“**本地 Intranet**”图标。  
    3.  在“**该区域的安全级别**”下，单击“**自定义级别**”。  
    4.  向下滚动到“**启用 XSS 筛选器**”（接近列表末尾）  
    5.  将默认的“**禁用**”更改为“**启用**”  
    6.  单击“**确定**”，然后在“**确实要更改该区域的设置吗？**”对话框上单击“**是**”。  
    7.  在“**Internet 选项**”对话框上单击“**确定**”。
  
    **变通办法的影响。** 以前未标记为 XSS 风险的内部站点将被标记。
...
#### Active Directory 证书服务漏洞 (CVE-2011-1264) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个可能允许特权提升的已反映的 XSS 漏洞。
  
**造成此漏洞的原因是什么？**    
该漏洞是由于在 Active Directory 证书服务 Web 注册站点上对请求参数不正确地执行输入验证导致的。
  
**什么是跨站点脚本执行 (XSS)？**    
跨站点脚本执行 (XSS) 是一种安全漏洞类别，允许攻击者在网页请求响应中注入脚本。 此脚本随后由发出请求的应用程序（通常是 Web 浏览器）运行。 该脚本可能欺骗内容、泄露信息或执行用户可以在受影响的网站上代表目标用户执行的任何操作。
  
**哪个组件受此漏洞的影响？**    
受影响的组件是 Active Directory 证书服务 Web 注册。
  
**攻击者可能利用此漏洞执行什么操作？**    
成功利用此漏洞的攻击者可以在用户的 Internet Explorer 实例中注入客户端脚本。 该脚本可能欺骗内容、泄露信息或执行用户可以在网站上代表目标用户执行的任何操作。
  
**攻击者如何利用此漏洞？**    
在电子邮件攻击情形中，攻击者可以通过向用户发送特制的链接并诱使用户单击该链接来利用此漏洞。
  
**受此漏洞威胁最大的系统有哪些？**    
运行 Active Directory 证书服务 Web 注册角色的服务器受到的威胁最大。 不运行此特定角色的服务器不受威胁。
  
**此更新有什么作用？**    
此更新通过更正 Active Directory 证书服务 Web 注册站点验证输入参数的方式来解决漏洞。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2518295$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2518295">Microsoft 知识库文章 2518295</a></td>
</tr>  
<tr class="even">
...
-   Ruggero Strabla、[Emaze Networks](https://www.emaze.net/)、[Saipem Security Team](https://www.saipem.com/) 报告了 Active Directory 证书服务漏洞 (CVE-2011-1264)
...
-   V1.1（2011 年 6 月 15 日）： 阐明了 XSS 筛选器缓解办法。
