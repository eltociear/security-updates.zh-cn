---
TOCTitle: 'MS11-033'
Title: 'Microsoft 安全公告 MS11-033 - 重要'
ms:assetid: 'ms11-033'
ms:contentKeyID: 61238171
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms11-033(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS11-033 - 重要
==================================

写字板文本转换器中的漏洞可能允许远程执行代码 (2485663)
------------------------------------------------------
...
此安全更新通过更改写字板文本转换器处理特制文件的方式来消除漏洞。 有关漏洞的详细信息，请参阅下一节“**漏洞信息**”下面特定漏洞条目的常见问题 (FAQ) 小节。
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------|--------------|--------------|-----------------------------------------------------------|
| [Windows XP Service Pack 3](https://www.microsoft.com/download/details.aspx?familyid=6753ca98-feb4-4c7f-9969-9294038a2bbb)                          | 远程执行代码 | 重要         | [MS10-067](https://go.microsoft.com/fwlink/?linkid=197102) |
| [Windows XP Professional x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=3c94bc96-99ea-44a1-9052-e69de5e21f81) | 远程执行代码 | 重要         | [MS10-067](https://go.microsoft.com/fwlink/?linkid=197102) |
| [Windows Server 2003 Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=9fbfc742-6c74-49a2-b3cc-e1d5d8c84b77)                 | 远程执行代码 | 重要         | [MS10-067](https://go.microsoft.com/fwlink/?linkid=197102) |
| [Windows Server 2003 x64 Edition Service Pack 2](https://www.microsoft.com/download/details.aspx?familyid=897b97b0-1bab-4b1b-b417-950fab0d4a65)     | 远程执行代码 | 重要         | [MS10-067](https://go.microsoft.com/fwlink/?linkid=197102) |
| [Windows Server 2003 SP2（用于基于 Itanium 的系统）](https://www.microsoft.com/download/details.aspx?familyid=ede38974-4e57-4ea1-8731-b91e96534693) | 远程执行代码 | 重要         | [MS10-067](https://go.microsoft.com/fwlink/?linkid=197102) |
...
<tr class="header">
<th>受影响的软件</th>
<th>写字板转换器分析漏洞 - CVE-2011-0028</th>
<th>综合严重等级</th>
</tr>
</thead>
<tbody>
...
写字板转换器分析漏洞 - CVE-2011-0028  
------------------------------------
  
<span></span>  
Microsoft 写字板分析特制 Word 文档的方式中存在一个远程执行代码漏洞。 如果用户打开带有格式错误的结构的特制 Word 文件，此漏洞可能允许远程执行代码。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
要在“常见漏洞和披露”列表中以标准条目查看此漏洞，请参阅 [CVE-2011-0028](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-0028)。
  
#### 写字板转换器分析漏洞 (CVE-2011-0028) 的缓解因素
  
缓解是指一种设置、通用配置或者一般的最佳实践，它以默认状态存在，能够降低利用漏洞的严重性。 以下缓解因素在您遇到的情形中可能会有所帮助：
  
-   成功利用此漏洞的攻击者可以获得与本地用户相同的用户权限。 那些帐户被配置为拥有较少系统用户权限的用户比具有管理用户权限的用户受到的影响要小。  
-   不会通过电子邮件自动利用此漏洞。 用户必须打开电子邮件附件，以电子邮件为载体的攻击才会得逞。 除非用户之前清除了选项“在打开这种类型的文件前始终询问”，否则系统可能会提示用户是否要打开文件。  
...
-   默认情况下，安装了 Microsoft Word 的系统通常使用 Microsoft Office（而不是写字板）打开 Word 文档。
  
#### 写字板转换器分析漏洞 (CVE-2011-0028) 的变通办法
  
变通办法是指一种设置或配置更改，它不能从根本上纠正漏洞，但有助于在应用更新之前封堵已知的攻击源。 Microsoft 已测试了以下变通方法，并在讨论中指明了变通方法是否会降低功能性：
  
-   通过禁止访问转换器文件来禁用写字板 Word 97 文本转换器。
  
    管理员可以向受影响的转换器应用访问控制列表，从而确保写字板不再加载转换器。 这样做可有效地防止借助此攻击媒介利用该问题。
  
    **警告** 请在安装此安全更新之前撤消此变通办法。
  
    要应用访问列表，请以管理员身份登录并从命令提示符运行以下命令。
  
    **注意** 其中某些命令可能导致出现错误消息；如果您尝试更改权限的文件不存在，就会发生这种情况。
    ...
    **变通办法的影响。** 实施此变通办法时，在写字板中打开 Word 文档会导致写字板中显示二进制数据而不是带格式文本。
  
    **如何撤消变通方法。** 以管理员身份登录并从命令提示符运行以下命令。
  
    **注意** 根据您的操作系统版本，以及操作系统升级和 Service Pack 的历史记录，下面提到的某些文件可能不存在；这是预期的情况。
...
#### 写字板转换器分析漏洞 (CVE-2011-0028) 的常见问题
  
**此漏洞的影响范围有多大？**    
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以完全远程控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。
  
**造成此漏洞的原因是什么？**    
漏洞是由于写字板文本转换器未正确分析 Word 文档中的特定字段造成的。
  
**什么是写字板文本转换器？**    
写字板是 Microsoft Windows 操作系统的一个默认组件。 写字板中的文本转换器允许没有安装 Microsoft Word 的用户以 Microsoft Windows Write (.wri) 和 Microsoft Word (.doc) 的文件格式打开文档。
...
在基于 Web 的攻击情形中，攻击者必须拥有一个网站，其中包含用来试图利用此漏洞的 Word 文档。 此外，接受或宿主用户提供的内容的网站以及受到破坏的网站可能包含可能利用此漏洞的特制内容。 攻击者无法强迫用户访问特制的网站。 相反，攻击者必须诱使用户访问该网站，所采用的方式通常是让用户单击指向攻击者网站的链接。 单击链接后，用户需要手动接受提示以打开文档。
  
**受此漏洞威胁最大的系统有哪些？**    
使用写字板打开来自不受信任来源的 Word 文档的受影响系统（例如工作站或终端服务器）受到的威胁最大。 如果管理员允许用户登录服务器并运行程序，服务器可能面临更大风险。 不过，最佳做法是不要授予这样的权限。 默认情况下，安装了 Microsoft Word 的系统通常使用 Microsoft Office（而不是写字板）打开 Word 文档。
  
**此更新有什么作用？**    
此更新通过更改写字板文本转换器处理特制文件的方式来消除漏洞。
  
**发布此安全公告时，此漏洞是否已公开披露？**    
否。 Microsoft 通过协调漏洞披露渠道了解到有关此漏洞的信息。 在最初发布此安全公告时，Microsoft 未收到任何表明此漏洞已公开披露的信息。
...
<td style="border:1px solid black;">使用控制面板中的“<strong>添加或删除程序</strong>”工具或 %Windir%\\$NTUninstallKB2485663$\Spuninst 文件夹中的 Spuninst.exe 实用程序</td>
</tr>  
<tr class="odd">
<td style="border:1px solid black;"><strong>文件信息</strong></td>
<td style="border:1px solid black;">请参阅 <a href="https://support.microsoft.com/kb/2485663">Microsoft 知识库文章 2485663</a></td>
</tr>  
<tr class="even">
...
-   [Secunia](https://secunia.com/) 的 Carsten Eiram 报告了写字板转换器分析漏洞 (CVE-2011-0028)
