...
|-------------------------------------------------------------------------------------------------------|------------|---------------------------|---------------------------|---------------------|------------------------------------|
| ICMP 连接重置漏洞 - [CVE-2004-0790](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2004-0790)  | 拒绝服务   | 低                        | 低                        | 低                  | 低                                 |
| TCP 连接重置漏洞 - [CVE-2004-0230](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2004-0230)   | 拒绝服务   | 低                        | 低                        | 低                  | 低                                 |
| 欺骗性连接请求漏洞 - [CVE-2005-0688](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-0688) | 拒绝服务   | 低                        | 低                        | 低                  | 低                                 |
| **所有漏洞的综合严重程度**                                                                            |            | **低**                    | **低**                    | **低**              | **低**                             |
...
**什么是 Teredo 互操作性更新？**  
由于 IANA 为 Teredo 分配了新的 Teredo 前缀 2001:0/32，以便通过 Teredo 在支持 Windows XP Service Pack 2 IPv6 的主机和 Windows Vista 之间建立连接，所有 Windows XP 计算机上的 Teredo 前缀将被修订。 要了解有关 Teredo 的更多信息，请访问以下 [Microsoft TechNet 文档](https://www.microsoft.com/technet/prodtechnol/winxppro/maintain/teredo.mspx)。 要了解有关新的 Teredo 前缀的更多信息，请访问以下 [RFC 文档](https://www.rfc-editor.org/rfc/rfc4380.txt)。

**是否存在任何影响 TCP/IP 的 IPv4 实施的更改或漏洞？**  
否。 发布 [MS05-019](https://technet.microsoft.com/security/bulletin/ms05-019) 之后，本公告涉及的安全问题已在 TCP/IP 相应的 IPv4 实施中得到解决。
...
IPv6 实施中存在一个[拒绝服务](https://go.microsoft.com/fwlink/?linkid=21142)漏洞，该漏洞可能允许攻击者向受影响系统发送特制的 Internet 控制消息协议 (ICMP) 消息。 成功利用此漏洞的攻击者可能导致受影响系统重置现有的 TCP 连接。
...
-   **在防火墙或路由器处禁止所有 ICMP 网络数据包：**
    ...
    **变通办法的影响：** 此变通办法也防止 TCP 优化网络通信，可能给性能带来负面影响。 在连接具有不同 MTU 的网络的路由器中，ICMP 网络数据能够消除拆分。 拆分会降低 TCP 吞吐量并增加网络拥塞。
    注意： Windows XP Service Pack 1 防火墙不能处理 IPv6 网络通信。 为了确保使用 Internet 连接防火墙保护您的 Windows XP Service Pack 1 系统，您应该应用 [KB 文件 817778“Advanced Networking Pack for Windows XP 概述”](https://support.microsoft.com/kb/817778)中识别的更新。
...
**什么是 IPv6？**  
IPv6 是当前 Internet 协议 (IP) 版本 IPv4 的升级。此新版本 IP 版本 6 (IPv6) 可解决非预期或的 IPv4 设计问题。 要了解有关 IPv6 的更多信息，请阅读以下 [Microsoft IPv6 常见问题解答](https://www.microsoft.com/technet/itsolutions/network/ipv6/ipv6faq.mspx)。
...
是。 攻击者可能试图通过 Internet 来利用此漏洞。 默认情况下，Windows XP Service Pack 1 和 Windows Server 2003 中的 Microsoft Internet 连接防火墙 (ICF) 允许这类恶意网络数据包通过并且无法过滤它们。 Windows XP Service Pack 2 和 Windows Server 2003 Service Pack 1 中的防火墙组件“Windows 防火墙”能够阻止此通讯。 如果您正在 Windows XP Service Pack 1 上运行 IPv6，则应该应用 [KB 文章 817778“Advanced Networking Pack for Windows XP 概述”](https://support.microsoft.com/kb/817778)中识别的更新，以获取经过更新的、能够处理 IPv6 通讯的 Internet 连接防火墙。 。
...
IPv6 实施中存在一个[拒绝服务](https://go.microsoft.com/fwlink/?linkid=21142)漏洞，该漏洞可能允许攻击者向受影响系统发送特制的 TCP 消息。 成功利用此漏洞的攻击者可能会导致受影响系统重置现有的 TCP 连接。
...
IPv6 实施中存在一个[拒绝服务](https://go.microsoft.com/fwlink/?linkid=21142)漏洞，该漏洞可能允许攻击者向受影响系统发送特制的 TCP/IP 消息。 成功利用此漏洞的攻击者可能会导致受影响系统停止响应。
...
这是一个[拒绝服务](https://go.microsoft.com/fwlink/?linkid=21142)漏洞。 利用此漏洞的攻击者可能导致受影响的系统在限定时间内停止响应，从而导致 CPU 过度利用。 在此期间，受影响的系统无法响应请求。 请注意，虽然攻击者无法利用拒绝服务漏洞来执行代码或提升他们的用户权限，但此漏洞可能导致受影响的系统停止接受请求。
...
**什么是 IPv6？**  
IPv6 是当前 Internet 协议 (IP) 版本 IPv4 的升级。此新版本 IP 版本 6 (IPv6) 可解决非预期或的 IPv4 设计问题。 要了解有关 IPv6 的更多信息，请阅读以下 [IPv6 常见问题解答](https://www.microsoft.com/technet/itsolutions/network/ipv6/ipv6faq.mspx)。
