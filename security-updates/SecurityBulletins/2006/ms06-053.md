---
TOCTitle: 'MS06-053'
Title: 'Microsoft 安全公告 MS06-053 - 警告'
ms:assetid: 'ms06-053'
ms:contentKeyID: 61236820
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms06-053(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS06-053 - 警告
==================================
...
-   Microsoft Windows 2000 Service Pack 4 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=778294ae-c5e3-4f17-b0e4-308e46e00105)
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=2731c0bf-6034-4c16-bb57-66e70a31a3d6)
-   Microsoft Windows XP Professional x64 Edition — [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=3f604b2a-1383-4a45-b25b-c468deefbfc1)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=0182e8e7-9755-46cc-a393-c1e95fd508b2)
-   Microsoft Windows Server 2003（用于基于 Itanium 的系统）和 Microsoft Windows Server 2003 SP1（用于基于 Itanium 的系统）— [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=e3e4a66c-ca9d-453b-8875-fb57528117ac)
-   Microsoft Windows Server 2003 x64 Edition — [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=acf35f34-0d26-4b79-b81f-1111a784a66d)
...
该漏洞可能允许攻击者未经授权访问信息。 注意，虽然攻击者无法利用此漏洞来执行代码提升他们的用户权限，但此漏洞可用于生成有用信息，这些信息可用于进一步危及受影响系统的安全。
...
|-----------------------------------------------------------------------------------------------------------|------------|-----------------------------|---------------------------|---------------------------|---------------------|------------------------------------|
| Microsoft 索引服务漏洞 - [CVE-2006-0032](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-0032) | 信息泄露   | 中等                        | 中等                      | 中等                      | 低                  | 低                                 |
...
|-------------------------------------------------------------------------|-----------------------------|---------------------------|---------------------------|---------------------|------------------------------------|
| [**MS05-003**](https://technet.microsoft.com/security/bulletin/ms05-003) | 未取代                      | 已替代                    | 已替代                    | 已替代              | 不适用                             |
...
#### Microsoft 索引服务漏洞 - CVE-2006-0032：

索引服务由于处理查询验证的方式而存在一个信息泄露漏洞。 该漏洞可能允许攻击者代表用户运行客户端脚本。 该脚本可能欺骗内容、泄露信息或执行用户可以在受影响的网站上执行的任何操作。

#### Microsoft 索引服务漏洞 (CVE-2006-0032) 的缓解因素：

-   默认情况下，Windows XP 或 Windows Server 2003 上不安装 Internet 信息服务 (IIS)。
-   在 Windows Server 2003 上，默认情况下不启用索引服务。
-   在 Windows Server 2003 上，即使安装了索引服务，默认情况下仍然无法从 IIS 访问该服务。 要使 IIS 成为索引服务基于 Web 的界面，需要手动操作。 默认情况下，索引服仅用于执行本地和远程文件系统查询。

#### Microsoft 索引服务漏洞 (CVE-2006-0032) 的变通办法：

Microsoft 已测试以下变通办法。 尽管这些变通办法不能从根本上消除此漏洞，但它们有助于阻止已知的攻击媒介。 如果某种变通办法导致功能下降，下一节中将进行说明。

-   **不要从服务器角色的系统浏览 Internet**
    不要从 Web 服务器或任何其他服务器角色的系统浏览 Internet。
-   **禁用 Internet Explorer 中的页面编码自动检测**
    禁用 Internet Explorer 中的页面编码自动检测可帮助防止受影响的系统尝试利用此漏洞。

    要配置组件和服务：

    1.  启动 Internet Explorer。
    2.  依次单击“**查看**”、“**编码**”，如果“**自动选择**”旁边有复选标记，则左键单击“**自动选择**”以取消选择此设置。

    **变通办法的影响：** Internet Explorer 可能无法正确地检测没有指定默认字符编码的网页上使用的编码，从而导致不正确地显示编码字符。 用户仍然可以手动选择正确的编码类型来正确显示页面。

-   **使用运行 IIS 5.0 的 Windows 2000 上的 URLScan**
    安装 [URLScan](https://www.microsoft.com/technet/security/tools/urlscan.mspx) 可帮助防止受影响的系统尝试利用此漏洞。

    1.  如果在其默认配置中安装 URLScan，URLScan 2.5 将阻止影响索引服务器扩展的 .IDA、.IDQ 和 .HTW 文件的请求。
    2.  重新启动 IISAdmin 和 WWW 发布服务，更改才会生效。

    **变通办法的影响：** URLScan 的默认配置将阻止索引服务器文件类型（.IDA、.IDQ、.HTW）的请求，因此将阻止任何利用这些文件类型的基于 Web 的搜索。

-   **从运行 IIS 5.0 的 Windows 2000 上的 Internet 信息服务中删除索引服务器 ISAPI 扩展脚本映射**
    从 IIS 中删除索引服务器 ISAPI 扩展脚本映射有助于防止受影响的系统尝试利用此漏洞。

    1.  单击“**开始**”，然后单击“**控制面板** ”。或者，指向“**设置**”，然后单击“**控制面板**”。
    2.  双击“**管理工具**”。
    3.  双击“**Internet 信息服务**”。
    4.  右键单击 WebServer，选择“**属性**”。
    5.  依次选择“**主属性**”、“**WWW 服务**”，然后单击“**编辑**”。
    6.  在“**WWW ServiceMaster 属性**”上，选择“**主目录**”，然后单击“**配置**”。
    7.  在“**应用程序配置**”上，突出显示 .HTW、.IDA 和 .IDQ 扩展名，然后单击“**删除**”。

    **变通办法的影响：** URLScan 的默认配置将阻止索引服务器文件类型（.IDA、.IDQ、.HTW）的请求，因此将阻止任何利用这些文件类型的基于 Web 的搜索。

-   **删除索引服务**
    **如果不再需要索引服务，可以按照下列过程删除它。**
    ...
    4.  单击以清除“索引服务”复选框，进而删除索引服务。
    5.  按照屏幕上的说明操作，完成 Windows 组件向导。

    **变通办法的影响：** 如果删除此服务，则系统将通过遍历文件夹层次结构并针对请求的字符串扫描每个文件来提供所有搜索功能，搜索响应速度将变慢。 如果 MMC 索引服务管理单元用于创建新的目录，则目录将保持脱机状态，直到启动此服务。

-   **从运行 IIS 6.0 的 Windows 2003 上的 IIS 中禁用索引服务扩展**
    **如果不再需要索引服务扩展，则可以通过执行下列步骤禁用它。**
    ...
    **变通办法的影响：** 如果删除此服务扩展，则系统将通过遍历文件夹层次结构并针对请求的字符串扫描每个文件来提供所有搜索功能，搜索响应速度将变慢。 如果 MMC 索引服务管理单元用于创建新的目录，则目录将保持脱机状态，直到启动此服务。

#### Microsoft 索引服务漏洞 (CVE-2006-0032) 的常见问题解答：

**此漏洞的影响范围有多大？**  
索引服务由于处理查询验证的方式而存在一个信息泄露漏洞。 该漏洞可能允许攻击者代表用户运行客户端脚本。 该脚本可能欺骗内容、泄露信息或执行用户可以在受影响的网站上执行的任何操作。

**造成漏洞的原因是什么？**  
索引服务不正确地验证查询参数，可能允许跨站点脚本执行。

**什么是索引服务？**  
索引服务是针对受影响的操作系统实施的一项基本服务。 以前也称作索引服务器，它最初的功能是创建 Internet 信息服务 (IIS) Web 服务器的内容索引。 现在，索引服务可以为文件系统和虚拟 Web 服务器创建内容和属性的索引目录。索引服务可用于应用程序和脚本，从而更高效地管理和查询文件系统或 Web 服务器中的信息并为这些信息编制索引。 索引服务还会提供许多查询机制，从而更高效地访问目录中的信息。 这些索引信息是在使用 Microsoft 提供的筛选器以及客户提供的筛选器（可选）对文件系统和 Web 服务器进行筛选的过程中生成的。

**什么是跨站点脚本？**  
跨站点脚本 (XSS) 是一个安全漏洞，它可能会使攻击者能够将代码“注入”用户与网站进行的会话。 攻击包括动态生成 HTML 页的 Web 服务器。 如果这些服务器在其发送回浏览器的动态页面中嵌入浏览器输入，则攻击者便可操纵这些服务器，并在动态页面中放置内容。 该内容允许执行恶意脚本。 由于 Web 浏览器对“受信任”站点的基本假设和使用 cookies 维持与其经常访问的网站的永久状态，因此使此问题永久性延续下去。 此攻击不修改网站内容。 相反，它将插入新的恶意脚本，该脚本可以在与受信任服务器关联的安全上下文中的浏览器执行。
...
**攻击者可能利用此漏洞执行什么操作？**  
成功利用此漏洞的攻击者可以运行恶意脚本。 如果运行此恶意脚本，该脚本将在客户端上用户的安全上下文中运行。 该脚本可能在用户的计算机上执行授权网站执行的任何操作。 这些操作可能包括监视用户的 Web 会话并将信息转发给第三方、在用户的系统上运行其他代码以及读取或写入 cookie。

**攻击者能够如何利用此漏洞？**  
在基于 Web 的攻击情形中，攻击者可能拥有一个网站，并在上面放置用来利用此漏洞的网页。 另外，接受或宿主用户提供的内容或广告的网站以及受到破坏的网站可能包含可能利用此漏洞的特制内容。 但是在所有情况下，攻击者无法强制用户访问这些网站。 相反，攻击者必须说服用户访问该网站，方法通常是让用户单击电子邮件或 Instant Messenger 消息中的链接以使用户链接到攻击者的网站。

**受此漏洞威胁最大的系统有哪些？**  
可通过 IIS 访问其索引服务的系统也可能会因此漏洞而受到来自 Internet 的攻击。 如果未启用索引服务，则系统将不会因此问题而受到攻击。

客户端系统可能允许攻击者代表用户运行客户端脚本。 该脚本可能欺骗内容、泄露信息或执行用户可以在受影响的网站上执行的任何操作。

**此更新有什么作用？**  
此更新通过修改索引服务验证查询参数的方式来消除此漏洞。
...
**注意** 使用 **/quiet** 开关将抑制所有消息。 这包括抑制失败消息。 管理员应该使用其中一种支持的方法，验证使用 **/quiet** 开关时安装是否成功。 管理员还应该检查 KB920685.log 文件，查看使用此开关时的任何失败消息。
...
系统管理员也可以使用 Spuninst.exe 实用工具删除此安全更新。 Spuninst.exe 实用工具位于 %Windir%\\$NTUninstallKB920685$\\Spuninst 文件夹中。
...
-   **注册表项验证**
    ...
    **注意** 此注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 920685 安全更新集成或组合到 Windows 安装源文件中，则可能无法正确创建此注册表项。
...
-   **注册表项验证**
    ...
    **注意** 这些注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 920685 安全更新集成或组合到 Windows 安装源文件中，则可能无法正确创建这些注册表项。
...
-   Eiji James Yoshida 报告了 Microsoft 索引服务漏洞 - [CVE-2006-0032](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-0032)。
