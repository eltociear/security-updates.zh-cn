---
TOCTitle: 'MS06-074'
Title: 'Microsoft 安全公告 MS06-074 - 重要'
ms:assetid: 'ms06-074'
ms:contentKeyID: 61236845
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms06-074(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS06-074 - 重要
==================================
...
-   Microsoft Windows 2000 Service Pack 4 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=ecc1e691-626b-405c-87d1-15931d1e4258)
-   Microsoft Windows XP Service Pack 2 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=2b57e00f-0f47-4567-b40f-f630ba5a29cb)
-   Microsoft Windows XP Professional x64 Edition — [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=65ab5876-7c9a-4add-8b6d-0fd7d617397a)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=7856ee11-4f3a-4138-bfce-1b97fb25be69)
-   Microsoft Windows Server 2003（用于基于 Itanium 的系统）和 Microsoft Windows Server 2003 SP1（用于基于 Itanium 的系统）— [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=c1b01b91-c565-4d1f-90ec-f57a70fa012e)
-   Microsoft Windows Server 2003 x64 Edition — [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=5b61249a-dba7-4fd5-85f3-b918044bbc92)
...
<tbody>
<tr class="odd">
<td style="border:1px solid black;">SNMP 内存损坏漏洞 - <a href="https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-5583">CVE-2006-5583</a></td>
...
-   Windows Server 2003 Service Pack 1（用于基于 Itanium 的系统）的严重等级与 Windows Server 2003 Service Pack 1 的严重等级相同。
...
#### SNMP 内存损坏漏洞 - CVE-2006-5583：

SNMP 服务中存在远程执行代码漏洞，该漏洞允许成功利用此漏洞的攻击者完全控制受影响的系统。

#### SNMP 内存损坏漏洞 (CVE-2006-5583) 的缓解因素：

-   默认情况下不会安装 SNMP 服务
-   对于需要受影响组件的客户，采用防火墙最佳做法和标准的默认防火墙配置可以帮助保护网络免受从企业外部发起的攻击。 按照最佳做法，应使连接到 Internet 的系统所暴露的端口数尽可能少。

#### SNMP 内存损坏漏洞 (CVE-2006-5583) 的变通办法：
...
-   **限制有权管理计算机的 IP 地址。**
    ...
    3.  单击 **SNMP Service**，然后选择“**属性**”。
    4.  单击“**安全**”选项卡，然后选择“**接受来自这些主机的 SNMP 数据包**”。
    5.  添加经过批准的管理站的 IP 地址，方法是：单击“**添加**”，键入 IP 地址或主机名称，然后单击“**添加**”。
-   **在防火墙处阻止以下内容：**

    -   UDP 端口 161

    此端口用于启动与受影响组件的连接。 将其阻止在防火墙处，有助于保护位于防火墙后面的系统尝试利用此漏洞。 此外，确保阻止远程系统上任何其他特殊配置的 SNMP 端口。 我们建议阻止所有来自 Internet 的未经请求的入站通信，以帮助防止可能使用其他端口进行的攻击。
...
-   **禁用 SNMP 服务**
    ...
    禁用 SNMP 服务将帮助防止受影响的系统尝试利用此漏洞。 要禁用 SNMP 服务，请按照下列步骤执行操作：
    ...
    您还可以通过在命令提示符处使用以下命令来停止和禁用 SNM 服务：

    `sc stop SNMP & sc config SNMP start= disabled`

-   **变通办法的影响：** 如果禁用 SNMP 服务，则不能在本地或远程打印。 因此，建议仅在不需要打印的系统上使用此变通方法。

#### SNMP 内存损坏漏洞 (CVE-2006-5583) 的常见问题：
...
简单网络管理协议 ([SNMP](https://search.msdn.microsoft.com/search/redirect.aspx?title=snmp+functions+%5bsnmp%5d&url=https://msdn.microsoft.com/library/en-us/snmp/snmp/snmp_functions.asp)) 允许管理员远程管理网络设备，例如服务器、工作站、路由器、桥、防火墙，等等。 SNMP 是一项行业标准协议，允许通过协议管理许多不同供应商提供的设备。

**什么是 SNMP 服务？**  

SNMP 服务允许本地计算机向传入（简单网络管理协议）SNMP 请求提供服务。 SNMP 包括监视网络设备中的活动并向网络控制台工作站报告的代理程序。

**如何知道我的系统上是否安装了 SNMP 服务？**  

您可以通过执行下列操作确认在系统上安装了 SNMP 服务：

1.  单击“**开始**”，然后单击“**运行**”。
2.  在“**打开**”框中，键入 **services.msc**，然后单击“**确定**”。
3.  在服务列表中搜索 **SNMP Service**。

如果列出了 SNMP Service，则表示已经安装了该服务。
...
此更新通过修改 SNMP 服务在将消息传递到分配的缓冲区之前验证消息长度的方式来消除此漏洞。
...
**注意** 使用 **/quiet** 开关将抑制所有消息。 这包括抑制失败消息。 管理员应该使用其中一种支持的方法，验证使用 **/quiet** 开关时安装是否成功。 管理员还应该检查 KB926247.log 文件，查看使用此开关时的任何失败消息。
...
系统管理员也可以使用 Spuninst.exe 实用工具删除此安全更新。 Spuninst.exe 实用工具位于 %Windir%\\$NTUninstallKB926247$\\Spuninst 文件夹中。
...
| 文件名   | 版本          | 日期                | 时间  | 大小   |
|----------|---------------|---------------------|-------|--------|
| Snmp.exe | 5.0.2195.7112 | 2006 年 10 月 11 日 | 07:49 | 30,480 |
...
| 文件名   | 版本          | 日期                | 时间  | 大小   | 文件夹 |
|----------|---------------|---------------------|-------|--------|--------|
| Snmp.exe | 5.1.2600.3038 | 2006 年 11 月 20 日 | 14:12 | 33,280 | SP2GDR |
| Snmp.exe | 5.1.2600.3038 | 2006 年 11 月 20 日 | 15:47 | 33,280 | SP2QFE |

Windows XP Professional x64：

| 文件名    | 版本          | 日期                | 时间  | 大小   | CPU | 文件夹      |
|-----------|---------------|---------------------|-------|--------|-----|-------------|
| Snmp.exe  | 5.2.3790.2837 | 2006 年 11 月 21 日 | 21:07 | 60,928 | x64 | SP1GDR      |
| Wsnmp.exe | 5.2.3790.2837 | 2006 年 11 月 21 日 | 21:07 | 46,080 | x86 | SP1GDR\\WOW |
| Snmp.exe  | 5.2.3790.2837 | 2006 年 11 月 21 日 | 20:56 | 60,928 | x64 | SP1QFE      |
| Wsnmp.exe | 5.2.3790.2837 | 2006 年 11 月 21 日 | 20:57 | 40,960 | x86 | SP1QFE\\WOW |
...
**注意** 使用 **/quiet** 开关将抑制所有消息。 这包括抑制失败消息。 管理员应该使用其中一种支持的方法，验证使用 **/quiet** 开关时安装是否成功。 管理员还应该检查 KB**926247**.log 文件，查看使用此开关时的任何失败消息。
...
此安全更新支持 HotPatching。 有关 HotPatching 的详细信息，请参阅 [Microsoft 知识库文章 897341](https://support.microsoft.com/kb/897341)。
...
| 文件名      | 版本          | 日期                | 时间  | 大小   | CPU | 文件夹 |
|-------------|---------------|---------------------|-------|--------|-----|--------|
| Snmp.exe    | 5.2.3790.615  | 2006 年 11 月 18 日 | 17:27 | 32,256 | x86 | RTMGDR |
| Snmp.exe    | 5.2.3790.615  | 2006 年 11 月 18 日 | 17:45 | 32,256 | x86 | RTMQFE |
| Snmp.exe    | 5.2.3790.2837 | 2006 年 11 月 21 日 | 20:11 | 46,080 | x86 | SP1GDR |
| Snmp.hp.exe | 5.2.3790.2837 | 2006 年 11 月 21 日 | 20:11 | 6,144  | x86 | SP1GDR |
| Snmp.exe    | 5.2.3790.2837 | 2006 年 11 月 21 日 | 15:53 | 40,960 | x86 | SP1QFE |
...
| 文件名    | 版本          | 日期                | 时间  | 大小    | CPU   | 文件夹      |
|-----------|---------------|---------------------|-------|---------|-------|-------------|
| Snmp.exe  | 5.2.3790.615  | 2006 年 11 月 21 日 | 20:57 | 97,280  | IA-64 | RTMGDR      |
| Snmp.exe  | 5.2.3790.615  | 2006 年 11 月 21 日 | 20:57 | 97,280  | IA-64 | RTMQFE      |
| Snmp.exe  | 5.2.3790.2837 | 2006 年 11 月 21 日 | 21:02 | 116,224 | IA-64 | SP1GDR      |
| Wsnmp.exe | 5.2.3790.2837 | 2006 年 11 月 21 日 | 21:02 | 46,080  | x86   | SP1GDR\\WOW |
| Snmp.exe  | 5.2.3790.2837 | 2006 年 11 月 21 日 | 21:00 | 116,224 | IA-64 | SP1QFE      |
| Wsnmp.exe | 5.2.3790.2837 | 2006 年 11 月 21 日 | 21:00 | 40,960  | x86   | SP1QFE\\WOW |
...
-   [European Aeronautic Defense and Space Company](https://www.eads.com/) 的 Kostya Kortchinsky 报告了 SNMP 内存损坏漏洞 (CVE-2006-5583)。
