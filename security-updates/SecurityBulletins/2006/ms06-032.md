---
TOCTitle: 'MS06-032'
Title: 'Microsoft 安全公告 MS06-032 - 重要'
ms:assetid: 'ms06-032'
ms:contentKeyID: 61236795
ms:mtpsurl: 'https://technet.microsoft.com/zh-CN/library/ms06-032(v=Security.10)'
---

Security Bulletin

Microsoft 安全公告 MS06-032 - 重要
==================================
...
**注意事项：** [Microsoft 知识库文章 917953](https://support.microsoft.com/kb/917953) 介绍了客户在安装此安全更新时可能遇到的当前已知问题。 本文还介绍了这些问题的建议解决办法。 有关详细信息，请参阅 [Microsoft 知识库文章 917953](https://support.microsoft.com/kb/917953)。

**经过测试的软件和安全更新下载位置：**

**受影响的软件：**

-   Microsoft Windows 2000 Service Pack 4 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=305e208c-d75c-471b-9e57-30d01e320ad1)
-   Microsoft Windows XP Service Pack 1 和 Microsoft Windows XP Service Pack 2 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=b62abe8e-4735-4934-a66e-5b957986efbf)
-   Microsoft Windows XP Professional x64 Edition — [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=44213900-9082-45dc-b514-31d38717fe89)
-   Microsoft Windows Server 2003 和 Microsoft Windows Server 2003 Service Pack 1 — [下载此更新](https://www.microsoft.com/download/details.aspx?displaylang=zh-cn&familyid=ea319c61-b405-41ab-9eee-d5b3488b90e0)
-   Microsoft Windows Server 2003（用于基于 Itanium 的系统）和 Microsoft Windows Server 2003 SP1（用于基于 Itanium 的系统）— [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=cd8b88b5-f90f-4c0c-a5ad-3641751381c9)
-   Microsoft Windows Server 2003 x64 Edition — [下载此更新](https://www.microsoft.com/download/details.aspx?familyid=cd8699bc-6760-4f0e-b8e0-2e7d89092ce8)
...
|--------------------------------------------------------------------------------------------------|--------------|--------------|---------------------------|---------------------------|---------------------|------------------------------------|
| IP 源路由漏洞 - [CVE-2006-2379](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-2379) | 远程执行代码 | 重要         | 重要                      | 中等                      | 重要                | 中等                               |
...
|--------------|--------------|---------------------------|---------------------------|---------------------|------------------------------------|
| **MS06-007** | 不适用       | 已取代                    | 已取代                    | 已取代              | 已取代                             |
| **MS05-019** | 已取代       | 未取代                    | 已取代                    | 已取代              | 不适用                             |
...
对于 SMS 2.0，SMS SUS 功能包（包含安全更新清单工具）可由 SMS 用来检测安全更新。 SMS UIT 使用 MBSA 1.2.1 引擎进行检测。 有关安全更新清单工具的详细信息，请访问以下[Microsoft 网站](https://support.microsoft.com/kb/894154/)。 有关安全更新清单工具限制的详细信息，请参阅 [Microsoft 知识库文章 306460](https://support.microsoft.com/kb/306460/)。SMS SUS 功能包还包含用于检测 Microsoft Office 应用程序更新的 Microsoft Office 清单工具。
...
#### IP 源路由漏洞 - CVE-2006-2379：

TCP/IP 协议驱动程序中存在一个远程执行代码漏洞，成功利用此漏洞的攻击者可以完全控制受影响的系统。

#### IP 源路由漏洞 (CVE-2006-2379) 的缓解因素：

-   采用防火墙最佳做法和标准的默认防火墙配置，有助于保护网络免受从企业外部发起的攻击。 按照最佳做法，应使连接到 Internet 的系统所暴露的端口数尽可能少。
-   只有当启用 IP 源路由时，系统才会受到威胁。 Windows XP Service Pack 2 和 Windows Server 2003 Service Pack 1 系统上禁用 IP 源路由。
-   启用 Routing and Remote Access 服务将会启用 IP 源路由。  但是，任何受影响的操作系统版本上会禁用路由和远程访问。
-   尝试利用此漏洞的攻击最有可能导致拒绝服务情况。 但是，很有可能是远程执行代码。

#### IP 源路由漏洞 (CVE-2006-2379) 的变通办法：

Microsoft 已测试以下变通办法。 尽管这些变通办法不能从根本上消除此漏洞，但它们有助于阻止已知的攻击媒介。 如果某种变通办法导致功能下降，下一节中将进行说明。

-   **在防火墙处阻止包含 IP 源路由选项 131 和 137 的 IP 数据包**

    包含 IP 源路由选项 131 和 137 的 IP 网络数据包可用于启动与受影响组件的连接。 在防火墙或路由器处阻止符合这些条件的网络数据包，有助于保护防火墙或路由器后面的系统免受尝试利用此漏洞进行的攻击。 我们建议您阻止所有来自 Internet 的非法入站通信。 ISA Server 2004 可用于阻止受影响的通信类型。
...
-   **禁用 IP 源路由**
    禁用 IP 源路由将防止受影响的主机处理可能允许攻击者执行代码的 IP 源相关数据包。 可通过执行下列步骤禁用 IP 源路由处理：
    ...
    **注意** 我们建议在编辑注册表之前先对其进行备份。
    ...
    3.  添加双字节值： **DisableIPSourceRouting**。 将该值设置为 **2**。此值禁用 IP 源路由处理。 默认情况下不出现此项。
    4.  要使此更改生效，必须重新启动您的系统。

有关禁用 IP 源路由的其他信息，请阅读 MSDN 文章“ [如何强化 TCP/IP 堆栈”中列出的“其他保护”信息。](https://msdn.microsoft.com/library/default.asp?url=/library/en-us/dnnetsec/html/hthardtcp.asp)

#### IP 源路由漏洞 (CVE-2006-2379) 的常见问题解答：

**此漏洞的影响范围有多大？**  
这是一个远程执行代码漏洞。 成功利用此漏洞的攻击者可以远程完全控制受影响的系统。 攻击者可随后安装程序；查看、更改或删除数据；或者创建拥有完全用户权限的新帐户。

**造成漏洞的原因是什么？**  
TCP/IP 协议驱动程序中未经检查的缓冲区。

**什么是 IP 源路由？**  
IP 源路由是一种机制，允许发送者确定数据报通过网络应采用的 IP 路由。
...
**此更新有什么作用？**  
此更新通过修改 TCP/IP 协议驱动程序在将消息传递到分配的缓冲区之前验证消息长度的方式来消除此漏洞。
...
**注意** 使用 **/quiet** 开关将抑制所有消息。 这包括抑制失败消息。 管理员应该使用其中一种支持的方法，验证使用 **/quiet** 开关时安装是否成功。 管理员还应该检查 KB917953.log 文件，查看使用此开关时的任何失败消息。
...
系统管理员也可以使用 Spuninst.exe 实用工具删除此安全更新。 Spuninst.exe 实用工具位于 %Windir%\\$NTUninstallKB917953$\\Spuninst 文件夹中。
...
| 文件名    | 版本          | 日期               | 时间  | 大小    | 文件夹 |
|-----------|---------------|--------------------|-------|---------|--------|
| Tcpip.sys | 5.2.3790.537  | 2006 年 5 月 20 日 | 00:27 | 317,440 | RTMGDR |
| Tcpip.sys | 5.2.3790.537  | 2006 年 5 月 20 日 | 00:39 | 318,464 | RTMQFE |
| Tcpip.sys | 5.2.3790.2709 | 2006 年 5 月 24 日 | 01:05 | 333,312 | SP1GDR |
| Tcpip.sys | 5.2.3790.2709 | 2006 年 5 月 24 日 | 01:20 | 386,048 | SP1QFE |
...
| 文件名    | 版本          | 日期               | 时间  | 大小      | 文件夹 |
|-----------|---------------|--------------------|-------|-----------|--------|
| Tcpip.sys | 5.2.3790.537  | 2006 年 5 月 24 日 | 20:34 | 973,824   | RTMGDR |
| Tcpip.sys | 5.2.3790.537  | 2006 年 5 月 24 日 | 20:34 | 977,920   | RTMQFE |
| Tcpip.sys | 5.2.3790.2709 | 2006 年 5 月 24 日 | 20:37 | 1,116,160 | SP1GDR |
| Tcpip.sys | 5.2.3790.2709 | 2006 年 5 月 24 日 | 20:34 | 1,285,120 | SP1QFE |
...
| 文件名    | 版本          | 日期               | 时间  | 大小    | 文件夹 |
|-----------|---------------|--------------------|-------|---------|--------|
| Tcpip.sys | 5.2.3790.2709 | 2006 年 5 月 24 日 | 20:38 | 703,488 | SP1GDR |
| Tcpip.sys | 5.2.3790.2709 | 2006 年 5 月 24 日 | 20:34 | 812,544 | SP1QFE |
...
-   **注册表项验证**
    ...
    **注意** 此注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 917953 安全更新集成或组合到 Windows 安装源文件中，则可能无法正确创建此注册表项。
...
| 文件名    | 版本          | 日期               | 时间  | 大小    | 文件夹 |
|-----------|---------------|--------------------|-------|---------|--------|
| Tcpip.sys | 5.1.2600.1831 | 2006 年 4 月 20 日 | 11:38 | 340,480 | SP1QFE |
| Tcpip.sys | 5.1.2600.2892 | 2006 年 4 月 20 日 | 11:51 | 359,808 | SP2GDR |
| Tcpip.sys | 5.1.2600.2892 | 2006 年 4 月 20 日 | 12:18 | 360,576 | SP2QFE |
...
-   **注册表项验证**
    ...
    **注意** 这些注册表项可能不包含已安装文件的完整列表。 另外，如果管理员或 OEM 将 917953 安全更新集成或组合到 Windows 安装源文件中，则可能无法正确创建这些注册表项。
...
| 文件名    | 版本          | 日期               | 时间  | 大小    |
|-----------|---------------|--------------------|-------|---------|
| Tcpip.sys | 5.0.2195.7087 | 2006 年 4 月 26 日 | 02:08 | 320,336 |
...
-   [Andrey Minaev](https://www.microsoft.com/technet/security/bulletin/邮件地址：angel3000@hotbox.ru) 报告了 IP 源路由漏洞 (CVE-2006-2379)。
...
-   V1.1（2006 年 6 月 21 日）： 更新了“与此安全更新相关的常见问题解答 (FAQ)”部分，阐述 MS05-019 公告替代。 另外还更新了公告的“漏洞详细资料”部分，提供有关禁用 IP 源路由的附加信息。
