...
此安全更新修复了 Adobe 安全公告 [APSB16-39](http://helpx.adobe.com/cn/security/products/flash-player/apsb16-39.html) 中描述的以下漏洞：
...
\*“替代的更新程序”列仅显示一系列被取代的更新程序中最新的更新程序。有关已替换更新的完整列表，请转到 [Microsoft 更新目录](http://catalog.update.microsoft.com/v7/site/home.aspx)，搜索更新知识库文章编号，然后查看更新详细信息（已替换更新的信息位于“程序包详情”选项卡中）。
...
考虑用户在 Windows 8 风格 UI 中使用 Internet Explorer 的情形——在此基于 Web 的攻击场景中，攻击者首先要破坏兼容性视图 (CV) 列表上已列出的某个网站。然后，攻击者可能托管包含旨在通过 Internet Explorer 利用其中任何漏洞的经特殊设计的 Flash 内容的网站，然后诱使用户查看该网站。攻击者无法强制用户查看由攻击者控制的内容。相反，攻击者必须诱使用户采取行动，方法通常是让用户单击电子邮件或即时消息中的链接，以使用户链接到攻击者的网站，或者让用户打开通过电子邮件发送的附件。有关 Internet Explorer 和 CV 列表的详细信息，请参阅 MSDN 文章[包含适用于 Windows 8 的 Adobe Flash Player 内容的网站的开发人员指南](http://msdn.microsoft.com/zh-cn/library/ie/jj193557(v=vs.85).aspx)。
